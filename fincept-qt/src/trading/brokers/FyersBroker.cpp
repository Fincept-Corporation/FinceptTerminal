...
TokenExchangeResponse FyersBroker::exchange_token(const QString& api_key,
    ...
    auto resp = BrokerHttp::instance().post_json(
        QString(base_url()) + "/api/v3/validate-authcode", payload);
...
OrderPlaceResponse FyersBroker::place_order(const BrokerCredentials& creds,
    ...
    auto resp = BrokerHttp::instance().post_json(
        QString(base_url()) + "/api/v3/orders", payload, auth_headers(creds));
...
ApiResponse<QJsonObject> FyersBroker::modify_order(const BrokerCredentials& creds,
    ...
    if (resp.json.value("s").toString() == "ok") return {true, resp.json, "", ts};
    return {false, std::nullopt, resp.json.value("message").toString("Modify failed"), ts};
}

ApiResponse<QJsonObject> FyersBroker::cancel_order(const BrokerCredentials& creds,
                                                     const QString& order_id) {
    QJsonObject payload{{"id", order_id}};
    auto resp = BrokerHttp::instance().del(
        QString(base_url()) + "/api/v3/orders", auth_headers(creds), payload);
    int64_t ts = now_ts();
    if (!resp.success) return {false, std::nullopt, resp.error, ts};
    if (resp.json.value("s").toString() == "ok") return {true, resp.json, "", ts};
...
ApiResponse<QVector<BrokerOrderInfo>> FyersBroker::get_orders(const BrokerCredentials& creds) {
    auto resp = BrokerHttp::instance().get(
        QString(base_url()) + "/api/v3/orders", auth_headers(creds));
    int64_t ts = now_ts();
    if (!resp.success) return {false, std::nullopt, resp.error, ts};
    if (resp.json.value("s").toString() != "ok")
...
ApiResponse<QJsonObject> FyersBroker::get_trade_book(const BrokerCredentials& creds) {
    auto resp = BrokerHttp::instance().get(
        QString(base_url()) + "/api/v3/tradebook", auth_headers(creds));
    int64_t ts = now_ts();
    if (!resp.success) return {false, std::nullopt, resp.error, ts};
    if (resp.json.value("s").toString() == "ok") return {true, resp.json, "", ts};
    return {false, std::nullopt, resp.json.value("message").toString("Failed"), ts};
...
ApiResponse<QVector<BrokerPosition>> FyersBroker::get_positions(const BrokerCredentials& creds) {
    auto resp = BrokerHttp::instance().get(
        QString(base_url()) + "/api/v3/positions", auth_headers(creds));
    int64_t ts = now_ts();
    if (!resp.success) return {false, std::nullopt, resp.error, ts};
    if (resp.json.value("s").toString() != "ok")
        return {false, std::nullopt, resp.json.value("message").toString("Failed"), ts};
...
ApiResponse<QVector<BrokerHolding>> FyersBroker::get_holdings(const BrokerCredentials& creds) {
    auto resp = BrokerHttp::instance().get(
        QString(base_url()) + "/api/v3/holdings", auth_headers(creds));
    int64_t ts = now_ts();
    if (!resp.success) return {false, std::nullopt, resp.error, ts};
    if (resp.json.value("s").toString() != "ok")
        return {false, std::nullopt, resp.json.value("message").toString("Failed"), ts};
...
ApiResponse<BrokerFunds> FyersBroker::get_funds(const BrokerCredentials& creds) {
    auto resp = BrokerHttp::instance().get(
        QString(base_url()) + "/api/v3/funds", auth_headers(creds));
    int64_t ts = now_ts();
    if (!resp.success) return {false, std::nullopt, resp.error, ts};
    if (resp.json.value("s").toString() != "ok")
        return {false, std::nullopt, resp.json.value("message").toString("Failed"), ts};
...
ApiResponse<QVector<BrokerQuote>> FyersBroker::get_quotes(
        const BrokerCredentials& creds, const QVector<QString>& symbols) {
    QString syms;
    ...
    for (const auto& val : d_arr) {
        ...
        if (item.value("s").toString() != "ok") continue;
...
ApiResponse<QVector<BrokerCandle>> FyersBroker::get_history(
