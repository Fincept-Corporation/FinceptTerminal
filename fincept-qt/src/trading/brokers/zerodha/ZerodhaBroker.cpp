...
QString ZerodhaBroker::checked_error(const BrokerHttpResponse& resp, const QString& fallback) {
    ...
    QString msg = resp.json.value("message").toString(
                      resp.error.isEmpty() ? fallback : resp.error);
...
ApiResponse<QJsonObject> ZerodhaBroker::modify_order(const BrokerCredentials& creds, const QString& order_id,
    ...
    QMap<QString, QString> params;
    if (mods.contains("order_type"))        params["order_type"]         = mods.value("order_type").toString();
    if (mods.contains("quantity"))          params["quantity"]           = mods.value("quantity").toString();
    if (mods.contains("price"))             params["price"]              = mods.value("price").toString();
    if (mods.contains("trigger_price"))     params["trigger_price"]      = mods.value("trigger_price").toString();
    if (mods.contains("disclosed_quantity"))params["disclosed_quantity"] = mods.value("disclosed_quantity").toString();
    params["validity"] = mods.value("validity").toString("DAY");

    auto resp = BrokerHttp::instance().put_form(
        QString("%1/orders/%2/%3").arg(base_url(), variety, order_id), params, auth_headers(creds));
    int64_t ts = now_ts();
    if (!resp.success || resp.json.value("status").toString() != "success")
...
ApiResponse<QJsonObject> ZerodhaBroker::cancel_order(const BrokerCredentials& creds, const QString& order_id) {
    ...
    if (!creds.additional_data.isEmpty()) {
        ...
        if (!v.isEmpty()) variety = v;
    }
    auto resp = BrokerHttp::instance().del(
        QString("%1/orders/%2/%3").arg(base_url(), variety, order_id), auth_headers(creds));
    int64_t ts = now_ts();
    if (!resp.success || resp.json.value("status").toString() != "success")
...
ApiResponse<QVector<BrokerHolding>> ZerodhaBroker::get_holdings(const BrokerCredentials& creds) {
    ...
    for (const auto& v : arr) {
        ...
        hold.pnl_pct        = hold.avg_price > 0
                              ? ((hold.ltp - hold.avg_price) / hold.avg_price) * 100.0
                              : 0.0;
...
ApiResponse<BrokerFunds> ZerodhaBroker::get_funds(const BrokerCredentials& creds) {
    ...
    BrokerFunds funds;
    funds.available_balance = equity.value("net").toDouble()
                            + commod.value("net").toDouble();
...
ApiResponse<QVector<BrokerCandle>> ZerodhaBroker::get_history(const BrokerCredentials& creds,
    ...
    if (token.isEmpty()) {
        return {false, std::nullopt,
                ...
                ". Call InstrumentService::refresh() first.", now_ts()};
    }
    ...
    for (const auto& v : arr) {
        ...
        candles.append({epoch, c[1].toDouble(), c[2].toDouble(),
...
static QJsonObject order_to_margin_leg(const UnifiedOrder& o) {
    ...
    leg["product"]           = (o.product_type == ProductType::Intraday) ? "MIS"
                             : (o.product_type == ProductType::Margin)    ? "NRML" : "CNC";
    leg["order_type"]        = (o.order_type == OrderType::Market)    ? "MARKET"
                             : (o.order_type == OrderType::StopLoss)   ? "SL-M"
                             : (o.order_type == OrderType::StopLossLimit) ? "SL" : "LIMIT";
...
ApiResponse<OrderMargin> ZerodhaBroker::get_order_margins(const BrokerCredentials& creds,
                                                           const UnifiedOrder& order) {
    QJsonArray arr;
    arr.append(order_to_margin_leg(order));

    auto resp = BrokerHttp::instance().post_json(
        QString(base_url()) + "/margins/orders", QJsonObject{{"orders", arr}},
        auth_headers(creds));
    int64_t ts = now_ts();
    if (!resp.success || resp.json.value("status").toString() != "success")
...
ApiResponse<BasketMargin> ZerodhaBroker::get_basket_margins(const BrokerCredentials& creds,
    ...
    auto resp = BrokerHttp::instance().post_json(
        QString(base_url()) + "/margins/basket", payload, auth_headers(creds));
    int64_t ts = now_ts();
    if (!resp.success || resp.json.value("status").toString() != "success")
...
static GttOrder parse_gtt(const QJsonObject& o) {
    ...
    for (int i = 0; i < ords.size(); ++i) {
        ...
        t.side          = (leg.value("transaction_type").toString() == "BUY")
                              ? OrderSide::Buy : OrderSide::Sell;
        t.order_type    = (leg.value("order_type").toString() == "MARKET")
                              ? OrderType::Market : OrderType::Limit;
        t.product       = (leg.value("product").toString() == "MIS")
                              ? ProductType::Intraday : ProductType::Delivery;
        g.triggers.append(t);
    }
    return g;
}
...
GttPlaceResponse ZerodhaBroker::gtt_place(const BrokerCredentials& creds, const GttOrder& order) {
    auto resp = BrokerHttp::instance().post_json(
        QString(base_url()) + "/gtt/triggers", build_gtt_body(order), auth_headers(creds));
    if (!resp.success || resp.json.value("status").toString() != "success")
        return {false, "", checked_error(resp, "GTT place failed")};
    QString id = QString::number(
        resp.json.value("data").toObject().value("trigger_id").toInt());
    return {true, id, ""};
}

ApiResponse<GttOrder> ZerodhaBroker::gtt_get(const BrokerCredentials& creds, const QString& gtt_id) {
    auto resp = BrokerHttp::instance().get(
        QString("%1/gtt/triggers/%2").arg(base_url(), gtt_id), auth_headers(creds));
    int64_t ts = now_ts();
    if (!resp.success || resp.json.value("status").toString() != "success")
...
ApiResponse<QVector<GttOrder>> ZerodhaBroker::gtt_list(const BrokerCredentials& creds) {
    auto resp = BrokerHttp::instance().get(
        QString(base_url()) + "/gtt/triggers", auth_headers(creds));
    int64_t ts = now_ts();
    if (!resp.success || resp.json.value("status").toString() != "success")
...
ApiResponse<GttOrder> ZerodhaBroker::gtt_modify(const BrokerCredentials& creds, const QString& gtt_id,
    ...
    auto resp = BrokerHttp::instance().put_json(
        QString("%1/gtt/triggers/%2").arg(base_url(), gtt_id),
