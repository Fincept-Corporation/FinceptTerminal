...
QString ShoonyaBroker::sh_status(const QString& s) {
    ...
    if (sl == "COMPLETE")                           return "filled";
    if (sl == "OPEN" || sl == "TRIGGER PENDING")    return "open";
    if (sl == "CANCELLED")                          return "cancelled";
    if (sl == "REJECTED")                           return "rejected";
    return "open";
}
...
bool ShoonyaBroker::is_token_expired(const BrokerHttpResponse& resp) {
    ...
    if (obj.value("stat").toString() != "Not_Ok") return false;
    QString emsg = obj.value("emsg").toString().toLower();
    return emsg.contains("session") || emsg.contains("invalid") ||
           emsg.contains("token")   || emsg.contains("login")   ||
...
TokenExchangeResponse ShoonyaBroker::exchange_token(const QString& api_key,
    ...
    QString pwd = QString(QCryptographicHash::hash(
        api_secret.toUtf8(), QCryptographicHash::Sha256).toHex());

    // appkey = SHA256(uid + "|" + api_secret)
    QString appkey = QString(QCryptographicHash::hash(
        (api_key + "|" + api_secret).toUtf8(), QCryptographicHash::Sha256).toHex());
    ...
    auto resp = http.post_raw(
        QString("%1/QuickAuth").arg(BASE),
        make_login_body(jdata),
...
OrderPlaceResponse ShoonyaBroker::place_order(const BrokerCredentials& creds,
    ...
    auto& http = BrokerHttp::instance();
    auto resp = http.post_raw(
        QString("%1/PlaceOrder").arg(BASE),
        make_body(jdata, creds.access_token),
        {{"Content-Type", "application/x-www-form-urlencoded"}}
    );
...
ApiResponse<QJsonObject> ShoonyaBroker::modify_order(const BrokerCredentials& creds,
    ...
    auto& http = BrokerHttp::instance();
    auto resp = http.post_raw(
        QString("%1/ModifyOrder").arg(BASE),
        make_body(jdata, creds.access_token),
        {{"Content-Type", "application/x-www-form-urlencoded"}}
    );
...
ApiResponse<QJsonObject> ShoonyaBroker::cancel_order(const BrokerCredentials& creds,
                                                       const QString& order_id) {
    int64_t ts = now_ts();
    ...
    auto& http = BrokerHttp::instance();
    auto resp = http.post_raw(
        QString("%1/CancelOrder").arg(BASE),
        make_body(jdata, creds.access_token),
        {{"Content-Type", "application/x-www-form-urlencoded"}}
    );
...
ApiResponse<QVector<BrokerPosition>> ShoonyaBroker::get_positions(const BrokerCredentials& creds) {
    ...
    auto& http = BrokerHttp::instance();
    auto resp = http.post_raw(
        QString("%1/PositionBook").arg(BASE),
        make_body(jdata, creds.access_token),
        {{"Content-Type", "application/x-www-form-urlencoded"}}
    );
    ...
    for (const QJsonValue& v : doc.array()) {
        ...
        pos.pnl          = o.value("urmtom").toString().toDouble()
                         + o.value("rpnl").toString().toDouble();
...
ApiResponse<QVector<BrokerHolding>> ShoonyaBroker::get_holdings(const BrokerCredentials& creds) {
    ...
    for (const QJsonValue& v : doc.array()) {
        ...
        int total_qty = btstqty + holdqty + brkcolqty + unplgdqty + benqty
                      + qMax(npoadqty, dpqty) - usedqty;
...
ApiResponse<QVector<BrokerQuote>> ShoonyaBroker::get_quotes(const BrokerCredentials& creds,
    ...
    for (const QString& sym : symbols) {
        ...
        if (token.isEmpty()) continue; // Shoonya GetQuotes requires numeric token
        ...
        if (o.value("stat").toString() != "Ok") continue;
...
ApiResponse<QVector<BrokerCandle>> ShoonyaBroker::get_history(const BrokerCredentials& creds,
    ...
    if (is_daily) {
        ...
        auto resp = http.post_raw(
            QString("%1/EODChartData").arg(BASE),
            make_body(jdata, creds.access_token),
            {{"Content-Type", "application/x-www-form-urlencoded"}}
        );
        if (!resp.success)
        ...
        if (token.isEmpty())
            return {false, std::nullopt, "get_history: numeric token required for intraday (format NSE:SYMBOL:TOKEN)", ts};

        // Map resolution to minutes
        static const QMap<QString, QString> interval_map = {
            {"1","1"},  {"3","3"},  {"5","5"},  {"10","10"},
            {"15","15"},{"30","30"},{"60","60"},{"120","120"},{"240","240"},
        };
