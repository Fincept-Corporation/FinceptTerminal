...
TokenExchangeResponse AlpacaBroker::exchange_token(const QString& api_key, const QString& api_secret, const QString&) {
    ...
    result.success = true;
    result.access_token = api_secret;
    result.user_id = resp.json.value("id").toString();
    return result;
}
...
ApiResponse<QJsonObject> AlpacaBroker::modify_order(const BrokerCredentials& creds, const QString& order_id,
    ...
    auto resp =
        BrokerHttp::instance().put_json(QString(base_url()) + "/v2/orders/" + order_id, mods, auth_headers(creds));
    int64_t ts = now_ts();
...
ApiResponse<QVector<BrokerOrderInfo>> AlpacaBroker::get_orders(const BrokerCredentials& creds) {
    auto resp =
        BrokerHttp::instance().get(QString(base_url()) + "/v2/orders?status=all&limit=100", auth_headers(creds));
    int64_t ts = now_ts();
    if (!resp.success)
    ...
    if (doc.isArray()) {
        for (const auto& v : doc.array()) {
            ...
            info.quantity = o.value("qty").toString().toDouble();
            info.price = o.value("limit_price").toString().toDouble();
            info.filled_qty = o.value("filled_qty").toString().toDouble();
            info.avg_price = o.value("filled_avg_price").toString().toDouble();
...
ApiResponse<QVector<BrokerPosition>> AlpacaBroker::get_positions(const BrokerCredentials& creds) {
    ...
    if (doc.isArray()) {
        for (const auto& v : doc.array()) {
            ...
            pos.quantity = p.value("qty").toString().toDouble();
...
ApiResponse<QVector<BrokerHolding>> AlpacaBroker::get_holdings(const BrokerCredentials& creds) {
    ...
    if (!pos_result.success)
        return {false, std::nullopt, pos_result.error, pos_result.timestamp};
    QVector<BrokerHolding> holdings;
    for (const auto& p : *pos_result.data) {
        BrokerHolding h;
        h.symbol = p.symbol;
        h.quantity = std::abs(p.quantity);
...
ApiResponse<QVector<BrokerQuote>> AlpacaBroker::get_quotes(const BrokerCredentials& creds,
    ...
    Q_UNUSED(creds);
    Q_UNUSED(symbols);
    return {false, std::nullopt, "Use data API for quotes", now_ts()};
}
