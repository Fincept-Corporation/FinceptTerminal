...
ApiResponse<QJsonObject> AlpacaBroker::modify_order(const BrokerCredentials& creds, const QString& order_id,
    ...
    auto resp = BrokerHttp::instance().put_json(trading_url(creds) + "/v2/orders/" + order_id, mods,
                                                auth_headers(creds));
    int64_t ts = now_ts();
...
ApiResponse<QVector<BrokerOrderInfo>> AlpacaBroker::get_orders(const BrokerCredentials& creds) {
    auto resp = BrokerHttp::instance().get(trading_url(creds) + "/v2/orders?status=all&limit=100",
                                           auth_headers(creds));
    int64_t ts = now_ts();
    if (!resp.success)
    ...
    auto jdo = [](const QJsonValue& v) -> double {
...
ApiResponse<QJsonObject> AlpacaBroker::get_trade_book(const BrokerCredentials& creds) {
    auto resp = BrokerHttp::instance().get(trading_url(creds) + "/v2/account/activities/FILL",
                                           auth_headers(creds));
    int64_t ts = now_ts();
    return resp.success ? ApiResponse<QJsonObject>{true, resp.json, "", ts}
                        : ApiResponse<QJsonObject>{false, std::nullopt, resp.error, ts};
...
ApiResponse<QVector<BrokerPosition>> AlpacaBroker::get_positions(const BrokerCredentials& creds) {
    ...
    QJsonParseError err;
    auto doc = QJsonDocument::fromJson(resp.raw_body.toUtf8(), &err);
    auto jd = [](const QJsonValue& v) -> double {
        return v.isDouble() ? v.toDouble() : v.toString().toDouble();
    };
    if (doc.isArray()) {
        for (const auto& v : doc.array()) {
...
ApiResponse<QVector<BrokerHolding>> AlpacaBroker::get_holdings(const BrokerCredentials& creds) {
    ...
    QJsonParseError err;
    auto doc = QJsonDocument::fromJson(resp.raw_body.toUtf8(), &err);
    auto jd2 = [](const QJsonValue& v) -> double {
        return v.isDouble() ? v.toDouble() : v.toString().toDouble();
    };
    if (doc.isArray()) {
        for (const auto& v : doc.array()) {
...
ApiResponse<BrokerFunds> AlpacaBroker::get_funds(const BrokerCredentials& creds) {
    ...
    funds.raw_data          = j;
    LOG_INFO("AlpacaBroker", QString("get_funds: equity=%1 buying_power=%2")
                 .arg(funds.total_balance).arg(funds.available_balance));
...
ApiResponse<QVector<BrokerQuote>> AlpacaBroker::get_quotes(const BrokerCredentials& creds,
    ...
    QMap<QString, QString> data_headers = {{"APCA-API-KEY-ID", creds.api_key},
                                           {"APCA-API-SECRET-KEY", creds.api_secret},
                                           {"Accept", "application/json"}};
...
ApiResponse<QVector<BrokerCandle>> AlpacaBroker::get_history(const BrokerCredentials& creds, const QString& symbol,
    ...
    static const QMap<QString, QString> tf_map = {
        ...
        {"30", "30Min"}, {"30m","30Min"},
        {"60", "1Hour"}, {"1h", "1Hour"}, {"1hour", "1Hour"},
        {"D",  "1Day"},  {"1D", "1Day"},  {"day",   "1Day"},  {"1day", "1Day"},
    ...
    QString url = data_url() + "/v2/stocks/" + symbol.toUpper() + "/bars"
                  + "?timeframe=" + alpaca_tf
                  + "&start=" + start
                  + "&end=" + end
                  + "&limit=500"
    ...
    for (const auto& v : bars_arr) {
        ...
        c.timestamp = QDateTime::fromString(b.value("t").toString(), Qt::ISODateWithMs)
                          .toSecsSinceEpoch();
...
ApiResponse<QVector<MarketCalendarDay>> AlpacaBroker::get_calendar(const BrokerCredentials& creds,
                                                                    const QString& start,
                                                                    const QString& end) {
    QMap<QString, QString> headers = {{"APCA-API-KEY-ID", creds.api_key},
                                      {"APCA-API-SECRET-KEY", creds.api_secret},
                                      {"Accept", "application/json"}};
...
ApiResponse<QVector<BrokerCandle>> AlpacaBroker::get_latest_bars(const BrokerCredentials& creds,
                                                                  const QVector<QString>& symbols) {
    if (symbols.isEmpty()) return {false, std::nullopt, "No symbols", now_ts()};
    ...
    QJsonParseError err;
    auto doc = QJsonDocument::fromJson(resp.raw_body.toUtf8(), &err);
    if (err.error != QJsonParseError::NoError) return {false, std::nullopt, err.errorString(), ts};

    QVector<BrokerCandle> result;
    auto bars_obj = doc.object().value("bars").toObject();
    for (const auto& sym : symbols) {
        auto b = bars_obj.value(sym).toObject();
        if (!b.isEmpty()) result.append(parse_bar(b, sym));
    }
...
ApiResponse<QVector<BrokerCandle>> AlpacaBroker::get_historical_bars(const BrokerCredentials& creds,
    ...
    QString url = data_url() + "/v2/stocks/bars?symbols=" + sym_param
                + "&timeframe=" + (timeframe.isEmpty() ? "1Day" : timeframe)
                + "&feed=iex&limit=1000&sort=asc";
    if (!start.isEmpty()) url += "&start=" + start;
    if (!end.isEmpty())   url += "&end="   + end;
...
ApiResponse<QVector<BrokerQuote>> AlpacaBroker::get_latest_quotes(const BrokerCredentials& creds,
    ...
    for (const auto& sym : symbols) {
        ...
        if (!q.isEmpty()) result.append(parse_quote_ba(q, sym));
    }
    return {true, result, "", ts};
}
...
ApiResponse<QVector<BrokerTrade>> AlpacaBroker::get_latest_trades(const BrokerCredentials& creds,
    ...
    for (const auto& sym : symbols) {
        ...
        if (!t.isEmpty()) result.append(parse_trade(t, sym));
    }
    return {true, result, "", ts};
}
...
ApiResponse<QVector<BrokerTrade>> AlpacaBroker::get_historical_trades(const BrokerCredentials& creds,
    ...
    if (symbols.isEmpty()) return {false, std::nullopt, "No symbols", now_ts()};
    const QString sym_param = QStringList(symbols.begin(), symbols.end()).join(",");
    QString url = data_url() + "/v2/stocks/trades?symbols=" + sym_param
                + "&feed=iex&limit=" + QString::number(limit) + "&sort=asc";
    if (!start.isEmpty()) url += "&start=" + start;
    if (!end.isEmpty())   url += "&end="   + end;
    ...
    for (const auto& sym : symbols) {
        for (const auto& day_v : auctions_obj.value(sym).toArray()) {
            ...
            for (const auto& e_v : day.value("openingAuction").toArray()) {
                ...
                for (const auto& c : e.value("c").toArray()) entry.conditions.append(c.toString());
...
// ─── Condition codes ──────────────────────────────────────────────────────────
// ticktype: "trades" | "quotes" | "bars"   tape: "A" | "B" | "C"
ApiResponse<QVector<BrokerMetaEntry>> AlpacaBroker::get_condition_codes(const BrokerCredentials& creds,
    ...
    // Response is a flat object: { "code": "description", ... }
    auto obj = doc.object();
...
ApiResponse<QVector<BrokerMetaEntry>> AlpacaBroker::get_exchange_codes(const BrokerCredentials& creds,
    ...
    if (!asset_class.isEmpty()) url += "?asset_class=" + asset_class;
    auto resp = BrokerHttp::instance().get(url, alpaca_data_headers(creds));
    int64_t ts = now_ts();
    if (!resp.success) return {false, std::nullopt, resp.error, ts};
...
ApiResponse<BrokerCandle> AlpacaBroker::get_latest_bar(const BrokerCredentials& creds,
    ...
    if (bar.isEmpty()) return {false, std::nullopt, "No bar data", ts};
    return {true, parse_bar(bar, symbol), "", ts};
}

// ─── Single-symbol: Latest quote ─────────────────────────────────────────────
ApiResponse<BrokerQuote> AlpacaBroker::get_latest_quote(const BrokerCredentials& creds,
                                                         const QString& symbol) {
    ...
    if (q.isEmpty()) return {false, std::nullopt, "No quote data", ts};
    return {true, parse_quote_ba(q, symbol), "", ts};
}

// ─── Single-symbol: Latest trade ─────────────────────────────────────────────
ApiResponse<BrokerTrade> AlpacaBroker::get_latest_trade(const BrokerCredentials& creds,
                                                         const QString& symbol) {
    ...
    if (t.isEmpty()) return {false, std::nullopt, "No trade data", ts};
    return {true, parse_trade(t, symbol), "", ts};
}

// ─── Single-symbol: Snapshot ─────────────────────────────────────────────────
ApiResponse<BrokerQuote> AlpacaBroker::get_snapshot(const BrokerCredentials& creds,
                                                     const QString& symbol) {
    ...
    q.close  = daily.value("c").toDouble();
    q.volume = daily.value("v").toDouble();
    if (q.ltp <= 0 && q.bid > 0 && q.ask > 0) q.ltp = (q.bid + q.ask) / 2.0;
    double prev_close = prev.value("c").toDouble();
    if (prev_close > 0 && q.ltp > 0) {
        q.change     = q.ltp - prev_close;
        q.change_pct = (q.change / prev_close) * 100.0;
...
ApiResponse<QVector<BrokerTrade>> AlpacaBroker::get_historical_trades_single(const BrokerCredentials& creds,
                                                                              ...
                                                                              const QString& end,
                                                                              int limit) {
    QString url = data_url() + "/v2/stocks/" + symbol.toUpper()
                + "/trades?feed=iex&limit=" + QString::number(limit) + "&sort=asc";
    if (!start.isEmpty()) url += "&start=" + start;
    if (!end.isEmpty())   url += "&end="   + end;
...
ApiResponse<QVector<BrokerQuote>> AlpacaBroker::get_historical_quotes_single(const BrokerCredentials& creds,
                                                                              ...
                                                                              int limit) {
    QString url = data_url() + "/v2/stocks/" + symbol.toUpper()
                + "/quotes?feed=iex&limit=" + QString::number(limit) + "&sort=asc";
    if (!start.isEmpty()) url += "&start=" + start;
    if (!end.isEmpty())   url += "&end="   + end;
