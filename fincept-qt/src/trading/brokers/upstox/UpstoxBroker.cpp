...
static QString instrument_key(const QString& symbol, const QString& exchange,
...
static const char* upstox_product(ProductType p, const QString& exchange) {
    ...
    switch (p) {
        ...
        case ProductType::Margin:   return "D";  // MTF maps to Delivery segment
        default:                    return "I";
    }
    (void)exchange;
}
...
struct UpstoxInterval { QString unit; QString interval; int max_days; };
static UpstoxInterval upstox_interval(const QString& resolution) {
    static const QMap<QString, UpstoxInterval> map = {
        {"1",   {"minutes", "1",  30}},  {"1m",  {"minutes", "1",  30}},
        {"2",   {"minutes", "2",  30}},  {"2m",  {"minutes", "2",  30}},
        {"3",   {"minutes", "3",  30}},  {"3m",  {"minutes", "3",  30}},
        {"5",   {"minutes", "5",  30}},  {"5m",  {"minutes", "5",  30}},
        {"10",  {"minutes", "10", 90}},  {"10m", {"minutes", "10", 90}},
        {"15",  {"minutes", "15", 90}},  {"15m", {"minutes", "15", 90}},
        {"30",  {"minutes", "30", 90}},  {"30m", {"minutes", "30", 90}},
        {"60",  {"minutes", "60", 90}},  {"60m", {"minutes", "60", 90}},
        {"1h",  {"minutes", "60", 90}},
        {"2h",  {"hours",   "2",  90}},
        {"4h",  {"hours",   "4",  90}},
        {"D",   {"days",    "1",  3650}}, {"1D", {"days",   "1",  3650}},
        {"W",   {"weeks",   "1",  7300}},
        {"M",   {"months",  "1",  7300}},
    };
...
TokenExchangeResponse UpstoxBroker::exchange_token(const QString& api_key,
    ...
    if (resp.json.value("status").toString() == "success" ||
        resp.json.contains("access_token")) {
...
// ── Modify Order ──────────────────────────────────────────────────────────────
// PUT /v2/order/modify
ApiResponse<QJsonObject> UpstoxBroker::modify_order(const BrokerCredentials& creds,
...
ApiResponse<QJsonObject> UpstoxBroker::cancel_order(const BrokerCredentials& creds,
                                                      const QString& order_id) {
    auto& http = BrokerHttp::instance();
    auto resp  = http.del("https://api.upstox.com/v2/order/cancel?order_id=" + order_id,
                           auth_headers(creds));
...
ApiResponse<QJsonObject> UpstoxBroker::get_trade_book(const BrokerCredentials& creds) {
    auto& http = BrokerHttp::instance();
    auto resp  = http.get("https://api.upstox.com/v2/order/trades/get-trades-for-day",
                           auth_headers(creds));
...
ApiResponse<QVector<BrokerPosition>> UpstoxBroker::get_positions(const BrokerCredentials& creds) {
    auto& http = BrokerHttp::instance();
    auto resp  = http.get("https://api.upstox.com/v2/portfolio/short-term-positions",
                           auth_headers(creds));
    ...
    for (const auto& v : arr) {
        ...
        if (qty == 0.0) continue;  // skip flat positions
...
ApiResponse<QVector<BrokerHolding>> UpstoxBroker::get_holdings(const BrokerCredentials& creds) {
    auto& http = BrokerHttp::instance();
    auto resp  = http.get("https://api.upstox.com/v2/portfolio/long-term-holdings",
                           auth_headers(creds));
...
ApiResponse<BrokerFunds> UpstoxBroker::get_funds(const BrokerCredentials& creds) {
    auto& http = BrokerHttp::instance();
    auto resp  = http.get("https://api.upstox.com/v2/user/get-funds-and-margin",
                           auth_headers(creds));
    ...
    BrokerFunds funds;
    funds.available_balance = equity.value("available_margin").toDouble()
                            + commod.value("available_margin").toDouble();
    funds.used_margin       = equity.value("used_margin").toDouble()
                            + commod.value("used_margin").toDouble();
...
ApiResponse<QVector<BrokerQuote>> UpstoxBroker::get_quotes(const BrokerCredentials& creds,
    ...
    const QString url = "https://api.upstox.com/v3/market-quote/ohlc?instrument_key="
                      + keys.join(",") + "&interval=1d";
...
ApiResponse<QVector<BrokerCandle>> UpstoxBroker::get_history(const BrokerCredentials& creds,
    ...
    while (chunk_end >= from) {
        ...
        if (chunk_start < from) chunk_start = from;
