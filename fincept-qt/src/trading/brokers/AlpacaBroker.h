...
class AlpacaBroker : public IBroker {
  public:
    ...
    // Trading base URL derived from credentials (additional_data == "live" → live URL)
    ...
    ApiResponse<QVector<MarketCalendarDay>> get_calendar(const BrokerCredentials& creds,
    ...
    ApiResponse<QVector<BrokerAuction>>     get_historical_auctions(const BrokerCredentials& creds,
                                                                    const QVector<QString>& symbols,
                                                                    const QString& start, const QString& end) override;
    ApiResponse<QVector<BrokerMetaEntry>>   get_condition_codes(const BrokerCredentials& creds,
    ...
    // Single-symbol stock data
    ApiResponse<BrokerCandle>               get_latest_bar(const BrokerCredentials& creds,
                                                           const QString& symbol) override;
    ApiResponse<BrokerQuote>                get_latest_quote(const BrokerCredentials& creds,
                                                             const QString& symbol) override;
    ApiResponse<BrokerTrade>                get_latest_trade(const BrokerCredentials& creds,
                                                             const QString& symbol) override;
    ApiResponse<BrokerQuote>                get_snapshot(const BrokerCredentials& creds,
                                                         const QString& symbol) override;
