...
QString DhanBroker::dhan_order_type(OrderType t) {
    switch (t) {
        ...
        case OrderType::StopLoss:      return "STOP_LOSS";         // SL (trigger + limit price)
        case OrderType::StopLossLimit: return "STOP_LOSS_MARKET";  // SL-M (trigger only)
    }
    return "MARKET";
}
...
QString DhanBroker::lookup_security_id(const QString& symbol, const QString& exchange,
    ...
    LOG_WARN(TAG, QString("Security ID not found for %1:%2 — instrument DB not loaded?")
             .arg(exchange, symbol));
    return {};
}
...
// auth_code  = unused
TokenExchangeResponse DhanBroker::exchange_token(const QString& api_key,
...
ApiResponse<QJsonObject> DhanBroker::modify_order(const BrokerCredentials& creds,
    ...
    auto resp = BrokerHttp::instance().put_json(BASE + "/v2/orders/" + order_id, body,
                                                 auth_headers(creds));
    if (!resp.success || resp.json.value("errorType").toString().length() > 0)
...
ApiResponse<QJsonObject> DhanBroker::cancel_order(const BrokerCredentials& creds,
                                                    const QString& order_id) {
    int64_t ts = now_ts();
...
ApiResponse<QVector<BrokerQuote>> DhanBroker::get_quotes(const BrokerCredentials& creds,
    ...
    auto resp = BrokerHttp::instance().post_json(BASE + "/v2/marketfeed/ltp", body,
                                                  auth_headers(creds));
    if (!resp.success || resp.json.value("errorType").toString().length() > 0)
...
ApiResponse<QVector<BrokerCandle>> DhanBroker::get_history(const BrokerCredentials& creds,
