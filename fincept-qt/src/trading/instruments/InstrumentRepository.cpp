...
fincept::Result<void> InstrumentRepository::replace_all(const QString& broker_id,
    ...
    if (rc.is_err()) { db().rollback(); return rc; }
    ...
    for (const auto& inst : instruments) {
        ...
        if (ri.is_err()) { db().rollback(); return ri; }
    }

    auto rc2 = db().commit();
    if (rc2.is_err()) { db().rollback(); return rc2; }

    LOG_INFO("InstrumentRepo", QString("Replaced %1 instruments for %2")
                                   .arg(instruments.size()).arg(broker_id));
...
std::optional<Instrument> InstrumentRepository::find(const QString& symbol,
    ...
    return query_optional(
        ...
        {symbol, exchange, broker_id},
        map_row);
}

std::optional<Instrument> InstrumentRepository::find_by_token(qint64 instrument_token,
                                                               const QString& broker_id) const {
    return query_optional(
        "SELECT instrument_token, exchange_token, symbol, brsymbol, name, exchange, brexchange, "
        "expiry, strike, lot_size, instrument_type, tick_size, broker_id "
        "FROM instruments WHERE instrument_token = ? AND broker_id = ? LIMIT 1",
        {instrument_token, broker_id},
        map_row);
}

std::optional<Instrument> InstrumentRepository::find_by_brsymbol(const QString& brsymbol,
    ...
    return query_optional(
        ...
        {brsymbol, brexchange, broker_id},
        map_row);
}

QVector<Instrument> InstrumentRepository::search(const QString& query,
                                                  const QString& exchange,
    ...
    auto& q = r.value();
    while (q.next()) results.append(map_row(q));
    return results;
}

QVector<Instrument> InstrumentRepository::list(const QString& exchange,
                                                const QString& broker_id,
