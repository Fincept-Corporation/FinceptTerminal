...
QString ZerodhaInstrumentParser::normalise_exchange(const QString& exchange,
...
QString ZerodhaInstrumentParser::normalise_symbol(const QString& tradingsymbol,
                                                   const QString& name,
                                                   const QString& instrument_type,
                                                   const QString& expiry_nd,
                                                   double strike) {
...
QVector<Instrument> ZerodhaInstrumentParser::parse(const QByteArray& csv_data) {
    ...
    while (!stream.atEnd()) {
        ...
        if (inst.instrument_token == 0) { ++row; continue; }
