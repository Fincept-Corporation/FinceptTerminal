...
public:
    ...
    // --- Authentication ---
    virtual TokenExchangeResponse exchange_token(const QString& api_key,
    ...
    virtual ApiResponse<QJsonObject> modify_order(const BrokerCredentials& creds,
                                                   const QString& order_id,
                                                   const QJsonObject& modifications) = 0;
    virtual ApiResponse<QJsonObject> cancel_order(const BrokerCredentials& creds,
                                                   const QString& order_id) = 0;
    ...
    virtual ApiResponse<QVector<BrokerCandle>> get_history(const BrokerCredentials& creds,
