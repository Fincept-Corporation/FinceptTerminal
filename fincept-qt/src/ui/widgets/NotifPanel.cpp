...
void NotifPanel::refresh() {
    ...
    if (hist.isEmpty()) {
        ...
        empty->setStyleSheet(
            QString("color: %1; font-size: 12px; background: transparent; padding: 24px;")
    ...
    for (const auto& rec : hist) {
        ...
        dot->setStyleSheet(
            QString("color: %1; font-size: 9px; background: transparent;").arg(dot_color));
        ...
        title_lbl->setStyleSheet(
            QString("color: %1; font-size: 12px; font-weight: %2; background: transparent;")
...
void NotifPanel::anchor_to(QWidget* ref_widget) {
    if (!ref_widget || !ref_widget->parentWidget()) return;
