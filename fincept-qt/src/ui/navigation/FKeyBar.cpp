...
TabBar::TabBar(QWidget* parent) : QWidget(parent) {
    ...
    tab_layout_->setContentsMargins(4,0,4,0); tab_layout_->setSpacing(0);
    ...
    for (const auto& def : tabs) add_tab(def);
    tab_layout_->addStretch();
    scroll_area->setWidget(container);
    auto* root = new QHBoxLayout(this);
    root->setContentsMargins(0,0,0,0); root->addWidget(scroll_area);
    update_styles();
}

void TabBar::add_tab(const TabDef& def) {
    auto* btn = new QPushButton(def.label);
    btn->setFixedHeight(32); btn->setCursor(Qt::PointingHandCursor);
    btn->setProperty("tab_id", def.id);
    connect(btn, &QPushButton::clicked, this, [this, id=def.id]() {
        set_active(id); emit tab_changed(id);
    });
    tab_layout_->addWidget(btn); tab_buttons_.append(btn);
}

void TabBar::set_active(const QString& tab_id) {
    if (active_id_ == tab_id) return;
    active_id_ = tab_id; update_styles();
}
