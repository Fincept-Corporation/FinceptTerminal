...
ToolBar::ToolBar(QWidget* parent) : QWidget(parent) {
    ...
    hl->setContentsMargins(4,0,10,0); hl->setSpacing(0);
    ...
    auto mk = [](const QString& t, const QString& c, int sz=13, bool b=false) -> QLabel* {
        ...
        l->setStyleSheet(QString("color:%1;font-size:%2px;%3background:transparent;%4")
            .arg(c).arg(sz).arg(b?"font-weight:700;":"").arg(MF));
        return l;
    };
    ...
    plan_btn_->setStyleSheet(
        "QPushButton{color:#525252;font-size:12px;background:transparent;border:none;"
    ...
    logout_btn->setFixedHeight(20); logout_btn->setCursor(Qt::PointingHandCursor);
    logout_btn->setStyleSheet(
        "QPushButton{background:transparent;color:#dc2626;border:1px solid #7f1d1d;"
...
void ToolBar::refresh_user_display() {
    ...
    credits_label_->setStyleSheet(QString("color:%1;font-size:12px;background:transparent;%2")
        .arg(s.user_info.credit_balance>0?"#16a34a":"#dc2626").arg(MF));

    QString plan_text = s.user_info.account_type.toUpper();
    if (!plan_text.isEmpty()) plan_text += " Plan";
    plan_btn_->setText(plan_text.isEmpty() ? "FREE" : plan_text);
}

QMenu* ToolBar::build_file_menu() {
    auto* m = new QMenu("File", this); m->setStyleSheet(POPUP_SS);
...
QMenu* ToolBar::build_navigate_menu() {
    ...
    nav(mkt,"Stock Screener","screener"); nav(mkt,"Economics","economics");
    nav(mkt,"GOVT Data","gov_data"); nav(mkt,"DBnomics","dbnomics");
    nav(mkt,"AKShare Data","akshare"); nav(mkt,"Asia Markets","asia_markets");
    nav(mkt,"Relationship Map","relationship_map");

    // Trading & Portfolio
    auto* trd = add_sub("Trading & Portfolio");
    nav(trd,"Equity Trading","equity_trading"); nav(trd,"Alpha Arena","alpha_arena");
    nav(trd,"Polymarket","polymarket"); nav(trd,"Derivatives","derivatives");
    nav(trd,"Watchlist","watchlist");

    // Research & Intelligence
    auto* res = add_sub("Research & Intelligence");
    nav(res,"Equity Research","equity_research"); nav(res,"M&A Analytics","ma_analytics");
    nav(res,"Alt. Investments","alt_investments"); nav(res,"Geopolitics","geopolitics");
    nav(res,"Maritime","maritime"); nav(res,"Market Sim","market_sim");
    nav(res,"Surface Analytics","surface_analytics");

    // Tools
    auto* tools = add_sub("Tools");
    nav(tools,"Agent Config","agent_config"); nav(tools,"MCP Servers","mcp_servers");
    nav(tools,"Data Mapping","data_mapping"); nav(tools,"Data Sources","data_sources");
    nav(tools,"Report Builder","report_builder"); nav(tools,"Excel","excel");
    nav(tools,"Trade Viz","trade_viz"); nav(tools,"Notes","notes");

    m->addSeparator();

    // Direct items (no submenu)
    nav(m,"Forum","forum"); nav(m,"Docs","docs");
    nav(m,"Support","support"); nav(m,"About","about");

    return m;
}

QMenu* ToolBar::build_view_menu() {
    auto* m = new QMenu("View", this); m->setStyleSheet(POPUP_SS);
    m->addAction("Fullscreen (F11)", this, [this](){emit action_triggered("fullscreen");});
    m->addSeparator();
    m->addAction("Zoom In (Ctrl++)", this, [this](){emit action_triggered("zoom_in");});
    m->addAction("Zoom Out (Ctrl+-)", this, [this](){emit action_triggered("zoom_out");});
    m->addAction("Reset Zoom", this, [this](){emit action_triggered("zoom_reset");});
    return m;
}

QMenu* ToolBar::build_help_menu() {
    auto* m = new QMenu("Help", this); m->setStyleSheet(POPUP_SS);
