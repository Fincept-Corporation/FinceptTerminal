...
void ScriptCatalog::seed_builtins() {
    ...
    register_entry({QStringLiteral("market.yfinance"), QStringLiteral("yfinance_data.py"), VenvId::Numpy2});
    register_entry(
        {QStringLiteral("agent.finagent_core"), QStringLiteral("agents/finagent_core/main.py"), VenvId::Numpy2});
    register_entry(
        {QStringLiteral("market.asia_china"), QStringLiteral("china_market_data_service.py"), VenvId::Numpy2});
    // Numpy1-only — vectorbt, gluonts, financepy backtests live here.
    register_entry({QStringLiteral("backtest.vectorbt"),
                    QStringLiteral("Analytics/backtesting/vectorbt/vectorbt_provider.py"), VenvId::Numpy1});
}
