...
static QJsonObject connector_to_json(const ConnectorConfig& c) {
    ...
    for (const auto& f : c.fields) {
        QString type_str;
        switch (f.type) {
            case FieldType::Text:      type_str = "text";     break;
            case FieldType::Password:  type_str = "password"; break;
            case FieldType::Number:    type_str = "number";   break;
            case FieldType::Url:       type_str = "url";      break;
            case FieldType::Select:    type_str = "select";   break;
            case FieldType::Textarea:  type_str = "textarea"; break;
            case FieldType::Checkbox:  type_str = "checkbox"; break;
            case FieldType::File:      type_str = "file";     break;
        }
        QJsonObject fo{
            {"name",          f.name},
            {"label",         f.label},
...
std::vector<ToolDef> get_data_sources_tools() {
    ...
    // ── ds_add_connection ────────────────────────────────────────────────────
    {
        ...
        t.input_schema.properties = QJsonObject{
            ...
            {"provider",     QJsonObject{{"type", "string"}, {"description", "Connector ID (e.g. 'postgresql', 'yahoo_finance')"}}},
            ...
            {"enabled",      QJsonObject{{"type", "boolean"}, {"description", "Whether to enable immediately (default: true)"}}},
        };
        ...
        t.handler = [](const QJsonObject& args) -> ToolResult {
            ...
            if (name.isEmpty())     return ToolResult::fail("Missing 'display_name'");
            if (provider.isEmpty()) return ToolResult::fail("Missing 'provider'");

            // Validate provider exists
            const auto* cfg = ConnectorRegistry::instance().get(provider);
            if (!cfg)
                return ToolResult::fail("Unknown provider '" + provider + "'. Use ds_list_connectors to see valid IDs.");
            ...
            QString config_json = QString::fromUtf8(
    ...
    // ── ds_update_connection ─────────────────────────────────────────────────
    {
        ...
        t.handler = [](const QJsonObject& args) -> ToolResult {
            ...
            if (args.contains("config") && args["config"].isObject()) {
                ds.config = QString::fromUtf8(
                    QJsonDocument(args["config"].toObject()).toJson(QJsonDocument::Compact));
            }
            ...
            return ToolResult::ok("Connection updated: " + ds.display_name,
                                  QJsonObject{{"id", id}});
        };
        tools.push_back(std::move(t));
    }
    ...
    // ── ds_set_enabled ───────────────────────────────────────────────────────
    {
        ...
        t.handler = [](const QJsonObject& args) -> ToolResult {
            QString id = args["id"].toString().trimmed();
            if (id.isEmpty()) return ToolResult::fail("Missing 'id'");
            if (!args.contains("enabled")) return ToolResult::fail("Missing 'enabled'");
    ...
    // ── ds_get_connector ─────────────────────────────────────────────────────
    {
        ...
        t.category    = "data-sources";
        t.input_schema.properties = QJsonObject{
            {"id", QJsonObject{{"type", "string"}, {"description", "Connector ID (e.g. 'postgresql', 'yahoo_finance')"}}},
        };
        t.input_schema.required = {"id"};
        t.handler = [](const QJsonObject& args) -> ToolResult {
            QString id = args["id"].toString().trimmed();
    ...
    // ── ds_search_connectors ─────────────────────────────────────────────────
    {
        ...
        t.category    = "data-sources";
        t.input_schema.properties = QJsonObject{
            {"query", QJsonObject{{"type", "string"}, {"description", "Search keyword (e.g. 'postgres', 'kafka', 'market')"}}},
        };
        t.input_schema.required = {"query"};
        t.handler = [](const QJsonObject& args) -> ToolResult {
            QString q = args["query"].toString().trimmed().toLower();
            if (q.isEmpty()) return ToolResult::fail("Missing 'query'");
            const auto& all = ConnectorRegistry::instance().all();
            QJsonArray arr;
            for (const auto& c : all) {
                bool match = c.id.toLower().contains(q)
                          || c.name.toLower().contains(q)
                          || c.description.toLower().contains(q)
                          || category_str(c.category).contains(q)
    ...
    // ── ds_connectors_by_category ────────────────────────────────────────────
    {
        ...
        t.input_schema.properties = QJsonObject{
            {"category", QJsonObject{{"type", "string"},
                {"description", "Category string: database | api | file | streaming | cloud | timeseries | market-data | search | warehouse"}}},
        };
        t.input_schema.required = {"category"};
        t.handler = [](const QJsonObject& args) -> ToolResult {
            QString cat = args["category"].toString().trimmed().toLower();
            if (cat.isEmpty()) return ToolResult::fail("Missing 'category'");

            // Map string -> enum
            Category cat_enum;
            if      (cat == "database")    cat_enum = Category::Database;
            else if (cat == "api")         cat_enum = Category::Api;
            else if (cat == "file")        cat_enum = Category::File;
            else if (cat == "streaming")   cat_enum = Category::Streaming;
        ...
        t.handler = [](const QJsonObject& args) -> ToolResult {
            ...
            if (ok) {
                return ToolResult::ok(
                    ...
                    QJsonObject{{"host", host}, {"port", port}, {"reachable", true},
                                {"note", "TCP reachability confirmed. API key validity is not verified."}});
            } else {
                return ToolResult::fail(
                    QString("Cannot reach %1:%2 — %3").arg(host).arg(port).arg(sock.errorString()));
            }
        };
        tools.push_back(std::move(t));
    }
    ...
    // ── ds_stats ─────────────────────────────────────────────────────────────
    {
        ...
        t.handler = [](const QJsonObject&) -> ToolResult {
            ...
            for (const auto& c : all_connectors)
                if (c.requires_auth) ++auth_required;
