...
std::vector<ToolDef> get_file_manager_tools() {
    ...
    // ── delete_file ─────────────────────────────────────────────────────
    {
        ...
        t.category    = "file_manager";
        t.input_schema.properties = QJsonObject{
            {"id", QJsonObject{{"type", "string"}, {"description", "File ID to delete"}}}
        };
        t.input_schema.required = {"id"};
        t.handler = [](const QJsonObject& args) -> ToolResult {
            QString id = args["id"].toString().trimmed();
    ...
    // ── read_file_contents ──────────────────────────────────────────────
    {
        ...
        t.description =
            "Read the text contents of a managed file by its ID. Supports plain text, CSV, JSON, "
        ...
        t.input_schema.properties = QJsonObject{
            {"id",        QJsonObject{{"type", "string"},
                          {"description", "File ID returned by list_files"}}},
        ...
        t.handler = [](const QJsonObject& args) -> ToolResult {
            ...
            if (mime.contains("image") || mime.contains("video") || mime.contains("audio") ||
                ...
                return ToolResult::fail(
                    "File type '" + mime + "' is binary and cannot be read as text.");
            }
            ...
            if (max_chars <= 0 || max_chars > 128000) max_chars = 32000;
            ...
            LOG_INFO("FileManagerTools", "read_file_contents: " + f.original_name +
                     " (" + QString::number(content.length()) + " chars)");
    ...
    // ── search_files ────────────────────────────────────────────────────
    {
        ...
        t.name        = "search_files";
        t.description =
            "Search managed files by name keyword, source screen, MIME type, or date range. "
        ...
        t.input_schema.properties = QJsonObject{
            {"query",       QJsonObject{{"type", "string"},
                            {"description", "Keyword to match against file name (case-insensitive)"}}},
            {"screen",      QJsonObject{{"type", "string"},
        ...
        t.handler = [](const QJsonObject& args) -> ToolResult {
            ...
            for (const auto& v : all) {
                ...
                if (!query.isEmpty() &&
                    !obj["originalName"].toString().toLower().contains(query))
                    continue;
                if (!screen.isEmpty() &&
                    !obj["sourceScreen"].toString().toLower().contains(screen))
                    continue;
                if (!mime.isEmpty() &&
                    !obj["type"].toString().toLower().contains(mime))
                    continue;
                if (dt_after.isValid()) {
                    QDateTime uploaded = QDateTime::fromString(obj["uploadedAt"].toString(), Qt::ISODate);
                    if (uploaded < dt_after) continue;
                }
                if (dt_before.isValid()) {
                    QDateTime uploaded = QDateTime::fromString(obj["uploadedAt"].toString(), Qt::ISODate);
                    if (uploaded > dt_before) continue;
                }
