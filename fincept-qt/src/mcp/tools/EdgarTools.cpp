...
static ToolResult run_edgar(const QStringList& args) {
    ...
    fincept::python::PythonRunner::instance().run(
        SCRIPT, args,
        [&](const fincept::python::PythonResult& r) {
            if (!r.success) {
...
std::vector<ToolDef> get_edgar_tools() {
    ...
    // ── edgar_resolve_cik ────────────────────────────────────────────────
    {
        ...
        t.input_schema.properties = QJsonObject{
            {"company", QJsonObject{{"type", "string"}, {"description", "Company name or ticker (e.g. AAPL, Apple Inc)"}}}};
    ...
    // ── edgar_get_metadata ───────────────────────────────────────────────
    {
        ...
        t.description = "Fetch SEC EDGAR company metadata: name, ticker, SIC code, state of incorporation, fiscal year end.";
        t.category    = "sec-edgar";
        t.input_schema.properties = QJsonObject{
            {"cik", QJsonObject{{"type", "string"}, {"description", "10-digit SEC CIK number"}}}};
    ...
    // ── edgar_get_financials ─────────────────────────────────────────────
    {
        ...
        t.description = "Fetch XBRL financials from SEC EDGAR: revenue, EBITDA, net income, debt, cash, shares. Returns annual and TTM figures.";
        t.category    = "sec-edgar";
        t.input_schema.properties = QJsonObject{
            {"cik",     QJsonObject{{"type", "string"},  {"description", "10-digit SEC CIK number"}}},
            {"periods", QJsonObject{{"type", "integer"}, {"description", "Number of annual periods to fetch (default: 4)"}}}};
        t.input_schema.required = {"cik"};
        t.handler = [](const QJsonObject& args) -> ToolResult {
            QString cik = args["cik"].toString().trimmed();
            if (cik.isEmpty())
    ...
    // ── edgar_get_filing_text ────────────────────────────────────────────
    {
        ...
        t.description = "Fetch the full text of the latest SEC filing (10-K, 10-Q, 8-K, etc.) for a company by ticker or CIK.";
        t.category    = "sec-edgar";
        t.input_schema.properties = QJsonObject{
            {"ticker", QJsonObject{{"type", "string"}, {"description", "Ticker symbol or CIK number"}}},
            {"form",   QJsonObject{{"type", "string"}, {"description", "Filing form type: 10-K, 10-Q, 8-K (default: 10-K)"}}}};
        t.input_schema.required = {"ticker"};
        t.handler = [](const QJsonObject& args) -> ToolResult {
            QString ticker = args["ticker"].toString().trimmed();
            QString form   = args["form"].toString().trimmed();
            if (form.isEmpty()) form = "10-K";
            if (ticker.isEmpty())
    ...
    // ── edgar_search_filings ─────────────────────────────────────────────
    {
        ...
        t.description = "Search recent SEC EDGAR filings for a company by ticker. Specify form type (8-K, 10-K, S-4, etc.) and date range.";
        t.category    = "sec-edgar";
        t.input_schema.properties = QJsonObject{
            {"ticker",      QJsonObject{{"type", "string"},  {"description", "Ticker symbol or CIK"}}},
            {"form",        QJsonObject{{"type", "string"},  {"description", "Filing form type (default: 8-K)"}}},
            {"months_back", QJsonObject{{"type", "integer"}, {"description", "How many months back to search (default: 12)"}}}};
        t.input_schema.required = {"ticker"};
        t.handler = [](const QJsonObject& args) -> ToolResult {
            QString ticker = args["ticker"].toString().trimmed();
            if (ticker.isEmpty())
    ...
    // ── edgar_calc_multiples ─────────────────────────────────────────────
    {
        ...
        t.description = "Calculate valuation multiples (EV/Revenue, EV/EBITDA, implied P/E, price per share) given a deal value and target CIK.";
        t.category    = "sec-edgar";
        t.input_schema.properties = QJsonObject{
    ...
    // ── edgar_get_filing_documents ───────────────────────────────────────
    {
        ...
        t.description = "List all documents (exhibits, attachments) in the latest SEC filing for a company by ticker and form type.";
        t.category    = "sec-edgar";
        t.input_schema.properties = QJsonObject{
            {"ticker", QJsonObject{{"type", "string"}, {"description", "Ticker symbol or CIK"}}},
    ...
    // ── edgar_find_merger_agreement ──────────────────────────────────────
    {
        ...
        t.description = "Find merger agreement documents (EX-2.1, S-4, DEFM14A) in SEC filings for a company by ticker.";
        t.category    = "sec-edgar";
        t.input_schema.properties = QJsonObject{
            {"ticker", QJsonObject{{"type", "string"}, {"description", "Ticker symbol or CIK"}}}};
        t.input_schema.required = {"ticker"};
    ...
    // ── edgar_10k_sections ───────────────────────────────────────────────
    {
        ...
        t.description = "Extract specific sections from a company's latest 10-K annual report (business, risk_factors, mda, financials, etc.).";
        t.category    = "sec-edgar";
        t.input_schema.properties = QJsonObject{
            {"ticker",   QJsonObject{{"type", "string"}, {"description", "Ticker symbol (e.g. AAPL)"}}},
            {"sections", QJsonObject{{"type", "string"}, {"description", "Comma-separated sections: business, risk_factors, mda, financials, controls, compensation, governance, all_items"}}}};
        t.input_schema.required = {"ticker"};
        t.handler = [](const QJsonObject& args) -> ToolResult {
            QString ticker   = args["ticker"].toString().trimmed();
            QString sections = args["sections"].toString().trimmed();
            if (ticker.isEmpty()) return ToolResult::fail("Missing 'ticker'");
            if (sections.isEmpty()) sections = "business";
    ...
    // ── edgar_10k_full_text ──────────────────────────────────────────────
    {
        ...
        t.input_schema.properties = QJsonObject{
            ...
            {"max_length", QJsonObject{{"type", "integer"}, {"description", "Max characters to return (default: 20000)"}}}};
        t.input_schema.required = {"ticker"};
        t.handler = [](const QJsonObject& args) -> ToolResult {
            QString ticker = args["ticker"].toString().trimmed();
            if (ticker.isEmpty()) return ToolResult::fail("Missing 'ticker'");
    ...
    // ── edgar_10k_search ─────────────────────────────────────────────────
    {
        ...
        t.input_schema.properties = QJsonObject{
            {"ticker",      QJsonObject{{"type", "string"},  {"description", "Ticker symbol"}}},
            {"query",       QJsonObject{{"type", "string"},  {"description", "Search query (e.g. revenue recognition, litigation)"}}},
        ...
        t.handler = [](const QJsonObject& args) -> ToolResult {
            ...
            if (ticker.isEmpty() || query.isEmpty()) return ToolResult::fail("Missing 'ticker' or 'query'");
    ...
    // ── edgar_10q_sections ───────────────────────────────────────────────
    {
        ...
        t.input_schema.properties = QJsonObject{
            {"ticker",   QJsonObject{{"type", "string"}, {"description", "Ticker symbol"}}},
            {"sections", QJsonObject{{"type", "string"}, {"description", "Comma-separated sections: part_i_item_1, part_i_item_2 (MD&A), part_i_item_3, part_ii_item_1, all_items"}}}};
        t.input_schema.required = {"ticker"};
        t.handler = [](const QJsonObject& args) -> ToolResult {
            QString ticker   = args["ticker"].toString().trimmed();
            QString sections = args["sections"].toString().trimmed();
            if (ticker.isEmpty()) return ToolResult::fail("Missing 'ticker'");
            if (sections.isEmpty()) sections = "part_i_item_2";
    ...
    // ── edgar_10q_full_text ──────────────────────────────────────────────
    {
        ...
        t.input_schema.properties = QJsonObject{
            {"ticker",     QJsonObject{{"type", "string"},  {"description", "Ticker symbol"}}},
            {"max_length", QJsonObject{{"type", "integer"}, {"description", "Max characters to return (default: 15000)"}}}};
        t.input_schema.required = {"ticker"};
        t.handler = [](const QJsonObject& args) -> ToolResult {
            QString ticker = args["ticker"].toString().trimmed();
            if (ticker.isEmpty()) return ToolResult::fail("Missing 'ticker'");
    ...
    // ── edgar_8k_events ──────────────────────────────────────────────────
    {
        ...
        t.description = "Get recent 8-K corporate event filings for a company (earnings, M&A, leadership changes, etc.).";
        t.category    = "sec-edgar";
        t.input_schema.properties = QJsonObject{
            {"ticker", QJsonObject{{"type", "string"},  {"description", "Ticker symbol"}}},
    ...
    // ── edgar_8k_full_text ───────────────────────────────────────────────
    {
        ...
        t.input_schema.properties = QJsonObject{
            {"ticker",     QJsonObject{{"type", "string"},  {"description", "Ticker symbol"}}},
            {"max_length", QJsonObject{{"type", "integer"}, {"description", "Max characters to return (default: 10000)"}}}};
        t.input_schema.required = {"ticker"};
        t.handler = [](const QJsonObject& args) -> ToolResult {
            QString ticker = args["ticker"].toString().trimmed();
            if (ticker.isEmpty()) return ToolResult::fail("Missing 'ticker'");
    ...
    // ── edgar_insider_transactions ───────────────────────────────────────
    {
        ...
        t.description = "Get recent insider transactions (Form 4) for a company — buys/sells by executives, directors, major shareholders.";
        t.category    = "sec-edgar";
        t.input_schema.properties = QJsonObject{
            {"ticker", QJsonObject{{"type", "string"},  {"description", "Ticker symbol"}}},
    ...
    // ── edgar_13f_holdings ───────────────────────────────────────────────
    {
        ...
        t.description = "Get 13F institutional holdings for a company across recent quarters (hedge funds, mutual funds, etc.).";
        t.category    = "sec-edgar";
        t.input_schema.properties = QJsonObject{
            {"ticker",   QJsonObject{{"type", "string"},  {"description", "Ticker symbol"}}},
    ...
    // ── edgar_10k_metadata ───────────────────────────────────────────────
    {
        ...
        t.description = "Get metadata for a company's latest 10-K filing: accession number, filing date, period of report.";
        t.category    = "sec-edgar";
        t.input_schema.properties = QJsonObject{
            {"ticker", QJsonObject{{"type", "string"}, {"description", "Ticker symbol"}}}};
        t.input_schema.required = {"ticker"};
    ...
    // ── edgar_10q_metadata ───────────────────────────────────────────────
    {
        ...
        t.description = "Get metadata for a company's latest 10-Q filing: accession number, filing date, period of report.";
        t.category    = "sec-edgar";
        t.input_schema.properties = QJsonObject{
            {"ticker", QJsonObject{{"type", "string"}, {"description", "Ticker symbol"}}}};
        t.input_schema.required = {"ticker"};
    ...
    // ── edgar_8k_events_categorized ──────────────────────────────────────
    {
        ...
        t.description = "Get recent 8-K corporate events categorized by type (M&A, earnings, leadership, bankruptcy, etc.).";
        t.category    = "sec-edgar";
        t.input_schema.properties = QJsonObject{
            {"ticker", QJsonObject{{"type", "string"},  {"description", "Ticker symbol"}}},
    ...
    // ── edgar_get_financial_metrics ──────────────────────────────────────
    {
        ...
        t.description = "Get key financial metrics for a company: revenue, net income, EPS, profit margin from SEC XBRL data.";
        t.category    = "sec-edgar";
        t.input_schema.properties = QJsonObject{
            {"ticker", QJsonObject{{"type", "string"}, {"description", "Ticker symbol"}}}};
        t.input_schema.required = {"ticker"};
    ...
    // ── edgar_get_company_facts ──────────────────────────────────────────
    {
        ...
        t.handler = [](const QJsonObject& args) -> ToolResult {
            QString cik = args["cik"].toString().trimmed();
            if (cik.isEmpty()) return ToolResult::fail("Missing 'cik'");
    ...
    // ── edgar_find_company ───────────────────────────────────────────────
    {
        ...
        t.description = "Search SEC EDGAR for companies by name with fuzzy matching. Returns list of matches with ticker and CIK.";
        t.category    = "sec-edgar";
        t.input_schema.properties = QJsonObject{
            {"query", QJsonObject{{"type", "string"},  {"description", "Company name or partial name (e.g. Apple, Tesla)"}}},
    ...
    // ── edgar_resolve_ticker ─────────────────────────────────────────────
    {
        ...
        t.category    = "sec-edgar";
        t.input_schema.properties = QJsonObject{
            {"query", QJsonObject{{"type", "string"}, {"description", "Company name (e.g. Apple Inc)"}}}};
        t.input_schema.required = {"query"};
        t.handler = [](const QJsonObject& args) -> ToolResult {
            QString query = args["query"].toString().trimmed();
            if (query.isEmpty()) return ToolResult::fail("Missing 'query'");
