...
void agents_internal::register_execution_tools(std::vector<ToolDef>& tools) {
    // ── 7. run_agent ────────────────────────────────────────────────────
    {
        ...
        t.async_handler = [](const QJsonObject& args, ToolContext ctx, std::shared_ptr<QPromise<ToolResult>> promise) {
            ...
            dispatch_agent_run(
                [query, config]() { return services::AgentService::instance().run_agent(query, config); },
    ...
    // ── 8. run_agent_structured ─────────────────────────────────────────
    {
        ...
        t.input_schema = ToolSchemaBuilder()
                             ...
                             .required()
                             .length(1, 8192)
                             .string("output_model", "Pydantic model name on the Python side")
                             .required()
                             .length(1, 128)
    ...
    // ── 9. route_agent_query ────────────────────────────────────────────
    {
        ...
        t.input_schema = ToolSchemaBuilder().string("query", "Query to route").required().length(1, 8192).build();
        t.async_handler = [](const QJsonObject& args, ToolContext ctx, std::shared_ptr<QPromise<ToolResult>> promise) {
            const QString query = args["query"].toString();
            auto* svc = &services::AgentService::instance();
            AsyncDispatch::callback_to_promise(svc, std::move(ctx), promise, [svc, query](auto resolve) {
    ...
    // ── 10. execute_multi_agent_query ───────────────────────────────────
    {
        ...
        t.input_schema = ToolSchemaBuilder()
                             ...
                             .required()
                             .length(1, 8192)
                             .boolean("aggregate", "Combine responses into a single result")
                             .default_bool(true)
                             .object("config", "Optional agent config override")
                             .build();
        t.async_handler = [](const QJsonObject& args, ToolContext ctx, std::shared_ptr<QPromise<ToolResult>> promise) {
            ...
            AsyncDispatch::callback_to_promise(svc, std::move(ctx), promise,
                                               [svc, query, aggregate, config](auto resolve) {
                                                   auto* holder = new QObject(svc);
                                                   QObject::connect(svc, &services::AgentService::multi_query_result,
                                                                    holder, [resolve, holder](QJsonObject result) {
...

    {
        ...
        t.async_handler = [bridge_workflow_no_reqid](const QJsonObject& args, ToolContext ctx,
            ...
            bridge_workflow_no_reqid([data]() { services::AgentService::instance().run_portfolio_rebalancing(data); },
                                     std::move(ctx), promise);
        };
        tools.push_back(std::move(t));
    }

    {
        ...
        t.async_handler = [bridge_workflow_no_reqid](const QJsonObject& args, ToolContext ctx,
                                                     std::shared_ptr<QPromise<ToolResult>> promise) {
            const QJsonObject data = args["portfolio_data"].toObject();
            bridge_workflow_no_reqid([data]() { services::AgentService::instance().run_risk_assessment(data); },
                                     std::move(ctx), promise);
        };
        tools.push_back(std::move(t));
    }

    {
        ...
        t.async_handler = [bridge_workflow_no_reqid](const QJsonObject& args, ToolContext ctx,
            ...
            bridge_workflow_no_reqid(
                [type, summary]() { services::AgentService::instance().run_portfolio_analysis(type, summary); },
                std::move(ctx), promise);
        };
        tools.push_back(std::move(t));
    }
    ...
    // ── 15. run_agent_team ──────────────────────────────────────────────
    {
        ...
        t.input_schema = ToolSchemaBuilder()
                             ...
                             .required()
                             .length(1, 8192)
                             .object("team_config", "Team config: name, mode, members[], coordinator_model")
                             .required()
                             .build();
        t.async_handler = [](const QJsonObject& args, ToolContext ctx, std::shared_ptr<QPromise<ToolResult>> promise) {
            const QString query = args["query"].toString();
            ...
            dispatch_agent_run(
                [query, team_config]() { return services::AgentService::instance().run_team(query, team_config); },
                std::move(ctx), promise);
        };
        tools.push_back(std::move(t));
    }
    ...
    // ── 16. run_agent_workflow ──────────────────────────────────────────
    {
        ...
        t.async_handler = [](const QJsonObject& args, ToolContext ctx, std::shared_ptr<QPromise<ToolResult>> promise) {
            ...
            dispatch_agent_run(
                [type, params]() { return services::AgentService::instance().run_workflow(type, params); },
                std::move(ctx), promise);
        };
        tools.push_back(std::move(t));
    }
    ...
    // ── Streaming agent (resolves on stream_done) ──────────────────────
    {
        ...
        t.description =
            "Run an agent in streaming mode; resolves with the final assembled response when streaming completes.";
        t.category = "agents";
        t.is_destructive = true;
        t.default_timeout_ms = kDefaultAgentTimeoutMs;
        t.input_schema = ToolSchemaBuilder()
        ...
        t.async_handler = [](const QJsonObject& args, ToolContext ctx, std::shared_ptr<QPromise<ToolResult>> promise) {
            ...
            const QJsonObject config = args["config"].toObject();
            auto* svc = &services::AgentService::instance();
            AsyncDispatch::callback_to_promise(svc, std::move(ctx), promise, [svc, query, config](auto resolve) {
    ...
    // ── execute_routed_agent_query ────────────────────────────────────
    {
        ...
        t.description =
            "Route a query and execute it on the chosen agent in one call. Optional session_id for context.";
        t.category = "agents";
        t.is_destructive = true;
        t.default_timeout_ms = kDefaultAgentTimeoutMs;
        t.input_schema = ToolSchemaBuilder()
                             ...
                             .string("session_id", "Optional session id for memory context")
                             .default_str("")
                             .length(0, 128)
                             .build();
        t.async_handler = [](const QJsonObject& args, ToolContext ctx, std::shared_ptr<QPromise<ToolResult>> promise) {
