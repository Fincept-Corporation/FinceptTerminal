...
Result<void> apply_v012(QSqlDatabase& db) {
    ...
    r = sql_v012(db,
        "CREATE INDEX IF NOT EXISTS idx_data_mappings_source "
        "ON data_mappings(source_id)"
    );
    if (r.is_err()) return r;

    r = sql_v012(db,
        "CREATE INDEX IF NOT EXISTS idx_data_mappings_schema "
    ...
    r = sql_v012(db,
        "CREATE INDEX IF NOT EXISTS idx_normalized_data_mapping "
        "ON normalized_data(mapping_id, extracted_at DESC)"
    );
    if (r.is_err()) return r;

    r = sql_v012(db,
        "CREATE INDEX IF NOT EXISTS idx_normalized_data_schema "
