...
static const char* kOrderCols =
    "id, portfolio_id, symbol, side, order_type, quantity, price, stop_price, "
    "filled_qty, avg_price, status, reduce_only, created_at, filled_at";

static const char* kPositionCols =
...
Result<void> PaperTradingRepository::insert_portfolio(const PtPortfolio& p) {
    return exec_write(
        "INSERT INTO pt_portfolios (id, name, initial_balance, balance, currency, "
        "leverage, margin_mode, fee_rate, exchange, created_at) "
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        {p.id, p.name, p.initial_balance, p.balance, p.currency,
         p.leverage, p.margin_mode, p.fee_rate, p.exchange, p.created_at});
}

Result<PtPortfolio> PaperTradingRepository::get_portfolio(const QString& id) {
    return query_one(
        QString("SELECT %1 FROM pt_portfolios WHERE id = ?").arg(kPortfolioCols),
        {id}, map_portfolio);
}

std::optional<PtPortfolio> PaperTradingRepository::find_portfolio(const QString& name,
                                                                    const QString& exchange) {
    return query_optional(
        QString("SELECT %1 FROM pt_portfolios WHERE name = ? AND exchange = ?").arg(kPortfolioCols),
...
Result<QVector<PtPortfolio>> PaperTradingRepository::list_portfolios(const QString& exchange) {
    if (exchange.isEmpty()) {
        return query_list(
            QString("SELECT %1 FROM pt_portfolios ORDER BY created_at DESC").arg(kPortfolioCols),
            {}, map_portfolio);
    }
    return query_list(
...
Result<void> PaperTradingRepository::insert_order(const PtOrder& o) {
    return exec_write(
        ...
        {o.id, o.portfolio_id, o.symbol, o.side, o.order_type, o.quantity,
         ...
         o.stop_price.has_value() ? QVariant(o.stop_price.value()) : QVariant(),
         o.filled_qty,
         o.avg_price.has_value() ? QVariant(o.avg_price.value()) : QVariant(),
         o.status, o.reduce_only ? 1 : 0, o.created_at});
}

Result<PtOrder> PaperTradingRepository::get_order(const QString& id) {
    auto r = db().execute(
        QString("SELECT %1 FROM pt_orders WHERE id = ?").arg(kOrderCols), {id});
    if (r.is_err()) return Result<PtOrder>::err(r.error());
    auto& q = r.value();
    if (!q.next()) return Result<PtOrder>::err("Not found");
    return Result<PtOrder>::ok(map_order(q));
}

Result<QVector<PtOrder>> PaperTradingRepository::get_orders(const QString& portfolio_id,
    ...
    if (r.is_err()) return Result<QVector<PtOrder>>::err(r.error());
    QVector<PtOrder> result;
    auto& q = r.value();
    while (q.next()) result.append(map_order(q));
    return Result<QVector<PtOrder>>::ok(std::move(result));
}

Result<void> PaperTradingRepository::update_order_fill(const QString& id,
...
Result<void> PaperTradingRepository::cancel_all_orders(const QString& portfolio_id) {
    return exec_write(
        "UPDATE pt_orders SET status = 'cancelled' WHERE portfolio_id = ? AND status = 'pending'",
        {portfolio_id});
}
...
Result<void> PaperTradingRepository::insert_position(const PtPosition& p) {
    return exec_write(
        "INSERT INTO pt_positions (id, portfolio_id, symbol, side, quantity, entry_price, "
        "current_price, unrealized_pnl, realized_pnl, leverage, liquidation_price, opened_at) "
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        {p.id, p.portfolio_id, p.symbol, p.side, p.quantity, p.entry_price,
         p.current_price, p.unrealized_pnl, p.realized_pnl, p.leverage,
...
Result<QVector<PtPosition>> PaperTradingRepository::get_positions(const QString& portfolio_id) {
    auto r = db().execute(
        ...
        {portfolio_id});
    if (r.is_err()) return Result<QVector<PtPosition>>::err(r.error());
    QVector<PtPosition> result;
    auto& q = r.value();
    while (q.next()) result.append(map_position(q));
    return Result<QVector<PtPosition>>::ok(std::move(result));
}

std::optional<PtPosition> PaperTradingRepository::find_position(const QString& portfolio_id,
...
Result<void> PaperTradingRepository::update_position(const QString& id,
...
Result<void> PaperTradingRepository::update_position_price(const QString& portfolio_id,
                                                             const QString& symbol, double price) {
    return exec_write(
        "UPDATE pt_positions SET current_price = ? WHERE portfolio_id = ? AND symbol = ?",
...
Result<void> PaperTradingRepository::insert_trade(const PtTrade& t) {
    return exec_write(
        ...
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        {t.id, t.portfolio_id, t.order_id, t.symbol, t.side,
         t.price, t.quantity, t.fee, t.pnl, t.timestamp});
}

Result<QVector<PtTrade>> PaperTradingRepository::get_trades(const QString& portfolio_id,
                                                              int64_t limit) {
    auto r = db().execute(
        QString("SELECT %1 FROM pt_trades WHERE portfolio_id = ? ORDER BY timestamp DESC LIMIT %2")
            .arg(kTradeCols).arg(limit),
        {portfolio_id});
    if (r.is_err()) return Result<QVector<PtTrade>>::err(r.error());
    QVector<PtTrade> result;
    auto& q = r.value();
    while (q.next()) result.append(map_trade(q));
...
Result<PtStats> PaperTradingRepository::get_stats(const QString& portfolio_id) {
    ...
    if (r.is_err()) return Result<PtStats>::err(r.error());

    auto& q = r.value();
    if (!q.next()) return Result<PtStats>::ok(PtStats{});
    ...
    s.win_rate        = s.total_trades > 0
                        ? static_cast<double>(s.winning_trades) / static_cast<double>(s.total_trades)
                        : 0.0;
