...
LlmModelConfig LlmConfigRepository::map_model(QSqlQuery& q) {
    return {q.value(0).toString(), q.value(1).toString(), q.value(2).toString(),
            ...
            q.value(6).toBool(), q.value(7).toBool(),
...
Result<QVector<LlmConfig>> LlmConfigRepository::list_providers() {
    return query_list(
        ...
        "FROM llm_configs ORDER BY provider", {}, map_config);
}

Result<LlmConfig> LlmConfigRepository::get_active_provider() {
    return query_one(
        "SELECT provider, api_key, base_url, model, is_active, created_at, updated_at "
        "FROM llm_configs WHERE is_active = 1 LIMIT 1", {}, map_config);
}
...
Result<LlmGlobalSettings> LlmConfigRepository::get_global_settings() {
    ...
    if (r.is_err()) return Result<LlmGlobalSettings>::err(r.error());
    auto& q = r.value();
    if (!q.next()) return Result<LlmGlobalSettings>::ok(LlmGlobalSettings{});
    return Result<LlmGlobalSettings>::ok({
        q.value(0).toDouble(), q.value(1).toInt(), q.value(2).toString()});
}

Result<void> LlmConfigRepository::save_global_settings(const LlmGlobalSettings& s) {
    return exec_write(
        "INSERT OR REPLACE INTO llm_global_settings (id, temperature, max_tokens, system_prompt) "
        "VALUES (1, ?, ?, ?)",
...
Result<QVector<LlmModelConfig>> LlmConfigRepository::list_models(const QString& provider) {
    if (provider.isEmpty()) {
        ...
        if (r.is_err()) return Result<QVector<LlmModelConfig>>::err(r.error());
        QVector<LlmModelConfig> result;
        auto& q = r.value();
        while (q.next()) result.append(map_model(q));
    ...
    auto r = db().execute(
        ...
        "WHERE provider = ? ORDER BY display_name", {provider});
    if (r.is_err()) return Result<QVector<LlmModelConfig>>::err(r.error());
    QVector<LlmModelConfig> result;
    auto& q = r.value();
    while (q.next()) result.append(map_model(q));
...
Result<void> LlmConfigRepository::save_model(const LlmModelConfig& m) {
    return exec_write(
        ...
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))",
        {m.id, m.provider, m.model_id, m.display_name, m.api_key, m.base_url,
         m.is_enabled ? 1 : 0, m.is_default ? 1 : 0});
}
