...
PortfolioHolding PortfolioHoldingsRepository::map_row(QSqlQuery& q) {
    return {
        ...
        q.value(3).toDouble(), q.value(4).toDouble(), q.value(5).toBool(),
...
Result<QVector<PortfolioHolding>> PortfolioHoldingsRepository::get_active() {
    return query_list(
        "SELECT id, symbol, name, shares, avg_cost, active, added_at, updated_at "
...
Result<qint64> PortfolioHoldingsRepository::add(const QString& symbol, double shares,
                                                  double avg_cost, const QString& name) {
    return exec_insert(
        "INSERT INTO portfolio_holdings (symbol, name, shares, avg_cost) VALUES (?, ?, ?, ?)",
        {symbol.toUpper(), name, shares, avg_cost});
}

Result<void> PortfolioHoldingsRepository::update(int id, double shares, double avg_cost) {
    return exec_write(
        "UPDATE portfolio_holdings SET shares = ?, avg_cost = ?, updated_at = datetime('now') "
        "WHERE id = ?",
        {shares, avg_cost, id});
}
...
Result<void> PortfolioHoldingsRepository::deactivate(int id) {
    return exec_write(
        "UPDATE portfolio_holdings SET active = 0, updated_at = datetime('now') WHERE id = ?",
        {id});
}
