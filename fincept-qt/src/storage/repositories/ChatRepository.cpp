...
ChatSession ChatRepository::map_session(QSqlQuery& q) {
    return {q.value(0).toString(), q.value(1).toString(), q.value(2).toString(),
            q.value(3).toString(), q.value(4).toInt(),
...
ChatMessage ChatRepository::map_message(QSqlQuery& q) {
    return {q.value(0).toString(), q.value(1).toString(), q.value(2).toString(),
            ...
            q.value(6).toInt(), q.value(7).toString()};
}

Result<ChatSession> ChatRepository::create_session(const QString& title,
                                                     const QString& provider,
                                                     const QString& model) {
    QString id = QUuid::createUuid().toString(QUuid::WithoutBraces);
    auto r = exec_write(
        "INSERT INTO chat_sessions (id, title, provider, model) VALUES (?, ?, ?, ?)",
        {id, title, provider, model});
    if (r.is_err()) return Result<ChatSession>::err(r.error());
...
Result<void> ChatRepository::add_message(const QString& session_id, const QString& role,
                                           const QString& content, const QString& provider,
                                           const QString& model, int tokens) {
    QString id = QUuid::createUuid().toString(QUuid::WithoutBraces);
    auto r = exec_write(
        "INSERT INTO chat_messages (id, session_id, role, content, provider, model, tokens_used) "
        "VALUES (?, ?, ?, ?, ?, ?, ?)",
...
Result<QVector<ChatMessage>> ChatRepository::get_messages(const QString& session_id) {
    auto r = db().execute(
        "SELECT id, session_id, role, content, provider, model, tokens_used, timestamp "
        "FROM chat_messages WHERE session_id = ? ORDER BY timestamp ASC",
        {session_id});
    if (r.is_err()) return Result<QVector<ChatMessage>>::err(r.error());
    QVector<ChatMessage> result;
    auto& q = r.value();
    while (q.next()) result.append(map_message(q));
