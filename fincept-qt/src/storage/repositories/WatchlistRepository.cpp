...
Watchlist WatchlistRepository::map_watchlist(QSqlQuery& q) {
    return {
        ...
        q.value(3).toString(), q.value(4).toInt(),    q.value(5).toBool(),
...
Result<Watchlist> WatchlistRepository::create(const QString& name, const QString& color) {
    ...
    auto r = exec_write(
        "INSERT INTO watchlists (id, name, color) VALUES (?, ?, ?)",
        {id, name, color});
    if (r.is_err()) return Result<Watchlist>::err(r.error());
    return get(id);
}

Result<QVector<Watchlist>> WatchlistRepository::list_all() {
    return query_list(
        "SELECT id, name, description, color, sort_order, is_default, created_at, updated_at "
...
Result<void> WatchlistRepository::update(const Watchlist& w) {
    return exec_write(
        "UPDATE watchlists SET name = ?, description = ?, color = ?, sort_order = ?, "
...
Result<void> WatchlistRepository::add_stock(const QString& watchlist_id, const QString& symbol,
                                             const QString& name, const QString& exchange) {
    return exec_write(
        "INSERT OR IGNORE INTO watchlist_stocks (watchlist_id, symbol, name, exchange) "
        "VALUES (?, ?, ?, ?)",
...
Result<QVector<WatchlistStock>> WatchlistRepository::get_stocks(const QString& watchlist_id) {
    ...
    auto r = db().execute(
        "SELECT id, watchlist_id, symbol, name, exchange, notes, sort_order, added_at "
        "FROM watchlist_stocks WHERE watchlist_id = ? ORDER BY sort_order, symbol",
        {watchlist_id});
    if (r.is_err()) return Result<QVector<WatchlistStock>>::err(r.error());
