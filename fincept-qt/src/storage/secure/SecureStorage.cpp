...
//           via the Secret Service compatibility layer), falling back to
//           XOR-obfuscated QSettings when libsecret is absent.
//           The XOR fallback is NOT cryptographically secure — it only stops
//           casual inspection. Build with libsecret-1-dev for production.
...
Result<void> SecureStorage::store(const QString& key, const QString& value) {
    ...
    // macOS Keychain via modern SecItem API. Try SecItemUpdate first; if no
    // existing item, fall through to SecItemAdd. Any other status is logged
    // (it indicates a real Keychain problem we don't want to silently mask
    // by overwriting with Add).
    ...
    CFDictionaryRef find_query = CFDictionaryCreate(
        nullptr, find_keys, find_vals, 3,
        &kCFTypeDictionaryKeyCallBacks, &kCFTypeDictionaryValueCallBacks);

    const void* upd_keys[] = {kSecValueData};
    const void* upd_vals[] = {secret_data};
    CFDictionaryRef upd_attrs = CFDictionaryCreate(
        nullptr, upd_keys, upd_vals, 1,
        &kCFTypeDictionaryKeyCallBacks, &kCFTypeDictionaryValueCallBacks);
    ...
    if (status == errSecItemNotFound) {
        ...
        CFDictionaryRef add_attrs = CFDictionaryCreate(
            nullptr, add_keys, add_vals, 4,
            &kCFTypeDictionaryKeyCallBacks, &kCFTypeDictionaryValueCallBacks);
        status = SecItemAdd(add_attrs, nullptr);
        CFRelease(add_attrs);
    } else if (status != errSecSuccess) {
        // Update returned an unexpected status — log it. Do NOT fall through
        // to Add: if the item exists but Update is rejected (ACL, locked
        // keychain, etc.) the right answer is to surface the failure, not
        // create a duplicate item.
        LOG_WARN(TAG, QString("SecItemUpdate returned unexpected OSStatus %1 for key %2")
                          .arg(status).arg(key));
    }
...
#    ifdef FINCEPT_HAVE_LIBSECRET
    ...
    if (err)
        g_error_free(err); // not fatal — item may simply not have existed
    return Result<void>::ok();
