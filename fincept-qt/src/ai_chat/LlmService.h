...
class LlmService : public QObject {
  ...
  public:
    ...
    LlmResponse chat(const QString& user_message,
    ...
    void chat_streaming(const QString& user_message,
                        const std::vector<ConversationMessage>& history,
    ...
    void fetch_models(const QString& provider, const QString& api_key,
  ...
  signals:
    ...
    void models_fetched(const QString& provider, const QStringList& models,
                        const QString& error);
  ...
  private:
    ...
    QJsonObject build_openai_request   (const QString& user_message,
                                        const std::vector<ConversationMessage>& history,
                                        bool stream, bool with_tools = true);
    QJsonObject build_anthropic_request(const QString& user_message,
                                        const std::vector<ConversationMessage>& history,
                                        bool stream);
    QJsonObject build_gemini_request   (const QString& user_message,
                                        const std::vector<ConversationMessage>& history);
    QJsonObject build_fincept_request  (const QString& user_message,
                                        const std::vector<ConversationMessage>& history,
                                        bool with_tools);
    ...
    LlmResponse do_request(const QString& user_message,
                           const std::vector<ConversationMessage>& history);
    LlmResponse do_streaming_request(const QString& user_message,
                                     const std::vector<ConversationMessage>& history,
                                     StreamCallback on_chunk);

    // Tool-call follow-up loop (OpenAI-compatible)
    LlmResponse do_tool_loop(QJsonArray loop_messages,
                              const QString& url,
                              const QMap<QString, QString>& headers);

    // Models-list helpers
    static QString get_models_url(const QString& provider, const QString& api_key,
                                  const QString& base_url);
    static QMap<QString, QString> get_models_headers(const QString& provider,
                                                      const QString& api_key);
    static QStringList parse_models_response(const QString& provider,
    ...
    static void parse_usage(LlmResponse& resp,
                             const QJsonObject& rj,
                             const QString& provider);

    // Synchronous POST helper (blocks calling thread via QEventLoop)
    struct HttpResult { bool success; int status; QByteArray body; QString error; };
    static HttpResult blocking_post(const QString& url,
