...
void RelationshipPanel::build_ui() {
    ...
    title->setStyleSheet(QString("color:#9D4EDD; font-size:%1px; font-weight:700; font-family:%2; letter-spacing:1px;")
                             .arg(ui::fonts::TINY)
    ...
    auto* stats = new QLabel(
        QString("NODES: %1 | CONFLICTS: %2 | ORGANIZATIONS: %3").arg(nodes.size()).arg(conflicts).arg(orgs), header);
    stats->setStyleSheet(
    ...
    conflict_title->setStyleSheet(QString("color:#FF0000; font-size:9px; font-weight:700; font-family:%1;"
                                          "letter-spacing:1px;")
                                      .arg(ui::fonts::DATA_FAMILY));
    ...
    crisis_title->setStyleSheet(QString("color:#FFA500; font-size:9px; font-weight:700; font-family:%1;"
                                        "letter-spacing:1px;")
                                    .arg(ui::fonts::DATA_FAMILY));
    ...
    org_title->setStyleSheet(QString("color:#2196F3; font-size:9px; font-weight:700; font-family:%1;"
                                     "letter-spacing:1px;")
                                 .arg(ui::fonts::DATA_FAMILY));
...
QWidget* RelationshipPanel::build_node_card(const RelationshipNode& node, QWidget* parent) {
    ...
    card->setStyleSheet(QString("background:%1; border:1px solid rgba(%2,%3,%4,0.3);"
    ...
    if (!node.severity.isEmpty()) {
        ...
        sev->setStyleSheet(
            QString("color:%1; font-size:9px; font-family:%2;").arg(color.name()).arg(ui::fonts::DATA_FAMILY));
        vl->addWidget(sev);
    }
