...
void RelationshipPanel::build_ui() {
    ...
    auto* stats = new QLabel(
        QString("NODES: %1  |  CONFLICTS: %2  |  ORGANIZATIONS: %3")
            .arg(nodes.size()).arg(conflicts).arg(orgs),
        header);
    stats->setStyleSheet(
...
QWidget* RelationshipPanel::build_node_card(const RelationshipNode& node, QWidget* parent) {
    ...
    const QString badge_text =
        node.severity.isEmpty() ? node.type.toUpper() : node.severity.toUpper();
    ...
    if (!node.connections.isEmpty()) {
        ...
        for (int i = 0; i < shown; ++i) {
            ...
            pill->setStyleSheet(
                QString("color:%1; font-size:%2px; padding:1px 6px;"
        ...
        if (node.connections.size() > max_pills) {
            auto* more = new QLabel(
                QString("+%1").arg(node.connections.size() - max_pills), pills_row);
            more->setStyleSheet(
                QString("color:%1; font-size:%2px; padding:1px 6px;"
                        "background:rgba(255,255,255,0.05); border:1px solid %3;"
                        "border-radius:8px; font-family:%4;")
