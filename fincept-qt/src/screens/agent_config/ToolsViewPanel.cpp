...
static QString section_title_style() {
    return QString("color:%1;font-size:10px;font-weight:700;letter-spacing:1px;")
        .arg(ui::colors::AMBER);
}
...
static QFrame* h_line() {
    ...
    f->setStyleSheet(QString("background:%1;border:none;max-height:1px;")
...
QWidget* ToolsViewPanel::build_left_panel() {
    ...
    target_status_->setStyleSheet(
        QString("color:%1;font-size:10px;padding:4px 0;").arg(ui::colors::TEXT_TERTIARY));
...
QWidget* ToolsViewPanel::build_right_panel() {
    ...
    detail_name_->setStyleSheet(
        QString("color:%1;font-size:13px;font-weight:700;padding:2px 0;")
...
void ToolsViewPanel::setup_connections() {
    ...
    connect(&svc, &services::AgentService::tools_loaded,
            this, &ToolsViewPanel::populate_tools);

    connect(&svc, &services::AgentService::agents_discovered,
            this, [this](const QVector<services::AgentInfo>& agents) {
    ...
    connect(selected_list_, &QListWidget::currentRowChanged,
            this, [this](int row) {
    ...
    connect(target_combo_, QOverload<int>::of(&QComboBox::currentIndexChanged),
            this, [this](int idx) {
                if (idx < 0) {
                ...
                target_status_->setText(
                    QString("Assigning to: %1 (%2)").arg(name, type));
...
void ToolsViewPanel::filter_tools(const QString& query) {
    ...
    for (const auto& cat : all_tools_.categories) {
        ...
        for (const auto& t : tools) {
            ...
            QString display = assigned.contains(name)
                ? QString("%1 ●").arg(name)
                : name;

            auto* child = new QTreeWidgetItem({display});
            child->setForeground(0, QColor(assigned.contains(name)
                ? ui::colors::POSITIVE()
        ...
        auto* cat_item = new QTreeWidgetItem(
            {QString("%1  [%2]").arg(cat.toUpper()).arg(matching.size())});
    ...
    if (!mcp_tools.empty()) {
        ...
        for (const auto& tool : mcp_tools) {
            ...
            QString display = assigned.contains(tool.name)
                ? QString("%1 ●").arg(tool.name)
                : tool.name;

            auto* child = new QTreeWidgetItem({display});
            child->setForeground(0, QColor(assigned.contains(tool.name)
                ? ui::colors::POSITIVE()
                : ui::colors::TEXT_PRIMARY()));
        ...
        for (auto it = by_cat.begin(); it != by_cat.end(); ++it) {
            auto* cat_item = new QTreeWidgetItem(
                {QString("%1  [%2]").arg(it.key().toUpper()).arg(it.value().size())});
...
void ToolsViewPanel::load_assign_targets() {
    ...
    if (target_combo_->count() == 0) {
        target_status_->setText(use_agents
...
void ToolsViewPanel::refresh_used_by(const QString& tool_name) {
    ...
    detail_used_by_->setPlainText(lines.isEmpty()
        ? "Not assigned to any agent or team yet."
        : lines.join("\n"));
}
...
void ToolsViewPanel::assign_to_target() {
    ...
    for (const auto& t : existing) all_tools << t.toString();
    for (const auto& t : selected_tools_) {
        if (!all_tools.contains(t)) all_tools << t;
    }

    QJsonArray merged;
    for (const auto& t : all_tools) merged.append(t);
    obj["tools"] = merged;
    ...
    if (save_result.is_ok()) {
        ...
        target_status_->setStyleSheet(
            QString("color:%1;font-size:11px;padding:2px 0;font-weight:600;")
                ...
                target_status_->setText(
                    QString("Target: %1 (%2)")
                        .arg(target_combo_->currentText(),
                             radio_agent_->isChecked() ? "Agent" : "Team"));
                target_status_->setStyleSheet(
                    QString("color:%1;font-size:11px;padding:2px 0;")
        ...
        target_status_->setStyleSheet(
            QString("color:%1;font-size:11px;padding:2px 0;").arg(ui::colors::NEGATIVE));
    }
}
