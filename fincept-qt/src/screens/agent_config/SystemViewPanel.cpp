...
static QLabel* make_stat_label(const QString& text) {
    ...
    lbl->setAlignment(Qt::AlignCenter);
    lbl->setStyleSheet(
        QString("color:%1;font-size:9px;font-weight:600;letter-spacing:1px;padding-top:2px;")
...
QWidget* SystemViewPanel::build_stats_row() {
    ...
    auto make_stat_card = [&](int col, const QString& label, const char* color) -> QLabel* {
        auto* card = new QFrame;
        card->setStyleSheet(
            QString("QFrame { background:%1;border:1px solid %2;padding:8px; }")
                .arg(ui::colors::BG_SURFACE, ui::colors::BORDER_DIM));
...
QWidget* SystemViewPanel::build_sysinfo_section() {
    ...
    version_label_->setStyleSheet(
        QString("color:%1;font-size:12px;").arg(ui::colors::CYAN));
    ...
    framework_label_->setStyleSheet(
        QString("color:%1;font-size:12px;").arg(ui::colors::CYAN));
...
void SystemViewPanel::setup_connections() {
    ...
    connect(&services::AgentService::instance(), &services::AgentService::agents_discovered, this,
            [this](QVector<services::AgentInfo> agents, QVector<services::AgentCategory>) {
                ...
                cache_count_->setText(
                    QString::number(services::AgentService::instance().cached_agent_count()));
            });

    // Tools
    connect(&services::AgentService::instance(), &services::AgentService::tools_loaded, this, [this](services::AgentToolsInfo info) {
    ...
    // System info
    connect(&services::AgentService::instance(), &services::AgentService::system_info_loaded, this, [this](services::AgentSystemInfo info) {
    ...
    if (providers.is_ok()) {
        ...
        for (const auto& p : providers.value()) {
            ...
            if (p.is_active) {
                ...
                badge->setStyleSheet(
                    QString("color:%1;font-size:8px;font-weight:700;background:%2;"
