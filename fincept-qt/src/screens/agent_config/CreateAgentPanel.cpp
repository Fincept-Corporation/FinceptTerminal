...
QWidget* CreateAgentPanel::build_saved_panel() {
    ...
    p->setStyleSheet(QString("background:%1;border-right:1px solid %2;")
...
QWidget* CreateAgentPanel::build_form_panel() {
    ...
    auto make_tag = [](const QString& label) -> QCheckBox* {
        ...
        cb->setStyleSheet(
            QString("QCheckBox{color:%1;font-size:10px;spacing:5px;padding:3px 6px;"
    ...
    struct { QCheckBox** ptr; const char* label; } toggles[] = {
    ...
    // ── AGENTIC MEMORY sub ───────────────────────────────────────────────────
    agentic_memory_sub_ = new QWidget;
    ...
    save_btn_->setStyleSheet(
        QString("QPushButton{background:%1;color:%2;border:none;padding:8px 20px;"
    ...
    status_lbl_->setStyleSheet(
        QString("color:%1;font-size:10px;padding:3px 0;").arg(ui::colors::TEXT_TERTIARY));
...
QWidget* CreateAgentPanel::build_test_panel() {
    ...
    p->setStyleSheet(QString("background:%1;border-left:1px solid %2;")
                         .arg(ui::colors::BG_SURFACE, ui::colors::BORDER_DIM));
    auto* vl = new QVBoxLayout(p);
    vl->setContentsMargins(0, 0, 0, 0);
    vl->setSpacing(0);
...
void CreateAgentPanel::setup_connections() {
    ...
    connect(&svc, &services::AgentService::config_saved, this, [this]() {
        ...
        status_lbl_->setStyleSheet(
            QString("color:%1;font-size:10px;padding:3px 0;").arg(ui::colors::POSITIVE));
        load_saved_agents();
    });
    connect(&svc, &services::AgentService::config_deleted, this, [this]() {
        status_lbl_->setText("Agent deleted");
        status_lbl_->setStyleSheet(
            QString("color:%1;font-size:10px;padding:3px 0;").arg(ui::colors::WARNING));
        editing_id_.clear();
        load_saved_agents();
    });
    ...
    connect(&svc, &services::AgentService::agent_stream_done, this,
            [this](services::AgentExecutionResult r) {
                ...
                if (r.success) {
                    ...
                    test_status_lbl_->setText(
                        QString("Done in %1ms").arg(r.execution_time_ms));
                    test_status_lbl_->setStyleSheet(
...
void CreateAgentPanel::load_profile_combo() {
    ...
    if (!prev_id.isEmpty()) {
        for (int i = 1; i < llm_profile_combo_->count(); ++i) {
            if (llm_profile_combo_->itemData(i).toString() == prev_id) { restore = i; break; }
        }
    }
...
void CreateAgentPanel::refresh_llm_pill() {
    ...
    if (!profile_id.isEmpty()) {
        ...
        for (const auto& p : profiles2) {
            if (p.id == profile_id) {
                resolved.profile_id = p.id; resolved.profile_name = p.name;
                resolved.provider   = p.provider; resolved.model_id = p.model_id;
                resolved.api_key    = p.api_key; resolved.base_url = p.base_url;
                resolved.temperature = p.temperature; resolved.max_tokens = p.max_tokens;
    ...
    if (!resolved.provider.isEmpty()) {
        ...
        if (text.length() > 55) text = text.left(53) + "..";
...
void CreateAgentPanel::load_agent_into_form(const AgentConfig& cfg) {
    ...
    if (!assigned_id.isEmpty()) {
        for (int i = 1; i < llm_profile_combo_->count(); ++i) {
            if (llm_profile_combo_->itemData(i).toString() == assigned_id) { select_idx = i; break; }
        }
    }
    llm_profile_combo_->blockSignals(false);
    ...
    for (int i = 0; i < tools_list_->count(); ++i)
        tools_list_->item(i)->setCheckState(
            selected.contains(tools_list_->item(i)->text()) ? Qt::Checked : Qt::Unchecked);
    ...
    for (const auto& u : kc["urls"].toArray()) url_list.append(u.toString());
    ...
    status_lbl_->setStyleSheet(
        QString("color:%1;font-size:10px;padding:3px 0;").arg(ui::colors::CYAN));
}
...
void CreateAgentPanel::clear_form() {
    ...
    for (auto* cb : {reasoning_check_, memory_check_, knowledge_check_, guardrails_check_,
                     agentic_memory_check_, storage_check_, tracing_check_,
                     compression_check_, hooks_check_, evaluation_check_})
        cb->setChecked(false);
    knowledge_urls_edit_->clear();
    memory_db_path_edit_->clear(); memory_table_edit_->clear();
    storage_db_path_edit_->clear(); storage_table_edit_->clear();
...
void CreateAgentPanel::save_agent() {
    ...
    if (name.isEmpty()) {
        ...
        status_lbl_->setStyleSheet(
            QString("color:%1;font-size:10px;padding:3px 0;").arg(ui::colors::NEGATIVE));
        return;
    }
    AgentConfig db;
...
void CreateAgentPanel::export_json() {
    const QString path = QFileDialog::getSaveFileName(this, "Export Agent Config", "agent_config.json", "JSON (*.json)");
