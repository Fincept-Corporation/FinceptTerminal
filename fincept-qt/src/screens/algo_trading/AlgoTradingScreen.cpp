...
QWidget* AlgoTradingScreen::build_top_bar() {
    ...
    for (int i = 0; i < tabs.size(); ++i) {
        ...
        btn->setStyleSheet(QString(
            ...
            .arg(colors[i].mid(1)).arg(colors[i]));
    ...
    deploy_count_label_->setStyleSheet(QString(
...
void AlgoTradingScreen::on_tab_changed(int index) {
    ...
    if (index == 1) AlgoTradingService::instance().list_strategies();
    if (index == 3) AlgoTradingService::instance().list_deployments();
}
