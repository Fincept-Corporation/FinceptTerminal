...
void ChatModeService::save_message(const QString& session_uuid, const QString& role,
                                   const QString& content, MessageCallback cb,
                                   const QString& provider, const QString& model,
                                   int tokens_used, int response_time_ms)
{
    LOG_INFO("ChatModeService", QString("Saving %1 message to session %2")
                 .arg(role, session_uuid));
    QJsonObject body;
    body["role"]    = role;
    body["content"] = content;
    if (!provider.isEmpty()) body["provider"] = provider;
    if (!model.isEmpty())    body["model"]    = model;
    if (tokens_used > 0)     body["tokens_used"] = tokens_used;
    if (response_time_ms > 0) body["response_time_ms"] = response_time_ms;
...
void ChatModeService::optimize_prompt(const QString& prompt, const QString& mode,
                                      OptimizeCallback cb)
{
    LOG_INFO("ChatModeService", QString("Optimizing prompt (%1): \"%2\"")
                 .arg(mode, prompt.left(60)));
    QJsonObject body;
    body["prompt"] = prompt;
    if (!mode.isEmpty()) body["mode"] = mode;

    // Longer timeout for LLM calls
    auto* reply = nam_->post(build_request("/chat/optimize-prompt"),
                              QJsonDocument(body).toJson(QJsonDocument::Compact));
    ...
    connect(reply, &QNetworkReply::finished, this,
            [this, reply, cb = std::move(cb)]() mutable {
                ...
                LOG_INFO("ChatModeService",
                         QString("Prompt optimized: %1 chars -> %2 chars")
                             .arg(result.original.length()).arg(result.optimized.length()));
                cb(true, result, {});
            });
}
...
void ChatModeService::agent_chat(const QString& query, const QString& session_id,
                                 StreamMode mode, AgentChatCallback cb,
                                 const QString& source, bool auto_approve)
{
    LOG_INFO("ChatModeService",
             QString("Agent chat [%1]: \"%2\"")
                 .arg(mode == StreamMode::Deep ? "deep" : "lite", query.left(60)));

    QJsonObject body;
    body["query"]        = query;
    body["mode"]         = (mode == StreamMode::Deep) ? "deep" : "lite";
    body["auto_approve"] = auto_approve;
    if (!session_id.isEmpty()) body["session_id"] = session_id;
    if (!source.isEmpty())     body["source"]     = source;

    // Longer timeout for agent calls
    auto* reply = nam_->post(build_request("/chat/agent/chat"),
                              QJsonDocument(body).toJson(QJsonDocument::Compact));
    auto* timer = new QTimer(reply);
    timer->setSingleShot(true);
...
void ChatModeService::get_task_activity(const QString& task_id, ActivityCallback cb,
...
QNetworkReply* ChatModeService::stream_task_activity(const QString& task_id, int after_id)
{
    ...
    const QString path = QString("/chat/agent/tasks/%1/activity/stream?after_id=%2")
                             .arg(task_id).arg(after_id);
    QNetworkRequest req = build_request(path);
...
void ChatModeService::get_credits(CreditsCallback cb)
{
    ...
    get(path, [cb = std::move(cb)](bool ok, QJsonDocument doc, QString err) {
        if (!ok) { cb(false, 0, err); return; }
        const int credits = doc.object().value("data").toObject()
                                .value("credit_balance").toInt();
        cb(true, credits, {});
    });
}
...
void ChatModeService::register_terminal_tools(const QJsonArray& tools,
                                              const QString& version,
                                              int tool_count,
                                              RegisterCallback cb)
{
    LOG_INFO("ChatModeService",
             QString("Registering %1 terminal tools (v%2)").arg(tool_count).arg(version));
    QJsonObject body;
    body["tools"]            = tools;
    body["terminal_version"] = version;
    body["tool_count"]       = tool_count;
    post("/chat/agent/terminal-tools/register", body,
         [cb = std::move(cb)](bool ok, QJsonDocument doc, QString err) {
             if (!ok) { cb(false, 0, err); return; }
...
void ChatModeService::submit_tool_result(const QString& call_id,
...
void ChatModeService::add_mcp_server(const QString& name, const QJsonObject& config,
                                     McpServerCallback cb)
{
    LOG_INFO("ChatModeService", QString("Adding MCP server: %1").arg(name));
    QJsonObject body;
    body["name"]   = name;
    body["config"] = config;
    post("/chat/agent/mcp/servers", body,
         [cb = std::move(cb), name](bool ok, QJsonDocument doc, QString err) {
             if (!ok) { cb(false, {}, err); return; }
             const QJsonObject root = doc.object();
             McpServer srv;
             srv.name        = root["name"].toString(name);
             srv.tools_count = root["tools_loaded"].toInt();
             srv.status      = root.contains("error") && !root["error"].toString().isEmpty()
                                   ? "error" : "connected";
...
{
    post("/chat/agent/mcp/servers/refresh", {},
         [cb = std::move(cb)](bool ok, QJsonDocument doc, QString err) {
             if (!ok) { cb(false, {}, 0, err); return; }
             const QJsonObject root = doc.object();
             QVector<McpServer> servers;
...
void ChatModeService::create_monitor(const QString& name, const QString& source_type,
                                     const QJsonObject& source_config,
                                     const QJsonObject& trigger_config,
                                     const QString& analysis_query,
...
void ChatModeService::handle_sse_line(const QByteArray& line)
{
    ...
    if (line.startsWith("data:")) {
        ...
        if (ev == "session-meta") {
            ...
            LOG_DEBUG("ChatModeService",
                      QString("SSE step-start: step %1").arg(step));
            ...
            LOG_DEBUG("ChatModeService",
                      QString("SSE thinking: %1 chars").arg(content.length()));
