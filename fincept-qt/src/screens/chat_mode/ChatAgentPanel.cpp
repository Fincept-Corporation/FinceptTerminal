...
static QString list_ss() {
    return QString(
        "QListWidget{background:%1;border:1px solid %2;color:%3;"
        ...
        .arg(ui::colors::BG_SURFACE, ui::colors::BORDER_DIM, ui::colors::TEXT_PRIMARY,
             ui::colors::BG_RAISED, ui::colors::AMBER, ui::colors::BORDER_MED,
             ui::colors::BG_HOVER);
}
...
void ChatAgentPanel::build_ui() {
    ...
    tab_bar_->setStyleSheet(QString(
...
QWidget* ChatAgentPanel::build_mcp_tab() {
    ...
    connect(mcp_list_, &QListWidget::itemSelectionChanged, this, [this]() {
        mcp_del_btn_->setEnabled(!mcp_list_->selectedItems().isEmpty());
    });
    ...
    ChatModeService::instance().list_tasks(
        [this](bool ok, QVector<AgentTask> tasks, QString err) {
            ...
            task_status_lbl_->setText(
                QString("%1 task(s)").arg(tasks_.size()));
            if (tasks_.isEmpty()) {
...
void ChatAgentPanel::refresh_mcp_servers() {
    ChatModeService::instance().list_mcp_servers(
        [this](bool ok, QVector<McpServer> servers, int total_tools, QString err) {
            ...
            mcp_tools_lbl_->setText(
                QString("%1 server(s) | %2 tools")
                    .arg(mcp_servers_.size()).arg(total_tools));
            ...
            for (const auto& s : mcp_servers_) {
                ...
                item->setText(QString("%1\n  %2 | %3 tools")
                    .arg(s.name, s.status).arg(s.tools_count));
...
void ChatAgentPanel::refresh_monitors() {
    ChatModeService::instance().list_monitors(
        [this](bool ok, QVector<AgentMonitor> monitors, QString err) {
            monitor_list_->clear();
            if (!ok) {
                auto* item = new QListWidgetItem("Not available.");
                item->setFlags(item->flags() & ~Qt::ItemIsEnabled);
            ...
            for (const auto& m : monitors_) {
                ...
                item->setText(QString("%1 %2\n  %3 | %4s")
                    .arg(icon, m.name, m.source_type)
                    .arg(m.check_interval_seconds));
...
void ChatAgentPanel::on_add_memory() {
    ...
    const QString key = QInputDialog::getText(
        this, "Add Memory", "Key:", QLineEdit::Normal, {}, &ok);
    ...
    const QString type = QInputDialog::getItem(
        this, "Type", "Memory type:", types, 0, false, &ok);
    if (!ok) return;

    ChatModeService::instance().save_memory(
        key.trimmed(), value.trimmed(), type,
        [this](bool saved_ok, QString err) {
            if (!saved_ok) LOG_WARN("ChatAgentPanel", "Save memory failed: " + err);
            refresh_memory();
        });
}
...
void ChatAgentPanel::on_clear_all_memory() {
    if (QMessageBox::question(this, "Clear Memory",
...
void ChatAgentPanel::on_add_schedule() {
    ...
    const QString query = QInputDialog::getText(
        this, "New Schedule", "Query:", QLineEdit::Normal, {}, &ok);
    if (!ok || query.trimmed().isEmpty()) return;

    const QString cron = QInputDialog::getText(
        this, "New Schedule", "Cron (e.g. 0 9 * * 1-5):",
    ...
    ChatModeService::instance().create_schedule(
        ...
        [this](bool created_ok, AgentSchedule, QString err) {
            if (!created_ok) LOG_WARN("ChatAgentPanel", "Create schedule failed: " + err);
            refresh_schedules();
        });
}
...
void ChatAgentPanel::on_toggle_schedule() {
    ...
    auto cb = [this](bool ok, QString err) {
        if (!ok) LOG_WARN("ChatAgentPanel", "Toggle schedule failed: " + err);
        refresh_schedules();
    };
    if (status == "active")
...
void ChatAgentPanel::on_refresh_tasks() { refresh_tasks(); }
...
void ChatAgentPanel::on_view_task_detail() {
    ...
    ChatModeService::instance().get_task(
        item->data(Qt::UserRole).toString(),
        [this](bool ok, AgentTask task, QString err) {
            if (!ok) { QMessageBox::warning(this, "Task", "Failed: " + err); return; }
            QString info = QString(
                "ID: %1\nQuery: %2\nStatus: %3\n"
                "Created: %4\nStarted: %5\nCompleted: %6\n\n"
                "Result:\n%7")
                .arg(task.task_id, task.query, task.status,
                     task.created_at, task.started_at, task.completed_at,
                     task.result.isEmpty() ? "(none)" : task.result.left(2000));
...
void ChatAgentPanel::on_send_feedback() {
    ...
    if (!item) return;
    bool ok = false;
    const QString feedback = QInputDialog::getText(
        this, "Task Feedback", "Feedback:", QLineEdit::Normal, {}, &ok);
    if (!ok || feedback.trimmed().isEmpty()) return;

    ChatModeService::instance().send_task_feedback(
        item->data(Qt::UserRole).toString(), feedback.trimmed(),
        [this](bool sent_ok, QString err) {
            if (!sent_ok) LOG_WARN("ChatAgentPanel", "Feedback failed: " + err);
            else QMessageBox::information(this, "Feedback", "Sent.");
        });
}
...
void ChatAgentPanel::on_add_mcp_server() {
    ...
    const QString name = QInputDialog::getText(
        this, "Add MCP Server", "Name:", QLineEdit::Normal, {}, &ok);
    if (!ok || name.trimmed().isEmpty()) return;

    const QStringList transports = {"stdio", "sse"};
    const QString transport = QInputDialog::getItem(
        this, "Transport", "Type:", transports, 0, false, &ok);
    if (!ok) return;
    ...
    if (transport == "stdio") {
        const QString command = QInputDialog::getText(
            this, "Command", "Command (e.g. uvx, npx):",
            QLineEdit::Normal, "uvx", &ok);
        if (!ok || command.trimmed().isEmpty()) return;

        const QString args_str = QInputDialog::getText(
            this, "Arguments", "Args (space-separated):",
            QLineEdit::Normal, {}, &ok);
        if (!ok) return;
        ...
        const QString url = QInputDialog::getText(
            this, "SSE URL", "URL:", QLineEdit::Normal, {}, &ok);
    ...
    ChatModeService::instance().add_mcp_server(
        name.trimmed(), config,
        [this](bool added_ok, McpServer srv, QString err) {
            if (!added_ok) {
                QMessageBox::warning(this, "MCP Server", "Failed: " + err);
                return;
            }
            if (srv.status == "error")
                QMessageBox::warning(this, "MCP Server",
                    QString("'%1' added with errors.").arg(srv.name));
            refresh_mcp_servers();
        });
}
...
void ChatAgentPanel::on_delete_mcp_server() {
    ...
    if (QMessageBox::question(this, "Remove Server",
            ...
            QMessageBox::Yes | QMessageBox::No) != QMessageBox::Yes)
        return;
    ChatModeService::instance().delete_mcp_server(
        name, [this](bool ok, QString err) {
            if (!ok) LOG_WARN("ChatAgentPanel", "Delete MCP server failed: " + err);
            refresh_mcp_servers();
        });
}
...
void ChatAgentPanel::on_refresh_mcp_servers() {
    ChatModeService::instance().refresh_mcp_servers(
        [this](bool ok, QVector<McpServer> servers, int total_tools, QString err) {
            if (!ok) { LOG_WARN("ChatAgentPanel", "Refresh MCP failed: " + err); return; }
            mcp_servers_ = std::move(servers);
            mcp_list_->clear();
            mcp_tools_lbl_->setText(
                QString("%1 server(s) | %2 tools")
...
void ChatAgentPanel::on_add_monitor() {
    bool ok = false;
    const QString name = QInputDialog::getText(
        this, "New Monitor", "Name:", QLineEdit::Normal, {}, &ok);
    if (!ok || name.trimmed().isEmpty()) return;

    const QStringList types = {"equity", "macro", "news"};
    const QString source_type = QInputDialog::getItem(
        this, "Source", "Data source:", types, 0, false, &ok);
    if (!ok) return;

    const QString analysis = QInputDialog::getText(
        this, "Analysis", "What should the agent analyze?:",
        QLineEdit::Normal, {}, &ok);
    if (!ok || analysis.trimmed().isEmpty()) return;

    const int interval = QInputDialog::getInt(
        this, "Interval", "Check interval (seconds):", 300, 60, 86400, 60, &ok);
    if (!ok) return;
    ...
    if (source_type == "equity") {
        ...
        if (!ok || sym.trimmed().isEmpty()) return;
        ...
        if (!ok || ind.trimmed().isEmpty()) return;
    ...
    const QString cond = QInputDialog::getText(
        this, "Trigger", "Condition:", QLineEdit::Normal, "any", &ok);
    if (!ok) return;
    trigger_config["condition"] = cond;

    ChatModeService::instance().create_monitor(
        name.trimmed(), source_type, source_config, trigger_config,
        analysis.trimmed(), interval, {},
        [this](bool created_ok, AgentMonitor, QString err) {
            if (!created_ok) QMessageBox::warning(this, "Monitor", "Failed: " + err);
            refresh_monitors();
        });
}
...
void ChatAgentPanel::on_delete_monitor() {
    ...
    if (!item) return;
    if (QMessageBox::question(this, "Delete Monitor", "Delete?",
            QMessageBox::Yes | QMessageBox::No) != QMessageBox::Yes)
        return;
    ChatModeService::instance().delete_monitor(
        item->data(Qt::UserRole).toString(),
        [this](bool ok, QString err) {
            if (!ok) LOG_WARN("ChatAgentPanel", "Delete monitor failed: " + err);
            refresh_monitors();
        });
}
...
void ChatAgentPanel::on_toggle_monitor() {
    ...
    const QString status = item->data(Qt::UserRole + 1).toString();
    auto cb = [this](bool ok, QString err) {
        if (!ok) LOG_WARN("ChatAgentPanel", "Toggle monitor failed: " + err);
        refresh_monitors();
    };
    if (status == "active")
