...
static QString str_color(const QString& s) {
    static const QString palette[] = {
        "#d97706","#06b6d4","#10b981","#8b5cf6","#ef4444",
...
void ForumPostReaderPanel::build_ui() {
    ...
    empty_page->setStyleSheet("background:#080808;");
    {
        ...
        icon->setStyleSheet(
            QString("color:#111111;font-size:36px;background:transparent;"));
        auto* lbl = new QLabel("SELECT A POST");
        lbl->setAlignment(Qt::AlignCenter);
        lbl->setStyleSheet(
            QString("color:#1a1a1a;font-size:13px;font-weight:700;letter-spacing:1.5px;"
                    "background:transparent;%1").arg(mono(13)));
        auto* sub = new QLabel("Click any post from the feed");
        sub->setAlignment(Qt::AlignCenter);
        sub->setStyleSheet(
            QString("color:#151515;font-size:11px;background:transparent;%1").arg(mono(11)));
        vl->addStretch();
        vl->addWidget(icon);
        vl->addSpacing(8);
        vl->addWidget(lbl);
    ...
    load_page->setStyleSheet("background:#080808;");
    {
        ...
        spin_label_->setStyleSheet(
            QString("color:%1;font-size:20px;background:transparent;%2")
                .arg(ui::colors::AMBER, mono(20)));
        vl->addStretch();
    ...
    category_chip_->setStyleSheet(
        QString("color:#d97706;font-size:9px;font-weight:700;letter-spacing:0.8px;"
                ...
                "padding:1px 6px;%1").arg(mono(9)));

    meta_label_ = new QLabel;
    meta_label_->setStyleSheet(
        QString("color:#2a2a2a;font-size:10px;background:transparent;%1").arg(mono(10)));
    ...
    title_label_->setStyleSheet(
        QString("color:#e5e5e5;font-size:15px;font-weight:700;line-height:1.4;"
                "background:transparent;%1").arg(mono(15)));
    ...
    body_label_->setStyleSheet(
        QString("color:#888888;font-size:13px;line-height:1.7;background:transparent;"
                "padding:16px;border-bottom:1px solid #0d0d0d;%1").arg(mono(13)));
    ...
    up_btn->setStyleSheet(
        QString("QPushButton{background:transparent;color:#333333;border:1px solid #1a1a1a;"
                "font-size:10px;font-weight:700;padding:0 8px;%1}"
                "QPushButton:hover{background:rgba(217,119,6,0.1);color:#d97706;"
                "border-color:rgba(217,119,6,0.4);}").arg(mono(10)));
    ...
    likes_label_->setStyleSheet(
        QString("color:#444444;font-size:11px;background:transparent;%1").arg(mono(11)));

    replies_label_ = new QLabel("◆ 0 replies");
    replies_label_->setStyleSheet(
        QString("color:#333333;font-size:11px;background:transparent;%1").arg(mono(11)));

    views_label_ = new QLabel("◉ 0 views");
    views_label_->setStyleSheet(
        QString("color:#222222;font-size:11px;background:transparent;%1").arg(mono(11)));
    ...
    com_hdr_lbl->setStyleSheet(
        QString("color:#222222;font-size:10px;font-weight:700;letter-spacing:1px;"
                "background:transparent;%1").arg(mono(10)));
    ...
    reply_input_->setStyleSheet(
        QString("QLineEdit{background:#0d0d0d;color:#888888;border:1px solid #1a1a1a;"
                "padding:4px 10px;font-size:12px;%1}"
                "QLineEdit:focus{border-color:#333333;color:#e5e5e5;}"
                "QLineEdit::placeholder{color:#2a2a2a;}").arg(mono(12)));
    ...
    auto submit = [this, send_btn]() {
        ...
        if (text.isEmpty() || current_detail_.post.post_uuid.isEmpty()) return;
...
void ForumPostReaderPanel::show_post(const services::ForumPostDetail& detail) {
    ...
    QString cat_color = detail.post.category_color.isEmpty()
                            ? str_color(detail.post.category_name)
    ...
    category_chip_->setStyleSheet(
        QString("color:%1;font-size:9px;font-weight:700;letter-spacing:0.8px;"
                ...
                "padding:1px 6px;%2").arg(cat_color, mono(9)));
    ...
    QString author_link =
        QString("<span style='background:%1;color:#080808;font-size:9px;"
            ...
            .arg(av_color, initials,
                 detail.post.author_name, "#555555",
                 detail.post.author_display_name.toUpper());
...
void ForumPostReaderPanel::rebuild_comments() {
    ...
    if (current_detail_.comments.isEmpty()) {
        ...
        lbl->setAlignment(Qt::AlignCenter);
        lbl->setStyleSheet(
            QString("color:#1a1a1a;font-size:11px;font-weight:700;letter-spacing:1px;"
                    "background:transparent;%1").arg(mono(11)));
        auto* sub = new QLabel("Be the first to reply");
        sub->setAlignment(Qt::AlignCenter);
        sub->setStyleSheet(
            QString("color:#141414;font-size:10px;background:transparent;%1").arg(mono(10)));
        vl->addWidget(lbl);
        vl->addWidget(sub);
    ...
    for (int i = 0; i < current_detail_.comments.size(); ++i) {
        ...
        thread_line->setStyleSheet(
            QString("background:%1;").arg(str_color(c.author_display_name)));
        ...
        author_lbl->setStyleSheet(
            QString("color:#666666;font-size:11px;font-weight:600;background:transparent;%1")
                .arg(mono(11)));

        // Time
        auto* time_lbl = new QLabel(rel_time(c.created_at));
        time_lbl->setStyleSheet(
            QString("color:#252525;font-size:10px;background:transparent;%1").arg(mono(10)));
        ...
        body->setStyleSheet(
            QString("color:#666666;font-size:12px;line-height:1.5;"
                    "background:transparent;padding-left:20px;%1").arg(mono(12)));
        ...
        auto make_act = [&](const QString& label, const QString& color, const QString& vtype) {
            ...
            connect(btn, &QPushButton::clicked, this, [this, uuid, vtype]() {
                emit vote_comment(uuid, vtype);
            });
            return btn;
        };
