...
void ForumSidebarPanel::build_ui() {
    ...
    auto mk_mini_stat = [&](const QString& label, const QString& color,
                            QLabel*& out) -> QWidget* {
...
void ForumSidebarPanel::set_categories(
    const QVector<services::ForumCategory>& cats) {
    categories_ = cats;
    rebuild_categories();
}
