...
void ForumPostListPanel::build_ui() {
    ...
    count_label_->setStyleSheet(
        QString("color:#2a2a2a;font-size:11px;background:transparent;%1").arg(mono(11)));
...
void ForumPostListPanel::show_skeleton() {
    if (skeleton_widget_) { skeleton_widget_->deleteLater(); skeleton_widget_ = nullptr; }
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        card->setStyleSheet(
            "background:#0a0a0a;border-bottom:1px solid #0d0d0d;");
...
void ForumPostListPanel::pulse_skeleton() {
    ...
    for (int i = 0; i < lay->count(); ++i) {
        ...
        for (int j = 0; j < cvl->count(); ++j) {
            ...
            if (ci && ci->widget() && ci->widget()->height() >= 8 &&
                ci->widget()->height() <= 12)
...
void ForumPostListPanel::rebuild_feed() {
    ...
    if (current_page_.posts.isEmpty()) {
        ...
        lbl->setStyleSheet(
            QString("color:#222222;font-size:13px;font-weight:700;letter-spacing:1px;"
                    "background:transparent;%1").arg(mono(13)));
        auto* sub = new QLabel("Be the first to start a discussion");
        sub->setAlignment(Qt::AlignCenter);
        sub->setStyleSheet(
            QString("color:#1a1a1a;font-size:11px;background:transparent;%1").arg(mono(11)));
        el->addStretch();
        el->addWidget(lbl);
        el->addWidget(sub);
        el->addStretch();
    ...
    for (int i = 0; i < current_page_.posts.size(); ++i) {
        ...
        QString cat_color = post.category_color.isEmpty()
                                ? str_color(post.category_name)
                                : post.category_color;

        card->setStyleSheet(
            active
                ? QString("background:#0d0d0d;border-bottom:1px solid #111111;"
                          "border-left:3px solid %1;").arg(cat_color)
        ...
        QString initials = post.author_display_name.isEmpty() ? "?"
        ...
        cat_chip->setStyleSheet(
            QString("color:%1;font-size:9px;font-weight:700;background:rgba(0,0,0,0);"
        ...
        title_lbl->setStyleSheet(
            active
                ? QString("color:#e5e5e5;font-size:13px;font-weight:700;background:transparent;%1")
                      .arg(mono(13))
                : QString("color:#cccccc;font-size:13px;background:transparent;%1")
                      .arg(mono(13)));
        ...
        auto make_eng = [&](const QString& icon, const QString& val, const QString& color) {
            ...
            ic->setStyleSheet(
                QString("color:%1;font-size:10px;background:transparent;%2").arg(color, mono(10)));
            auto* vl = new QLabel(val);
            vl->setStyleSheet(
                QString("color:#444444;font-size:10px;background:transparent;%1").arg(mono(10)));
            hl->addWidget(ic);
            hl->addWidget(vl);
            return w;
        };
        ...
        if (post.reply_count > 5) {
            ...
            hot->setStyleSheet(
                QString("color:#ef4444;font-size:9px;font-weight:700;letter-spacing:0.8px;"
    ...
    if (current_page_.page < current_page_.pages) {
        ...
        more_btn->setStyleSheet(
            QString("QPushButton{background:#0a0a0a;color:#333333;border:none;"
