...
static QWidget* make_section_hdr(const QString& title, const QString& accent = "#d97706") {
    ...
    lbl->setStyleSheet(QString("color:#707070;font-size:9px;font-weight:700;letter-spacing:1.5px;"
...
void ForumFeedPanel::build_ui() {
    ...
    posts_header_lbl_->setStyleSheet(QString("color:#707070;font-size:9px;font-weight:700;letter-spacing:1.5px;"
                                             "background:transparent;%1")
                                         .arg(M(9)));
    posts_count_lbl_ = new QLabel;
    posts_count_lbl_->setStyleSheet(QString("color:#555555;font-size:9px;background:transparent;%1").arg(M(9)));
...
void ForumFeedPanel::rebuild_stats() {
    ...
    StatDef defs[] = {
        {"CATEGORIES", fmt_n(stats_.total_categories), "#555555"}, {"POSTS", fmt_n(stats_.total_posts), "#e5e5e5"},
        {"COMMENTS", fmt_n(stats_.total_comments), "#06b6d4"},     {"VOTES", fmt_n(stats_.total_votes), "#10b981"},
        {"24H ACTIVE", fmt_n(stats_.recent_posts_24h), "#d97706"},
    };

    for (int i = 0; i < 5; ++i) {
        auto* cell = new QWidget;
        cell->setStyleSheet(i < 4 ? "background:transparent;border-right:1px solid #0e0e0e;"
        ...
        val_lbl->setStyleSheet(
            QString("color:%1;font-size:16px;font-weight:700;background:transparent;%2").arg(defs[i].color, M(16)));
        auto* lbl = new QLabel(defs[i].label);
        lbl->setStyleSheet(
            QString("color:#606060;font-size:8px;letter-spacing:1px;background:transparent;%1").arg(M(8)));
...
void ForumFeedPanel::rebuild_profile() {
    ...
    avatar->setStyleSheet(QString("color:#080808;font-size:12px;font-weight:700;background:%1;%2").arg(av_col, M(12)));
    ...
    name_lbl->setStyleSheet(
        QString("color:%1;font-size:12px;font-weight:700;background:transparent;%2").arg(av_col, M(12)));
    auto* handle = new QLabel("@" + profile_.username);
    handle->setStyleSheet(QString("color:#666666;font-size:10px;background:transparent;%1").arg(M(10)));
    ...
    if (!profile_.bio.isEmpty()) {
        ...
        bio->setStyleSheet(QString("color:#444444;font-size:11px;font-style:italic;"
                                   "background:transparent;%1")
                               .arg(M(11)));
    ...
    auto mk_ps = [&](const QString& lbl, int val, const QString& col) {
        ...
        v->setStyleSheet(QString("color:%1;font-size:11px;font-weight:700;background:transparent;%2").arg(col, M(11)));
        auto* l = new QLabel(lbl);
        l->setStyleSheet(QString("color:#666666;font-size:10px;background:transparent;%1").arg(M(10)));
        hl->addWidget(v);
        hl->addWidget(l);
        return w;
    };
    ...
    if (!profile_.signature.isEmpty()) {
        ...
        sig->setStyleSheet(QString("color:#666666;font-size:10px;background:transparent;%1").arg(M(10)));
...
void ForumFeedPanel::rebuild_contributors() {
    ...
    for (int i = 0; i < std::min(5, (int)stats_.top_contributors.size()); ++i) {
        ...
        row->setStyleSheet(i % 2 == 0 ? "background:#0a0a0a;border-bottom:1px solid #0d0d0d;"
        ...
        rank_lbl->setStyleSheet(
            QString("color:%1;font-size:10px;font-weight:700;background:transparent;%2").arg(rank_col[i], M(10)));

        // Avatar
        auto* av = new QLabel(c.display_name.left(2).toUpper());
        av->setFixedSize(18, 18);
        av->setAlignment(Qt::AlignCenter);
        av->setStyleSheet(QString("color:#080808;font-size:8px;font-weight:700;background:%1;%2")
                              .arg(det_color(c.display_name), M(8)));

        // Name
        auto* name = new QLabel(c.display_name.left(16));
        name->setStyleSheet(QString("color:#666666;font-size:11px;background:transparent;%1").arg(M(11)));

        // Rep + posts
        auto* rep = new QLabel(QString("▲ %1").arg(c.reputation));
        rep->setStyleSheet(QString("color:%1;font-size:10px;background:transparent;%2").arg(rank_col[i], M(10)));

        auto* posts = new QLabel(QString("%1 posts").arg(c.posts_count));
        posts->setStyleSheet(QString("color:#666666;font-size:10px;background:transparent;%1").arg(M(10)));
    ...
    if (stats_.top_contributors.isEmpty()) {
        ...
        empty->setStyleSheet(QString("color:#1a1a1a;font-size:10px;background:#0a0a0a;%1").arg(M(10)));
        vl->addWidget(empty);
    }
...
void ForumFeedPanel::rebuild_categories() {
    ...
    for (const auto& cat : categories_) {
        ...
        card->setStyleSheet(active ? QString("background:#111111;border:none;border-bottom:2px solid %1;").arg(cc)
        ...
        auto* dot = new QLabel("●");
        dot->setStyleSheet(QString("color:%1;font-size:8px;background:transparent;").arg(cc));
        auto* nm = new QLabel(cat.name.toUpper());
        nm->setStyleSheet(
            active ? QString("color:#e5e5e5;font-size:11px;font-weight:700;background:transparent;%1").arg(M(11))
                   : QString("color:#888888;font-size:11px;font-weight:600;background:transparent;%1").arg(M(11)));
        auto* cnt = new QLabel(QString::number(cat.post_count));
        cnt->setStyleSheet(QString("color:%1;font-size:10px;font-weight:700;background:transparent;%2").arg(cc, M(10)));
        ...
        if (!cat.description.isEmpty()) {
            ...
            desc->setStyleSheet(QString("color:#666666;font-size:10px;background:transparent;%1").arg(M(10)));
        ...
        connect(click, &QPushButton::clicked, this,
                [this, cat_copy]() { emit category_clicked(cat_copy.id, cat_copy.name, cat_copy.color); });
...
void ForumFeedPanel::rebuild_posts() {
    ...
    posts_count_lbl_->setText(
        QString("%1 posts · page %2/%3").arg(page_.total).arg(page_.page).arg(std::max(1, page_.pages)));
    ...
    if (page_.posts.isEmpty()) {
        ...
        lbl->setStyleSheet(QString("color:#555555;font-size:12px;background:transparent;%1").arg(M(12)));
        evl->addStretch();
        evl->addWidget(lbl);
        evl->addStretch();
    ...
    for (int i = 0; i < page_.posts.size(); ++i) {
        ...
        auto* card = new QWidget;
        card->setCursor(Qt::PointingHandCursor);
        card->setStyleSheet(active ? QString("background:#0c0c0c;border:1px solid #181818;"
        ...
        au->setStyleSheet(QString("color:#555555;font-size:11px;font-weight:600;background:transparent;%1").arg(M(11)));
        ...
        tm->setStyleSheet(QString("color:#666666;font-size:10px;background:transparent;%1").arg(M(10)));
        ...
        title->setStyleSheet(active ? QString("color:#f0f0f0;font-size:14px;font-weight:700;"
                                              "background:transparent;%1")
                                          .arg(M(14))
        ...
        if (!p.content.isEmpty()) {
            ...
            if (p.content.length() > 160)
                preview += "...";
            auto* body_lbl = new QLabel(preview);
            body_lbl->setWordWrap(true);
            body_lbl->setMaximumHeight(36);
            body_lbl->setStyleSheet(QString("color:#777777;font-size:12px;background:transparent;"
                                            "line-height:1.4;%1")
                                        .arg(M(12)));
        ...
        auto mk_eng = [&](const QString& icon, const QString& val, const QString& col) {
            ...
            ic->setStyleSheet(QString("color:%1;font-size:10px;background:transparent;%2").arg(col, M(10)));
            auto* v = new QLabel(val);
            v->setStyleSheet(QString("color:#666666;font-size:10px;background:transparent;%1").arg(M(10)));
            hl->addWidget(ic);
            hl->addWidget(v);
            return w;
        };
        ...
        eng_hl->addWidget(
            mk_eng("◆", QString("%1 replies").arg(p.reply_count), p.reply_count > 0 ? "#06b6d4" : "#1e1e1e"));
        eng_hl->addWidget(mk_eng("◉", fmt_n(p.views) + " views", "#1e1e1e"));
        eng_hl->addStretch();

        if (voted_up) {
            auto* voted_badge = new QLabel("✓ VOTED");
            voted_badge->setStyleSheet(QString("color:#d97706;font-size:9px;font-weight:700;"
                                               "background:transparent;%1")
                                           .arg(M(9)));
        ...
        if (p.reply_count > 3) {
            ...
            hot->setStyleSheet(QString("color:#ef4444;font-size:9px;font-weight:700;"
                                       "background:transparent;letter-spacing:0.5px;%1")
                                   .arg(M(9)));
    ...
    if (page_.page < page_.pages) {
        auto* more = new QPushButton(QString("↓  LOAD MORE  ·  %1 remaining").arg(page_.total - page_.posts.size()));
        more->setFixedHeight(34);
        more->setCursor(Qt::PointingHandCursor);
        more->setStyleSheet(QString("QPushButton{background:#0a0a0a;color:#252525;border:1px solid #0e0e0e;"
                                    "font-size:11px;font-weight:700;letter-spacing:0.5px;margin:0 12px;%1}"
                                    "QPushButton:hover{color:#d97706;border-color:#1a1a1a;}")
                                .arg(M(11)));
        int np = page_.page + 1;
