...
void ForumScreen::build_ui() {
    ...
    connect(feed_, &ForumFeedPanel::load_more_requested,
            this, [this](int page) {
                if (active_category_id_ > 0) {
                    ...
                    services::ForumService::instance().fetch_posts(
                        ...
                        [this](bool ok, services::ForumPostsPage p) {
                            if (ok) feed_->set_posts(p, active_category_color_);
                            else    feed_->set_loading(false);
                        });
                }
            });
    ...
    connect(thread_, &ForumThreadPanel::comment_submitted,
            this, [this](const QString& uuid, const QString& content) {
                services::ForumService::instance().create_comment(uuid, content,
                    [this, uuid](bool ok, const QString&) {
                        if (ok) {
                            thread_->set_loading(true);
                            services::ForumService::instance().fetch_post(uuid,
                                [this](bool ok2, services::ForumPostDetail d) {
                                    if (ok2) thread_->show_post(d);
                                    else thread_->set_loading(false);
                                });
                        }
                    });
            });
    ...
    connect(thread_, &ForumThreadPanel::vote_post,
            this, [this](const QString& uuid, const QString& vtype) {
                services::ForumService::instance().vote_post(uuid, vtype,
                    [this, uuid](bool ok, const QString&) {
                        if (ok) {
                            services::ForumService::instance().fetch_post(uuid,
                                [this](bool ok2, services::ForumPostDetail d) {
    ...
    connect(thread_, &ForumThreadPanel::vote_comment,
            this, [this](const QString& uuid, const QString& vtype) {
                services::ForumService::instance().vote_comment(uuid, vtype,
            ...
            this, [this](const QString& username) {
                services::ForumService::instance().fetch_profile(
                    username, [this](bool ok, services::ForumProfile profile) {
                        ...
                        dlg->setStyleSheet(
                            "QDialog{background:#0a0a0a;border:1px solid #1a1a1a;}"
                        ...
                        name_lbl->setStyleSheet(
                            QString("color:%1;font-size:16px;font-weight:700;%2")
                                .arg(profile.avatar_color.isEmpty() ? "#d97706"
                                                                    : profile.avatar_color, M(16)));
                        ...
                        if (!profile.bio.isEmpty()) {
                            ...
                            bio->setStyleSheet(
                                QString("color:#444444;font-size:12px;font-style:italic;%1")
                                    .arg(M(12)));
                            vl->addWidget(bio);
                        }
                        ...
                        auto mk_row = [&](const QString& l, const QString& v,
                                          const QString& vc) {
                            auto* w = new QWidget;
                            ...
                            ll->setStyleSheet(
                                QString("color:#252525;font-size:11px;%1").arg(M(11)));
                            auto* vl2 = new QLabel(v);
                            vl2->setStyleSheet(
                                QString("color:%1;font-size:11px;font-weight:700;%2")
                                    .arg(vc, M(11)));
                            hl->addWidget(ll);
                            hl->addStretch();
                            hl->addWidget(vl2);
                        ...
                        vl->addWidget(mk_row("REPUTATION",
                            QString::number(profile.reputation), "#d97706"));
                        vl->addWidget(mk_row("POSTS",
                            QString::number(profile.posts_count), "#888888"));
                        vl->addWidget(mk_row("COMMENTS",
                            QString::number(profile.comments_count), "#888888"));
                        vl->addWidget(mk_row("LIKES GIVEN",
                            QString::number(profile.likes_given), "#16a34a"));
...
void ForumScreen::build_toolbar() {
    ...
    forum_lbl->setStyleSheet(
        QString("color:#e5e5e5;font-size:12px;font-weight:700;letter-spacing:0.8px;"
                "background:transparent;%1").arg(M(12)));
    ...
    chips_scroll->setStyleSheet(
        "QScrollArea{border:none;background:transparent;border-right:1px solid #111111;}");
    ...
    trend_chip->setStyleSheet(
        QString("QPushButton{background:rgba(217,119,6,0.08);color:#d97706;"
    ...
    stat_active_lbl_->setStyleSheet(
        QString("color:#d97706;font-size:11px;font-weight:700;background:transparent;%1")
            .arg(M(11)));
    ...
    search_input_->setStyleSheet(
        QString("QLineEdit{background:#0a0a0a;color:#888888;border:1px solid #141414;"
                "padding:3px 10px;font-size:11px;%1}"
                "QLineEdit:focus{border-color:#252525;color:#e5e5e5;}"
                "QLineEdit::placeholder{color:#555555;}").arg(M(11)));
    connect(search_input_, &QLineEdit::returnPressed, this, [this]() {
        on_search(search_input_->text().trimmed());
    });
    ...
    profile_avatar_->setStyleSheet(
        QString("color:#080808;font-size:11px;font-weight:700;"
                "background:#d97706;%1").arg(M(11)));
    ...
    new_btn->setStyleSheet(
        QString("QPushButton{background:rgba(217,119,6,0.1);color:#555555;"
                ...
                "background:rgba(217,119,6,0.18);}").arg(M(10)));
...
void ForumScreen::load_initial_data() {
    ...
    services::ForumService::instance().fetch_stats(
        [this](bool ok, services::ForumStats s) {
            if (!ok) return;
            stat_posts_lbl_->setText(
                QString("%1 posts").arg(s.total_posts));
            stat_active_lbl_->setText(
                QString("%1 today").arg(s.recent_posts_24h));
            feed_->set_stats(s);
        });
    ...
    services::ForumService::instance().fetch_categories(
        [this](bool ok, QVector<services::ForumCategory> cats, services::ForumPermissions) {
            ...
            for (int i = 0; i < cats.size(); ++i) {
                if (cats[i].post_count > 0) { sel_idx = i; break; }
            }
            ...
            services::ForumService::instance().fetch_posts(
                ...
                [this, first](bool ok2, services::ForumPostsPage p) {
                    if (ok2) feed_->set_posts(p, first.color);
                    else     feed_->set_loading(false);
                });
        });
}
...
void ForumScreen::rebuild_category_chips() {
    ...
    for (const auto& cat : categories_) {
        ...
        chip->setStyleSheet(
            active
                ? QString("QPushButton{background:%1;color:#080808;"
...
void ForumScreen::on_category_selected(int id, const QString& name, const QString& color) {
    ...
    services::ForumService::instance().fetch_posts(id, 1, "latest",
        [this, color](bool ok, services::ForumPostsPage p) {
            if (ok) feed_->set_posts(p, color);
            else    feed_->set_loading(false);
        });
}
...
void ForumScreen::on_post_selected(const services::ForumPost& post) {
    ...
    services::ForumService::instance().fetch_post(post.post_uuid,
        [this](bool ok, services::ForumPostDetail d) {
            if (ok) thread_->show_post(d);
            else    thread_->set_loading(false);
        });
...
void ForumScreen::on_search(const QString& query) {
    ...
    services::ForumService::instance().search(query, 1,
        [this](bool ok, services::ForumPostsPage p) {
            if (ok) feed_->set_posts(p, "#06b6d4");
            else    feed_->set_loading(false);
        });
}
...
void ForumScreen::on_trending() {
    ...
    services::ForumService::instance().fetch_trending(
        [this](bool ok, services::ForumPostsPage p) {
            if (ok) feed_->set_posts(p, "#d97706");
            else    feed_->set_loading(false);
        });
}
...
void ForumScreen::show_new_post_dialog(int category_id) {
    ...
    hdr->setStyleSheet(
        QString("color:#d97706;font-size:14px;font-weight:700;letter-spacing:1px;%1")
            .arg(M(14)));
    vl->addWidget(hdr);
    ...
    cancel->setStyleSheet(
        QString("QPushButton{background:transparent;color:#333333;"
    ...
    submit->setStyleSheet(
        QString("QPushButton{background:rgba(217,119,6,0.12);color:#555555;"
            ...
            [this, dlg, title_edit, body_edit, category_id]() {
                ...
                services::ForumService::instance().create_post(
                    ...
                    [this](bool ok, const QString& msg) {
                        if (ok) on_category_selected(active_category_id_,
                                                     active_category_name_,
                                                     active_category_color_);
                        else LOG_WARN("ForumScreen", "Create post failed: " + msg);
                    });
            });
                        ...
                        if (ok) {
                            ...
                            services::ForumService::instance().fetch_my_profile(
                                [this](bool ok2, services::ForumProfile p) {
                                    if (!ok2) return;
                                    QString col = p.avatar_color.isEmpty()
                                                      ? "#d97706" : p.avatar_color;
                                    QString ini = p.display_name.isEmpty()
                                                      ? "?" : p.display_name.left(2).toUpper();
                                    profile_avatar_->setText(ini);
                                    profile_avatar_->setStyleSheet(
                                        QString("color:#080808;font-size:11px;"
