...
EquityPeersTab::EquityPeersTab(QWidget* parent) : QWidget(parent) {
    ...
    connect(&svc, &services::equity::EquityResearchService::peers_loaded,
            this, &EquityPeersTab::on_peers_loaded);
}
...
void EquityPeersTab::build_ui() {
    ...
    lbl->setStyleSheet(
        QString("color:%1; font-size:10px; font-weight:700; letter-spacing:1px; "
                "background:transparent; border:0;").arg(ui::colors::TEXT_SECONDARY));
    hl->addWidget(lbl);
    ...
    load_btn->setStyleSheet(
        QString("QPushButton { background:%1; color:%2; border:0; border-radius:3px; "
    ...
    status_label_->setStyleSheet(
        QString("color:%1; font-size:11px; padding:4px; background:transparent;")
    ...
    auto make_leg = [&](const QString& color, const QString& text) {
        ...
        t->setStyleSheet(QString("color:%1; font-size:10px; background:transparent; border:0;")
...
void EquityPeersTab::populate_table(const QVector<services::equity::PeerData>& peers) {
    static const QStringList headers = {
        "SYMBOL", "PRICE", "P/E", "FWD P/E", "P/B", "P/S", "PEG",
        "ROE", "ROA", "GROSS MGN", "NET MGN", "OP MGN",
    ...
    auto color_ratio = [](double v, double good, double warn) -> QColor {
        if (v <= 0.0)   return QColor("#6b7280");
        if (v <= good)  return QColor("#22c55e");
        if (v <= warn)  return QColor("#eab308");
        return QColor("#ef4444");
    };
    auto color_pct_pos = [](double v) -> QColor {
        if (v == 0.0)  return QColor("#6b7280");
        if (v > 0.0)   return QColor("#22c55e");
        return QColor("#ef4444");
    };

    auto set_cell = [&](int row, int col, const QString& text,
    ...
    auto fmt = [](double v, int dec = 2) -> QString {
        return v != 0.0 ? QString::number(v, 'f', dec) : "—";
    };
    auto fmt_pct = [](double v) -> QString {
        return v != 0.0 ? QString("%1%").arg(v * 100.0, 0, 'f', 1) : "—";
    };
    ...
    for (int r = 0; r < peers.size(); ++r) {
        ...
        sym_item->setForeground(p.symbol == current_symbol_
                                ? QColor(ui::colors::AMBER) : QColor("#22d3ee"));
        sym_item->setFont([&]{ QFont f; f.setBold(true); return f; }());
        ...
        set_cell(r, 14, fmt_pct(p.dividend_yield),
                 p.dividend_yield > 0 ? QColor("#22c55e") : QColor("#6b7280"));
        set_cell(r, 15, fmt(p.beta, 2),
                 p.beta >= 0 && p.beta <= 1.5 ? QColor("#22c55e") : QColor("#ef4444"));
    }
