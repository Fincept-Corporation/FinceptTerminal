...
QWidget* metric_card(const QString& label, QLabel*& val_out, QLabel*& sub_out,
    ...
    if (!initial_sub.isNull()) {
        ...
        sub_out->setStyleSheet(QString("color:%1; font-size:9px; background:transparent; border:0;")
...
QLabel* ratio_row(QWidget* parent_vl_owner, const QString& label, const QString& color) {
    ...
    k->setStyleSheet(QString("color:%1; font-size:10px; background:transparent; border:0;")
                     .arg(ui::colors::TEXT_SECONDARY));
    auto* v = new QLabel("—");
    v->setStyleSheet(QString("color:%1; font-size:10px; font-weight:600; background:transparent; border:0;")
                     .arg(color));
...
QChartView* make_chart_view(int fixed_height = 0) {
    ...
    if (fixed_height > 0) cv->setFixedHeight(fixed_height);
    return cv;
}
...
EquityFinancialsTab::EquityFinancialsTab(QWidget* parent) : QWidget(parent) {
    ...
    connect(&svc, &services::equity::EquityResearchService::financials_loaded,
            this, &EquityFinancialsTab::on_financials_loaded);
}
...
void EquityFinancialsTab::build_ui() {
    ...
    btn_bar->setStyleSheet(QString("background:%1; border-bottom:1px solid %2;")
                           .arg(ui::colors::BG_SURFACE, ui::colors::BORDER_DIM));
...
QWidget* EquityFinancialsTab::build_income_view() {
    ...
    // ── DuPont & Return Metrics ───────────────────────────────────────────────
    {
        ...
        ret_title->setStyleSheet(QString("color:%1; font-size:10px; font-weight:700; "
                                          "background:transparent; border:0;").arg(kCyan));
        ...
        dp_title->setStyleSheet(QString("color:%1; font-size:10px; font-weight:700; "
                                         "background:transparent; border:0;").arg(kPurple));
        ...
        auto make_dp_val = [&](QLabel*& out, const QString& label_text) {
            ...
            out->setStyleSheet(QString("color:%1; font-size:14px; font-weight:700; "
                                        "background:transparent; border:0;").arg(kGreen));
            out->setAlignment(Qt::AlignCenter);
            auto* lbl = new QLabel(label_text);
            lbl->setStyleSheet(QString("color:%1; font-size:9px; background:transparent; border:0;")
                               .arg(ui::colors::TEXT_TERTIARY));
...
QWidget* EquityFinancialsTab::build_balance_view() {
    ...
    // ── Liquidity & Leverage Ratios ───────────────────────────────────────────
    {
        ...
        et->setStyleSheet(QString("color:%1; font-size:10px; font-weight:700; "
                                   "background:transparent; border:0;").arg(kOrange));
        evl->addWidget(et);
...
void EquityFinancialsTab::populate_income_view(const services::equity::FinancialsData& d) {
    if (d.income_statement.isEmpty()) return;
    ...
    auto set = [](QLabel* l, const QString& t) { if (l) l->setText(t); };
...
void EquityFinancialsTab::populate_balance_view(const services::equity::FinancialsData& d) {
    if (d.balance_sheet.isEmpty()) return;
    ...
    if (!d.income_statement.isEmpty()) {
        ...
        if (interest < 0) interest = -interest;
    }
...
void EquityFinancialsTab::populate_cashflow_view(const services::equity::FinancialsData& d) {
    ...
    if (capex > 0) capex = -capex; // capex is usually negative
    ...
    if (dividends > 0) dividends = -dividends;
    double buybacks   = get_val(cf, {"Repurchase Of Capital Stock", "Common Stock Repurchased",
                                     "Common Stock Payments", "Purchase Of Stock"});
    if (buybacks > 0) buybacks = -buybacks;
...
void EquityFinancialsTab::rebuild_revenue_chart(const services::equity::FinancialsData& d) {
    if (d.income_statement.isEmpty() || !inc_revenue_chart_) return;
    ...
    auto* rev_set   = new QBarSet("Revenue");   rev_set->setColor(QColor(kBlue));
    auto* gross_set = new QBarSet("Gross Profit"); gross_set->setColor(QColor(kCyan));
    for (int i = 0; i < cats.size(); ++i) { *rev_set << revenue_v[i]; *gross_set << gross_v[i]; }
    ...
    for (int i = 0; i < cats.size(); ++i) net_series->append(i, net_v[i]);
...
void EquityFinancialsTab::rebuild_margin_chart(const services::equity::FinancialsData& d) {
    if (d.income_statement.isEmpty() || !inc_margin_chart_) return;
    ...
    auto make_line = [&](const QString& name, const QString& color, const QVector<double>& vals) {
        ...
        for (int i = 0; i < cats.size(); ++i) s->append(i, vals[i]);
        return s;
    };
    ...
    chart->addSeries(s3); chart->addSeries(s4);
    chart->legend()->show();
    ...
    chart->addAxis(axX, Qt::AlignBottom);
    chart->addAxis(axY, Qt::AlignLeft);
    for (auto* s : {s1, s2, s3, s4}) { s->attachAxis(axX); s->attachAxis(axY); }

    inc_margin_chart_->setChart(chart);
}

void EquityFinancialsTab::rebuild_balance_chart(const services::equity::FinancialsData& d) {
    if (d.balance_sheet.isEmpty() || !bal_chart_) return;

    int n = qMin(5, d.balance_sheet.size());
    QStringList cats;

    auto* assets_set = new QBarSet("Assets");     assets_set->setColor(QColor(kBlue));
    auto* liab_set   = new QBarSet("Liabilities"); liab_set->setColor(QColor(kRed));
    auto* equity_set = new QBarSet("Equity");      equity_set->setColor(QColor(kGreen));
...
void EquityFinancialsTab::rebuild_cashflow_chart(const services::equity::FinancialsData& d) {
    if (d.cash_flow.isEmpty() || !cf_chart_) return;

    int n = qMin(5, d.cash_flow.size());
    QStringList cats;

    auto* op_set  = new QBarSet("Operating"); op_set->setColor(QColor(kGreen));
    auto* inv_set = new QBarSet("Investing"); inv_set->setColor(QColor(kOrange));
    auto* fin_set = new QBarSet("Financing"); fin_set->setColor(QColor(kPurple));
...
void EquityFinancialsTab::rebuild_return_chart(const services::equity::FinancialsData& d) {
    if (d.income_statement.isEmpty() || d.balance_sheet.isEmpty() || !ret_chart_) return;
    ...
    chart->addAxis(axX, Qt::AlignBottom);
    chart->addAxis(axY, Qt::AlignLeft);
    roe_series->attachAxis(axX); roe_series->attachAxis(axY);
    roa_series->attachAxis(axX); roa_series->attachAxis(axY);
...
void EquityFinancialsTab::populate_table(
    QTableWidget* table,
    const QVector<QPair<QString, QJsonObject>>& stmt) {

    if (!table || stmt.isEmpty()) return;
    table->clearContents();
    ...
    for (int r = 0; r < metrics.size(); ++r) {
        ...
        for (int c = 0; c < stmt.size(); ++c) {
            ...
            if (val < 0) cell->setForeground(QColor(kRed));
...
QString EquityFinancialsTab::fmt_large(double v) {
    ...
    else if (av >= 1e6)  s = QString("%1M").arg(av / 1e6,  0, 'f', 1);
