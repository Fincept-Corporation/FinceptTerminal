...
EquityResearchScreen::EquityResearchScreen(QWidget* parent) : QWidget(parent) {
    ...
    connect(&svc, &services::equity::EquityResearchService::search_results_loaded,
            this, &EquityResearchScreen::on_search_results_loaded);
...
QWidget* EquityResearchScreen::build_search_bar() {
    ...
    container->setStyleSheet(QString("background:%1; border-bottom:1px solid %2;")
    ...
    title->setStyleSheet(QString("color:%1; font-size:13px; font-weight:700; letter-spacing:2px;")
                         .arg(ui::colors::AMBER));
    hl->addWidget(title);
    hl->addStretch();
...
QWidget* EquityResearchScreen::build_quote_bar() {
    ...
    auto make_label = [&](const QString& txt, const QString& color = "") -> QLabel* {
        ...
        else style += "color:" + QString(ui::colors::TEXT_SECONDARY) + ";";
...
void EquityResearchScreen::on_search_results_loaded(
    QVector<services::equity::SearchResult> results) {
...
void EquityResearchScreen::on_tab_changed(int index) {
    ...
    switch (index) {
        case 1: financials_tab_->set_symbol(current_symbol_); break;
        case 2: analysis_tab_->set_symbol(current_symbol_);   break;
        case 3: technicals_tab_->set_symbol(current_symbol_); break;
        case 4: talipp_tab_->set_symbol(current_symbol_);     break;
        case 5: peers_tab_->set_symbol(current_symbol_);      break;
        case 6: news_tab_->set_symbol(current_symbol_);       break;
        default: break;
    }
}
...
void EquityResearchScreen::load_symbol(const QString& symbol) {
    if (symbol.isEmpty() || symbol == current_symbol_) return;
...
void EquityResearchScreen::update_quote_bar(const services::equity::QuoteData& q) {
    ...
    hl_label_->setText(QString("H:%1  L:%2")
                       .arg(q.high, 0, 'f', 2)
                       .arg(q.low,  0, 'f', 2));
}
