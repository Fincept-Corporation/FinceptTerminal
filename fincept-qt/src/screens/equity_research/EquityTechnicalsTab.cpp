...
static const QStringList KEY_INDICATORS = {
    "RSI", "MACD", "Stoch %K", "ADX", "CCI", "MFI", "BB %B", "Williams %R"
};

// ── Signal helpers ────────────────────────────────────────────────────────────
QString EquityTechnicalsTab::signal_label(services::equity::TechSignal s) {
    switch (s) {
        case services::equity::TechSignal::StrongBuy:  return "STRONG BUY";
        case services::equity::TechSignal::Buy:        return "BUY";
        case services::equity::TechSignal::Sell:       return "SELL";
        case services::equity::TechSignal::StrongSell: return "STRONG SELL";
...
QString EquityTechnicalsTab::signal_color(services::equity::TechSignal s) {
    switch (s) {
        case services::equity::TechSignal::StrongBuy:  return "#22c55e";
        case services::equity::TechSignal::Buy:        return "#4ade80";
        case services::equity::TechSignal::Sell:       return "#f87171";
        case services::equity::TechSignal::StrongSell: return "#ef4444";
...
QString EquityTechnicalsTab::signal_bg(services::equity::TechSignal s) {
    switch (s) {
        case services::equity::TechSignal::StrongBuy:  return "#052e16";
        case services::equity::TechSignal::Buy:        return "#14532d";
        case services::equity::TechSignal::Sell:       return "#450a0a";
        case services::equity::TechSignal::StrongSell: return "#7f1d1d";
        default: return ui::colors::BG_RAISED;
    }
}
...
QString EquityTechnicalsTab::compute_signal(const services::equity::TechIndicator& ti) {
    ...
    switch (ti.signal) {
        ...
        case services::equity::TechSignal::StrongSell: return "SELL";
        default: return "NEUTRAL";
    }
}
...
EquityTechnicalsTab::EquityTechnicalsTab(QWidget* parent) : QWidget(parent) {
    ...
    connect(&svc, &services::equity::EquityResearchService::technicals_loaded,
            this, &EquityTechnicalsTab::on_technicals_loaded);
}
...
void EquityTechnicalsTab::set_symbol(const QString& symbol) {
    ...
    rating_label_->setStyleSheet(
        QString("color:%1; font-size:20px; font-weight:700; letter-spacing:2px; "
...
void EquityTechnicalsTab::build_ui() {
    ...
    rating_label_->setStyleSheet(
        QString("color:#6b7280; font-size:20px; font-weight:700; letter-spacing:2px; "
    ...
    auto make_count_cell = [&](const QString& color, const QString& label_text,
                                QLabel*& count_out) -> QWidget* {
        auto* w = new QWidget;
        w->setStyleSheet(
            QString("background:%1; border-left:2px solid %2;")
            .arg(color + "10", color));
        ...
        count_out->setStyleSheet(
            QString("color:%1; font-size:16px; font-weight:700; background:transparent; border:0;")
            .arg(color));
        auto* lbl = new QLabel(label_text);
        lbl->setAlignment(Qt::AlignCenter);
        lbl->setStyleSheet(
            QString("color:#6b7280; font-size:9px; background:transparent; border:0;"));
    ...
    total_label_->setStyleSheet(
        "color:#4b5563; font-size:9px; letter-spacing:1px; background:transparent; border:0;");
    ...
    kp_hdr->setStyleSheet(
        QString("background:%1; border-bottom:2px solid #22d3ee;").arg(ui::colors::BG_RAISED));
    ...
    kp_title->setStyleSheet(
        "color:#22d3ee; font-size:11px; font-weight:700; letter-spacing:1px; "
        "background:transparent; border:0;");
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        card->setStyleSheet(
            QString("QFrame { background:%1; border:1px solid %2; border-left:2px solid #6b7280; }")
            .arg(ui::colors::BG_BASE, ui::colors::BORDER_DIM));
        ...
        val_lbl->setStyleSheet(
            "color:#f9fafb; font-size:14px; font-weight:700; font-family:monospace; "
            "background:transparent; border:0;");
        auto* sig_lbl = new QLabel("—");
        sig_lbl->setObjectName("signal");
        sig_lbl->setStyleSheet(
            "color:#6b7280; font-size:9px; font-weight:700; background:transparent; border:0;");
    ...
    for (const auto& note : notes) {
        ...
        txt->setStyleSheet(
            "color:#6b7280; font-size:11px; line-height:1.6; background:transparent; border:0;");
...
void EquityTechnicalsTab::populate(const services::equity::TechnicalsData& data) {
    ...
    if (total > 0) {
        ...
        if      (buy_pct  >= 70) { recommendation = "STRONG BUY";  rec_color = "#22c55e"; }
        else if (buy_pct  >= 50) { recommendation = "BUY";         rec_color = "#4ade80"; }
        else if (sell_pct >= 70) { recommendation = "STRONG SELL"; rec_color = "#ef4444"; }
        else if (sell_pct >= 50) { recommendation = "SELL";        rec_color = "#f87171"; }
    }

    rating_label_->setText(recommendation);
    rating_label_->setStyleSheet(
        QString("color:%1; font-size:20px; font-weight:700; letter-spacing:2px; "
                "background:transparent; border:0;").arg(rec_color));
    ...
    for (int i = 0; i < 8 && i < cards.size(); ++i) {
        ...
        if (!name_lbl || !val_lbl || !sig_lbl) continue;
        ...
        if (display_name.length() > 10) display_name = display_name.left(10);
        ...
        sig_lbl->setStyleSheet(
            QString("color:%1; font-size:9px; font-weight:700; background:transparent; border:0;")
            .arg(sig_color));
        card->setStyleSheet(
            QString("QFrame { background:%1; border:1px solid %2; border-left:2px solid %3; }")
    ...
    struct CatDef { QString title; const QVector<services::equity::TechIndicator>* inds; QString color; };
    QList<CatDef> cats = {
    ...
    for (const auto& cat : cats) {
        ...
        hdr->setStyleSheet(
            QString("background:%1; border-left:2px solid %2; border-bottom:1px solid %3;")
        ...
        hdr_title->setStyleSheet(
            QString("color:#f9fafb; font-size:11px; font-weight:700; letter-spacing:1px; "
                    "background:transparent; border:0;"));
        hdr_l->addWidget(hdr_title);

        auto* count_badge = new QLabel(QString("%1 INDICATORS").arg(cat.inds->size()));
        count_badge->setStyleSheet(
            QString("color:#6b7280; font-size:9px; background:%1; border:1px solid %2; "
                    "padding:2px 6px;").arg(ui::colors::BG_BASE, ui::colors::BORDER_DIM));
        ...
        for (const auto& ti : *cat.inds) {
            ...
            ic_name->setStyleSheet(
                QString("color:%1; font-size:11px; background:transparent; border:0;")
                .arg(ui::colors::TEXT_PRIMARY));
            ic_name->setMinimumWidth(120);

            auto* ic_val = new QLabel(QString::number(ti.value, 'f', 4));
            ic_val->setStyleSheet(
            ...
            ic_sig->setStyleSheet(
                QString("background:%1; color:%2; border-radius:2px; padding:2px 4px; "
...
void EquityTechnicalsTab::on_technicals_loaded(services::equity::TechnicalsData data) {
    if (data.symbol != current_symbol_) return;
