...
QFrame* make_panel(const QString& title, const QString& accent_color) {
    ...
    hdr->setStyleSheet(
        QString("background:transparent; border:0; border-bottom:2px solid %1;")
        .arg(accent_color));
    ...
    lbl->setStyleSheet(
        QString("color:%1; font-size:11px; font-weight:700; letter-spacing:1px; "
                "background:transparent; border:0;").arg(accent_color));
    hl->addWidget(lbl);
    hl->addStretch();
...
QWidget* make_card(const QString& label, QLabel*& val_out, const QString& val_color) {
    ...
    val_out->setStyleSheet(
        QString("color:%1; font-size:16px; font-weight:700; font-family:monospace; "
                "background:transparent; border:0;").arg(val_color));
...
EquityAnalysisTab::EquityAnalysisTab(QWidget* parent) : QWidget(parent) {
    ...
    connect(&svc, &services::equity::EquityResearchService::info_loaded,
            this, &EquityAnalysisTab::on_info_loaded);
}
...
void EquityAnalysisTab::on_info_loaded(services::equity::StockInfo info) {
    ...
    book_val_->setText(info.book_value > 0
        ? QString("$%1").arg(info.book_value, 0, 'f', 2) : "—");

    // Revenue & Profits
    rev_val_->setText(fmt_large(info.total_revenue));
    rev_share_val_->setText(info.revenue_per_share > 0
        ? QString("$%1").arg(info.revenue_per_share, 0, 'f', 2) : "—");
...
QString EquityAnalysisTab::fmt_large(double v) {
    ...
    double a = std::abs(v);
    QString s;
    if (a >= 1e12)      s = QString("%1T").arg(a / 1e12, 0, 'f', 2);
    else if (a >= 1e9)  s = QString("%1B").arg(a / 1e9,  0, 'f', 2);
    else if (a >= 1e6)  s = QString("%1M").arg(a / 1e6,  0, 'f', 1);
    else                s = QString::number(a, 'f', 0);
    return neg ? "-" + s : s;
}
