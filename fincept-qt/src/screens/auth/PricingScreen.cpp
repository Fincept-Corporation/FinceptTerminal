...
static const char* CARD_NORMAL =
    "QWidget#plan_card { background: #0a0a0a; border: 1px solid #1a1a1a; }"
    "QLabel { border: none; background: transparent; }"
    "QFrame { border: none; background: transparent; }";

static const char* CARD_POPULAR =
    "QWidget#plan_card { background: #0a0a0a; border: 1px solid #78350f; }"
    "QLabel { border: none; background: transparent; }"
    "QFrame { border: none; background: transparent; }";

static const char* CARD_ACTIVE =
    "QWidget#plan_card { background: #0a0a0a; border: 1px solid #16a34a; }"
    "QLabel { border: none; background: transparent; }"
    "QFrame { border: none; background: transparent; }";
...
void PricingScreen::build_ui() {
    ...
    title->setStyleSheet(QString("color: #d97706; font-size: 20px; font-weight: 700; "
                                 "letter-spacing: 1px; background: transparent; %1").arg(MF));
    vl->addWidget(title);
    ...
    error_label_->setStyleSheet(QString("color: #dc2626; font-size: 12px; background: rgba(220,38,38,0.1); "
                                        "border: 1px solid #7f1d1d; padding: 6px 12px; %1").arg(MF));
    error_label_->hide();
...
void PricingScreen::fetch_plans() {
    ...
    auth::AuthApi::instance().get_subscription_plans([this](auth::ApiResponse r) {
        ...
        std::sort(plans_.begin(), plans_.end(),
                  [](const auth::SubscriptionPlan& a, const auth::SubscriptionPlan& b) {
...
QWidget* PricingScreen::create_plan_card(const auth::SubscriptionPlan& plan, int index) {
    ...
    if (is_popular) {
        ...
        badge->setStyleSheet(QString("color: #d97706; background: rgba(217,119,6,0.1); "
                                     ...
                                     "font-size: 11px; font-weight: 700; letter-spacing: 0.5px; %1").arg(MF));
    ...
    name->setStyleSheet(QString("color: %1; font-size: 16px; font-weight: 700; "
                            ...
                            .arg(is_popular ? "#d97706" : "#e5e5e5").arg(MF));
    vl->addWidget(name);
    ...
    if (plan.is_free) {
        ...
        price->setStyleSheet(QString("color: #e5e5e5; font-size: 28px; font-weight: 700; "
                                     "background: transparent; %1").arg(MF));
        vl->addWidget(price);
    } else {
    ...
    if (is_current) {
        ...
        btn->setStyleSheet(QString(
            ...
            "QPushButton:disabled { color: #16a34a; }").arg(MF));
        ...
        if (user_has_paid) {
            ...
            btn->setEnabled(false);
            btn->setStyleSheet(QString(
                "QPushButton { background: #111111; color: #404040; "
                "border: 1px solid #1a1a1a; font-size: 11px; font-weight: 700; %1 }").arg(MF));
        } else {
            btn->setText("CONTINUE FREE");
            btn->setStyleSheet(QString(
                "QPushButton { background: #111111; color: #808080; "
                "border: 1px solid #1a1a1a; font-size: 11px; font-weight: 700; %1 }"
                "QPushButton:hover { color: #e5e5e5; background: #161616; }").arg(MF));
        ...
        btn->setStyleSheet(QString(
            ...
            "QPushButton:disabled { background: #111111; color: #404040; border-color: #1a1a1a; }").arg(MF));
...
void PricingScreen::on_select_plan(const QString& plan_id) {
    ...
    auth::AuthApi::instance().create_payment_order(plan_id, [this](auth::ApiResponse r) {
        for (auto* btn : cards_container_->findChildren<QPushButton*>()) {
...
void PricingScreen::update_footer() {
    ...
    if (user_has_paid) {
        ...
        back_btn->setStyleSheet(QString(
            "QPushButton { color: #808080; background: transparent; border: none; "
            "font-size: 12px; %1 }"
            "QPushButton:hover { color: #e5e5e5; }").arg(MF));
        ...
        free_btn->setStyleSheet(QString(
            "QPushButton { color: #808080; background: transparent; border: none; "
            "font-size: 12px; %1 }"
            "QPushButton:hover { color: #e5e5e5; }").arg(MF));
