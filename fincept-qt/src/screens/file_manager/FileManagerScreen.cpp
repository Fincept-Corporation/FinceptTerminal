...
FileManagerScreen::FileManagerScreen(QWidget* parent) : QWidget(parent) {
    ...
    connect(&FileManagerService::instance(), &FileManagerService::files_changed,
            this, [this]() { render_files(); update_stats(); });
}
...
void FileManagerScreen::build_filter_bar(QVBoxLayout* root) {
    ...
    sort_combo_->addItems({"Date (newest)", "Date (oldest)", "Name (A-Z)", "Name (Z-A)",
                           "Size (largest)", "Size (smallest)", "Type"});
    ...
    static const QStringList screens = {
        ...
        "algo_trading", "ai_quant_lab", "notes", "code_editor",
        "report_builder", "data_sources", "excel"
    };
...
void FileManagerScreen::delete_selected() {
    ...
    auto reply = QMessageBox::question(
        this, "Delete Files",
        QString("Delete %1 selected file(s)? This cannot be undone.").arg(to_delete.size()),
        QMessageBox::Yes | QMessageBox::No);
...
void FileManagerScreen::show_preview(const QString& file_id) {
    ...
    QString date_str = QDateTime::fromString(f.uploaded_at, Qt::ISODate)
                           .toLocalTime().toString("yyyy-MM-dd HH:mm");
    preview_meta_->setText(
        format_size(f.size) + "  |  " + f.mime_type + "  |  " + f.source_screen + "  |  " + date_str);
    ...
    if (f.mime_type.contains("image") || f.mime_type.contains("video") ||
        f.mime_type.contains("audio") || f.mime_type.contains("zip") ||
        f.mime_type.contains("spreadsheet") || f.mime_type.contains("excel") ||
    ...
    if (f.mime_type.contains("csv") || f.original_name.endsWith(".csv")) {
        ...
        if (lines.isEmpty()) { preview_empty_->setVisible(true); return; }
        ...
        auto parse_csv_row = [](const QString& line) -> QStringList {
            ...
            for (QChar c : line) {
                ...
                else if (c == ',' && !in_quotes) { fields << cur.trimmed(); cur.clear(); }
                else cur += c;
            }
            fields << cur.trimmed();
            return fields;
        ...
        for (int r = 1; r < qMin(lines.size(), 201); ++r) {
            ...
            for (int c = 0; c < headers.size(); ++c) {
                preview_table_->setItem(r - 1, c,
                    new QTableWidgetItem(c < row.size() ? row[c] : QString()));
            }
        }
...
void FileManagerScreen::render_files() {
    ...
    for (const auto& v : all) {
        ...
        if (!active_screen_filter_.isEmpty() &&
            obj["sourceScreen"].toString() != active_screen_filter_)
            continue;
        if (!filter.isEmpty()) {
    ...
    if (files.isEmpty()) {
        if (all.isEmpty()) {
            ...
            auto* hero_hint = new QLabel(
                "Files are registered automatically when you export, save, or generate output\n"
...
void FileManagerScreen::update_stats() {
    ...
    for (const auto& v : files)
        if (v.isObject()) total_size += v.toObject()["size"].toInteger();

    stats_label_->setText(
        QString("%1 files | %2").arg(files.size()).arg(format_size(total_size)));
...
QString FileManagerScreen::open_with_label(const QString& mime) {
    if (mime.contains("ipynb")||mime.contains("python")) return "NOTEBOOK";
    if (mime.contains("spreadsheet")||mime.contains("csv")||mime.contains("excel")) return "EXCEL";
    if (mime.contains("pdf")) return "REPORT";
    return "OPEN";
}

QString FileManagerScreen::route_for_mime(const QString& mime) {
    if (mime.contains("ipynb")||mime.contains("x-ipynb")) return "code_editor";
