...
QWidget* ProfileScreen::make_panel(const QString& title) {
    auto* w = new QWidget; w->setStyleSheet(PANEL_SS);
    auto* vl = new QVBoxLayout(w); vl->setContentsMargins(0,0,0,0); vl->setSpacing(0);
    auto* hdr = new QWidget; hdr->setFixedHeight(34); hdr->setStyleSheet(HDR_SS);
    auto* hl = new QHBoxLayout(hdr); hl->setContentsMargins(12,0,12,0);
    auto* t = new QLabel(title);
    t->setStyleSheet(QString("color:#d97706;font-size:12px;font-weight:700;background:transparent;letter-spacing:0.5px;%1").arg(MF));
...
QWidget* ProfileScreen::make_data_row(const QString& label, QLabel*& value_out) {
    auto* row = new QWidget; row->setStyleSheet("background:transparent;border-bottom:1px solid #1a1a1a;");
    auto* hl = new QHBoxLayout(row); hl->setContentsMargins(12,6,12,6);
    auto* lbl = new QLabel(label);
    lbl->setStyleSheet(QString("color:#808080;font-size:12px;font-weight:700;background:transparent;letter-spacing:0.5px;%1").arg(MF));
...
QWidget* ProfileScreen::make_stat_box(const QString& label, QLabel*& value_out, const QString& color) {
    auto* w = new QWidget; w->setStyleSheet("background:#111111;border:1px solid #1a1a1a;");
    auto* vl = new QVBoxLayout(w); vl->setContentsMargins(12,14,12,14); vl->setAlignment(Qt::AlignCenter);
    value_out = new QLabel("0"); value_out->setAlignment(Qt::AlignCenter);
    value_out->setStyleSheet(QString("color:%1;font-size:28px;font-weight:700;background:transparent;%2").arg(color,MF));
    vl->addWidget(value_out);
    auto* l = new QLabel(label); l->setAlignment(Qt::AlignCenter);
    l->setStyleSheet(QString("color:#808080;font-size:10px;font-weight:700;background:transparent;letter-spacing:0.5px;%1").arg(MF));
    vl->addWidget(l);
    return w;
}
...
void ProfileScreen::build_header(QVBoxLayout* root) {
    auto* bar = new QWidget; bar->setFixedHeight(38);
    bar->setStyleSheet("background:#0a0a0a;border-bottom:1px solid #1a1a1a;");
    auto* hl = new QHBoxLayout(bar); hl->setContentsMargins(14,0,14,0); hl->setSpacing(8);
    ...
    credits_badge_->setStyleSheet(QString("color:#0891b2;font-size:12px;font-weight:700;background:transparent;%1").arg(MF));
    hl->addWidget(credits_badge_);
    plan_badge_ = new QLabel;
    plan_badge_->setStyleSheet(QString("color:#d97706;font-size:12px;font-weight:700;background:transparent;%1").arg(MF));
    hl->addWidget(plan_badge_);
    auto* rb = new QPushButton("REFRESH"); rb->setFixedHeight(22); rb->setCursor(Qt::PointingHandCursor);
    rb->setStyleSheet("QPushButton{background:#111111;color:#808080;border:1px solid #1a1a1a;padding:0 10px;"
...
void ProfileScreen::build_tab_nav(QVBoxLayout* root) {
    auto* nav = new QWidget; nav->setFixedHeight(32);
    nav->setStyleSheet("background:#111111;border-bottom:1px solid #1a1a1a;");
    auto* hl = new QHBoxLayout(nav); hl->setContentsMargins(4,0,4,0); hl->setSpacing(0);
    QStringList tabs={"OVERVIEW","USAGE","SECURITY","BILLING","SUPPORT"};
    for (int i=0; i<tabs.size(); i++) {
        auto* btn = new QPushButton(tabs[i]); btn->setFixedHeight(32); btn->setCursor(Qt::PointingHandCursor);
...
void ProfileScreen::on_section_changed(int index) {
    ...
    for (auto* btn : findChildren<QPushButton*>()) {
        ...
        btn->setStyleSheet(v.toInt()==index?
    ...
    if (index==1) fetch_usage_data();
    else if (index==2) fetch_login_history();
    else if (index==3) fetch_billing_data();
}

QWidget* ProfileScreen::build_overview() {
    auto* scroll = new QScrollArea; scroll->setWidgetResizable(true);
    scroll->setStyleSheet("QScrollArea{border:none;background:transparent;}");
    auto* page = new QWidget; auto* vl = new QVBoxLayout(page);
    ...
    auto* eb = new QPushButton("EDIT PROFILE"); eb->setFixedHeight(26); eb->setCursor(Qt::PointingHandCursor);
    eb->setStyleSheet("QPushButton{background:#111111;color:#808080;border:1px solid #1a1a1a;padding:0 12px;margin:8px 12px;"
        "font-size:11px;font-weight:700;font-family:'Consolas',monospace;}QPushButton:hover{color:#e5e5e5;}");
    ...
    ov_credits_big_ = new QLabel("0"); ov_credits_big_->setAlignment(Qt::AlignCenter);
    ov_credits_big_->setStyleSheet(QString("color:#0891b2;font-size:42px;font-weight:700;background:transparent;padding:20px 0 4px 0;%1").arg(MF));
    cvl2->addWidget(ov_credits_big_);
    auto* cl = new QLabel("AVAILABLE CREDITS"); cl->setAlignment(Qt::AlignCenter);
    cl->setStyleSheet(QString("color:#808080;font-size:10px;font-weight:700;background:transparent;letter-spacing:0.5px;padding-bottom:12px;%1").arg(MF));
    cvl2->addWidget(cl);
    auto* sp = new QFrame; sp->setFixedHeight(1); sp->setStyleSheet("background:#1a1a1a;"); cvl2->addWidget(sp);
    ...
    auto* ar = new QWidget; ar->setStyleSheet("background:transparent;");
    auto* arl = new QHBoxLayout(ar); arl->setContentsMargins(12,8,12,8); arl->setSpacing(10);
    auto* eb2 = new QPushButton("EDIT PROFILE"); eb2->setFixedHeight(26);
    eb2->setStyleSheet("QPushButton{background:#111111;color:#808080;border:1px solid #1a1a1a;padding:0 12px;"
        "font-size:11px;font-weight:700;font-family:'Consolas',monospace;}QPushButton:hover{color:#e5e5e5;}");
    connect(eb2, &QPushButton::clicked, this, &ProfileScreen::show_edit_profile_dialog);
    arl->addWidget(eb2);
    auto* lb = new QPushButton("LOGOUT"); lb->setFixedHeight(26);
    lb->setStyleSheet("QPushButton{background:rgba(220,38,38,0.1);color:#dc2626;border:1px solid #7f1d1d;padding:0 12px;"
        "font-size:11px;font-weight:700;font-family:'Consolas',monospace;}QPushButton:hover{background:#dc2626;color:#e5e5e5;}");
    connect(lb, &QPushButton::clicked, this, &ProfileScreen::show_logout_confirm);
    arl->addWidget(lb); arl->addStretch();
    act_vl->addWidget(ar);
    grid->addWidget(actions, 1, 0, 1, 2);
    vl->addLayout(grid); vl->addStretch();
    scroll->setWidget(page); return scroll;
}
...
QWidget* ProfileScreen::build_usage() {
    ...
    auto* page = new QWidget; auto* vl = new QVBoxLayout(page);
    vl->setContentsMargins(14,14,14,14); vl->setSpacing(10);
    auto* sr = new QWidget; sr->setStyleSheet("background:transparent;");
    auto* srl = new QHBoxLayout(sr); srl->setContentsMargins(0,0,0,0); srl->setSpacing(8);
    ...
    auto* smr = new QWidget; smr->setStyleSheet("background:transparent;");
    auto* smrl = new QHBoxLayout(smr); smrl->setContentsMargins(8,8,8,8); smrl->setSpacing(8);
    ...
    svl->addWidget(smr); vl->addWidget(sp);
    auto* dp = make_panel("DAILY USAGE");
    auto* dvl = qobject_cast<QVBoxLayout*>(dp->layout());
    usg_daily_table_ = new QTableWidget; usg_daily_table_->setColumnCount(3);
    usg_daily_table_->setHorizontalHeaderLabels({"DATE","REQUESTS","CREDITS"});
    usg_daily_table_->verticalHeader()->setVisible(false); usg_daily_table_->setShowGrid(false);
    ...
    dvl->addWidget(usg_daily_table_); vl->addWidget(dp);
    auto* ep = make_panel("TOP ENDPOINTS");
    auto* evl = qobject_cast<QVBoxLayout*>(ep->layout());
    usg_endpoint_table_ = new QTableWidget; usg_endpoint_table_->setColumnCount(4);
    usg_endpoint_table_->setHorizontalHeaderLabels({"ENDPOINT","REQUESTS","CREDITS","AVG MS"});
    usg_endpoint_table_->verticalHeader()->setVisible(false); usg_endpoint_table_->setShowGrid(false);
    ...
    evl->addWidget(usg_endpoint_table_); vl->addWidget(ep);
    vl->addStretch(); scroll->setWidget(page); return scroll;
}
...
QWidget* ProfileScreen::build_security() {
    ...
    auto* kr = new QWidget; kr->setStyleSheet("background:transparent;");
    auto* krl = new QHBoxLayout(kr); krl->setContentsMargins(12,10,12,10); krl->setSpacing(8);
    ...
    auto* sb = new QPushButton("SHOW"); sb->setFixedHeight(22);
    sb->setStyleSheet("QPushButton{background:#111111;color:#808080;border:1px solid #1a1a1a;padding:0 10px;"
    ...
    connect(sb, &QPushButton::clicked, this, [this,sb](){
        api_key_visible_=!api_key_visible_; sb->setText(api_key_visible_?"HIDE":"SHOW");
        sec_api_key_->setText(api_key_visible_?auth::AuthManager::instance().session().api_key:QString(20,QChar(0x2022)));
    });
    krl->addWidget(sb);
    auto* cb = new QPushButton("COPY"); cb->setFixedHeight(22);
    cb->setStyleSheet("QPushButton{background:#111111;color:#808080;border:1px solid #1a1a1a;padding:0 10px;"
        "font-size:10px;font-weight:700;font-family:'Consolas',monospace;}QPushButton:hover{color:#e5e5e5;}");
    connect(cb, &QPushButton::clicked, this, [cb](){
        auto key = auth::AuthManager::instance().session().api_key;
        if(!key.isEmpty()){QApplication::clipboard()->setText(key);cb->setText("COPIED");
        QTimer::singleShot(1500,cb,[cb](){cb->setText("COPY");});}
    });
    krl->addWidget(cb);
    auto* rg = new QPushButton("REGENERATE"); rg->setFixedHeight(22);
    rg->setStyleSheet("QPushButton{background:rgba(217,119,6,0.1);color:#d97706;border:1px solid #78350f;padding:0 10px;"
        "font-size:10px;font-weight:700;font-family:'Consolas',monospace;}QPushButton:hover{background:#d97706;color:#080808;}");
    connect(rg, &QPushButton::clicked, this, &ProfileScreen::show_regen_confirm);
    krl->addWidget(rg);
    kvl->addWidget(kr); vl->addWidget(kp);
    ...
    sec_login_hist_ = new QTableWidget; sec_login_hist_->setColumnCount(3);
    sec_login_hist_->setHorizontalHeaderLabels({"TIMESTAMP","IP ADDRESS","STATUS"});
    sec_login_hist_->verticalHeader()->setVisible(false); sec_login_hist_->setShowGrid(false);
    ...
    hvl->addWidget(sec_login_hist_); vl->addWidget(hp);
    vl->addStretch(); scroll->setWidget(page); return scroll;
}
...
QWidget* ProfileScreen::build_billing() {
    ...
    auto* hvl = qobject_cast<QVBoxLayout*>(hp->layout());
    bill_history_ = new QTableWidget; bill_history_->setColumnCount(5);
    bill_history_->setHorizontalHeaderLabels({"DATE","PLAN","AMOUNT","CREDITS","STATUS"});
    bill_history_->verticalHeader()->setVisible(false); bill_history_->setShowGrid(false);
    ...
    hvl->addWidget(bill_history_); vl->addWidget(hp);
    vl->addStretch(); scroll->setWidget(page); return scroll;
}
...
QWidget* ProfileScreen::build_support() {
    ...
    auto* cg = new QGridLayout; cg->setSpacing(12); cg->setContentsMargins(12,10,12,10);
    auto add_c = [&](const QString& l, const QString& e, int r, int c2) {
        ...
        lb->setStyleSheet(QString("color:#525252;font-size:10px;font-weight:700;background:transparent;letter-spacing:0.5px;%1").arg(MF));
        wl->addWidget(lb);
    ...
    cvl2->addLayout(cg); vl->addWidget(cp);
    auto* lp = make_panel("RESOURCES");
    auto* lvl = qobject_cast<QVBoxLayout*>(lp->layout());
    auto* lr = new QWidget; lr->setStyleSheet("background:transparent;");
    auto* lrl = new QHBoxLayout(lr); lrl->setContentsMargins(12,10,12,10); lrl->setSpacing(8);
    for (auto& s : {"DOCS","GITHUB","DISCORD","FAQ"}) {
        auto* b = new QPushButton(s); b->setFixedHeight(26);
        b->setStyleSheet("QPushButton{background:#111111;color:#0891b2;border:1px solid #1a1a1a;padding:0 12px;"
            "font-size:11px;font-family:'Consolas',monospace;}QPushButton:hover{color:#e5e5e5;background:#161616;}");
        lrl->addWidget(b);
    }
    lrl->addStretch();
    lvl->addWidget(lr); vl->addWidget(lp);
    vl->addStretch(); scroll->setWidget(page); return scroll;
}
...
void ProfileScreen::refresh_all() {
    ...
    ov_account_type_->setStyleSheet(QString("color:#d97706;font-size:13px;font-weight:700;background:transparent;%1").arg(MF));
    ...
    ov_verified_->setStyleSheet(QString("color:%1;font-size:13px;font-weight:700;background:transparent;%2").arg(s.user_info.is_verified?"#16a34a":"#dc2626").arg(MF));
    ov_mfa_->setText(s.user_info.mfa_enabled?"YES":"NO");
    ov_mfa_->setStyleSheet(QString("color:%1;font-size:13px;font-weight:700;background:transparent;%2").arg(s.user_info.mfa_enabled?"#16a34a":"#dc2626").arg(MF));
    ...
    sec_verified_->setStyleSheet(QString("color:%1;font-size:13px;font-weight:700;background:transparent;%2").arg(s.user_info.is_verified?"#16a34a":"#dc2626").arg(MF));
    sec_mfa_->setText(s.user_info.mfa_enabled?"ENABLED":"DISABLED");
    sec_mfa_->setStyleSheet(QString("color:%1;font-size:13px;font-weight:700;background:transparent;%2").arg(s.user_info.mfa_enabled?"#16a34a":"#808080").arg(MF));
...
void ProfileScreen::fetch_usage_data() {
    ...
    auth::UserApi::instance().get_user_usage(30, [this](auth::ApiResponse r) {
        ...
        if (data.contains("daily_usage")) {
            auto d = data["daily_usage"].toArray(); usg_daily_table_->setRowCount(0);
            for (int i=d.size()-1;i>=0&&i>=d.size()-10;i--) {
                auto e=d[i].toObject(); int row=usg_daily_table_->rowCount(); usg_daily_table_->insertRow(row);
                ...
                usg_daily_table_->setItem(row,2,new QTableWidgetItem(QString::number(e["credits_used"].toDouble(),'f',0)));
            }
        }
        if (data.contains("endpoint_breakdown")) {
            auto eps=data["endpoint_breakdown"].toArray(); usg_endpoint_table_->setRowCount(0);
            for (const auto& v:eps) {
                auto e=v.toObject(); int row=usg_endpoint_table_->rowCount(); usg_endpoint_table_->insertRow(row);
                ...
                usg_endpoint_table_->setItem(row,2,new QTableWidgetItem(QString::number(e["credits_used"].toDouble(),'f',0)));
                usg_endpoint_table_->setItem(row,3,new QTableWidgetItem(QString::number(e["avg_response_time_ms"].toDouble(),'f',0)));
            }
        }
    });
}
...
void ProfileScreen::fetch_billing_data() {
    ...
    auth::UserApi::instance().get_user_subscription([this](auth::ApiResponse r) {
        if (!r.success) { LOG_WARN("Profile", "Subscription fetch failed: " + r.error); return; }
    ...
    auth::UserApi::instance().get_payment_history(1,20,[this](auth::ApiResponse r) {
        if (!r.success) { LOG_WARN("Profile", "Payment history failed: " + r.error); return; }
        auto d=r.data.contains("data")?r.data["data"].toObject():r.data;
        auto p=d["payments"].toArray();
        if (p.isEmpty()) p=d["transactions"].toArray();
        if (p.isEmpty() && d.contains("data")) p=d["data"].toArray();
        bill_history_->setRowCount(0);
        for (const auto& v:p) {
            auto e=v.toObject(); int row=bill_history_->rowCount(); bill_history_->insertRow(row);
            ...
            bill_history_->setItem(row,2,new QTableWidgetItem(QString("$%1").arg(e["amount_usd"].toDouble(),0,'f',2)));
...
void ProfileScreen::fetch_login_history() {
    ...
    auth::UserApi::instance().get_login_history(20,0,[this](auth::ApiResponse r) {
        if (!r.success) { LOG_WARN("Profile", "Login history failed: " + r.error); return; }
        auto d=r.data.contains("data")?r.data["data"].toObject():r.data;
        auto h=d["login_history"].toArray(); if(h.isEmpty()) h=d["history"].toArray();
        sec_login_hist_->setRowCount(0);
        for (const auto& v:h) {
            auto e=v.toObject(); int row=sec_login_hist_->rowCount(); sec_login_hist_->insertRow(row);
...
void ProfileScreen::show_edit_profile_dialog() {
    auto* dlg = new QDialog(this); dlg->setWindowTitle("Edit Profile");
    dlg->setFixedSize(420,300);
    dlg->setStyleSheet("background:#0a0a0a;color:#e5e5e5;font-family:'Consolas',monospace;");
    auto* vl = new QVBoxLayout(dlg); vl->setContentsMargins(20,16,20,16); vl->setSpacing(10);
    ...
    auto add_f = [&](const QString& l, const QString& v) -> QLineEdit* {
        auto* lb = new QLabel(l); lb->setStyleSheet("color:#808080;font-size:11px;font-weight:700;background:transparent;");
        vl->addWidget(lb);
        auto* i = new QLineEdit(v); i->setFixedHeight(28); vl->addWidget(i); return i;
    };
    ...
    auto* br = new QWidget; br->setStyleSheet("background:transparent;");
    auto* brl = new QHBoxLayout(br); brl->setContentsMargins(0,0,0,0); brl->setSpacing(8); brl->addStretch();
    auto* cn = new QPushButton("CANCEL"); cn->setFixedHeight(26);
    connect(cn, &QPushButton::clicked, dlg, &QDialog::reject); brl->addWidget(cn);
    auto* sv = new QPushButton("SAVE"); sv->setFixedHeight(26);
    sv->setStyleSheet("QPushButton{background:#d97706;color:#080808;border:none;padding:0 16px;"
    ...
    connect(sv, &QPushButton::clicked, this, [=](){
        QJsonObject data;
        if(!un->text().trimmed().isEmpty()) data["username"]=un->text().trimmed();
        if(!ph->text().trimmed().isEmpty()) data["phone"]=ph->text().trimmed();
        if(!co->text().trimmed().isEmpty()) data["country"]=co->text().trimmed();
        auth::UserApi::instance().update_user_profile(data,[this,dlg](auth::ApiResponse r){
            if(r.success){auth::AuthManager::instance().refresh_user_data();dlg->accept();}
        });
    });
    brl->addWidget(sv); vl->addWidget(br);
...
void ProfileScreen::show_regen_confirm() {
    if(QMessageBox::warning(this,"Regenerate API Key","Your current API key will be invalidated. Continue?",
        ...
        auth::UserApi::instance().regenerate_api_key([this](auth::ApiResponse r){
            if(r.success){auth::AuthManager::instance().refresh_user_data();
                api_key_visible_=false;sec_api_key_->setText(QString(20,QChar(0x2022)));}
        });
    }
}
