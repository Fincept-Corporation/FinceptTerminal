...
GovDataCanadaPanel::GovDataCanadaPanel(QWidget* parent) : QWidget(parent) {
    ...
    connect(&ui::ThemeManager::instance(), &ui::ThemeManager::theme_changed, this,
            [this]() { setStyleSheet(build_canada_style()); });
}
...
void GovDataCanadaPanel::build_ui() {
    ...
    connect(publishers_table_, &QTableWidget::cellDoubleClicked, this, &GovDataCanadaPanel::on_publisher_clicked);
    ...
    connect(datasets_table_, &QTableWidget::cellDoubleClicked, this, &GovDataCanadaPanel::on_dataset_clicked);
...
QWidget* GovDataCanadaPanel::build_toolbar() {
    ...
    connect(search_input_, &QLineEdit::returnPressed, this, &GovDataCanadaPanel::on_search);
...
void GovDataCanadaPanel::load_initial_data() {
    ...
    services::GovDataService::instance().execute(kGovDataCanadaScript, "publishers", {}, "gov_orgs_canada_gov_api.py");
}

void GovDataCanadaPanel::on_result(const QString& request_id, const services::GovDataResult& result) {
    // Only handle requests belonging to this script
    if (!request_id.contains("canada_gov_api.py"))
        return;
...
void GovDataCanadaPanel::populate_resources(const QJsonArray& data) {
    ...
    for (int i = 0; i < data.size(); ++i) {
        ...
        if (!url.isEmpty())
            url_item->setForeground(QColor(kGovDataCanadaColor));
...
void GovDataCanadaPanel::on_publisher_clicked(int row) {
    ...
    show_loading("Loading datasets for "
                 ...
                 "…");
    services::GovDataService::instance().execute(kGovDataCanadaScript, "datasets", {selected_publisher_, "100"},
...
void GovDataCanadaPanel::on_dataset_clicked(int row) {
    ...
    services::GovDataService::instance().execute(kGovDataCanadaScript, "resources", {selected_dataset_},
...
void GovDataCanadaPanel::on_search() {
    ...
    show_loading("Searching for "
                 " + query + "
                 "…");
    services::GovDataService::instance().execute(kGovDataCanadaScript, "search", {query, "50"},
...
void GovDataCanadaPanel::show_loading(const QString& message) {
    status_label_->setStyleSheet(QString("color:%1; font-size:13px; background:transparent;").arg(kGovDataCanadaColor));
