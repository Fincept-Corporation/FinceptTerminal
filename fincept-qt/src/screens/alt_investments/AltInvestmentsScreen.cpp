...
{
    AltField f;
    f.key = key; f.label = label; f.type = AltField::Text;
    f.combo_items = def;
    return f;
}

static AltField spin_field(const QString& key, const QString& label,
                            double def, double min_v, double max_v,
                            int dec = 2, const QString& pfx = "",
                            const QString& sfx = "", bool div100 = false)
{
    AltField f;
    f.key = key; f.label = label; f.type = AltField::Spin;
    f.default_val = def; f.min_val = min_v; f.max_val = max_v;
    f.decimals = dec; f.prefix = pfx; f.suffix = sfx; f.divide_100 = div100;
    return f;
}

static AltField combo_field(const QString& key, const QString& label,
                             const QString& items)
{
    AltField f;
    f.key = key; f.label = label; f.type = AltField::Combo;
    f.combo_items = items;
    return f;
}
...
static QList<AltCategory> build_categories() {
    return {
        { "bonds", "Bonds & Fixed Income", "#3B82F6", "B",
          {
            { "high-yield",        "High-Yield Bonds",
              "Spread analysis, default probability, equity-like behavior" },
            { "em-bonds",          "Emerging Market Bonds",
              "Yield spread, sovereign default risk, currency risk" },
            { "convertible-bonds", "Convertible Bonds",
              "Conversion premium, bond floor, upside participation" },
            { "preferred-stocks",  "Preferred Stocks",
              "Current yield, call risk, dividend safety analysis" },
          }
        },
        { "real_estate", "Real Estate", "#8B5CF6", "RE",
          {
            { "real-estate", "Direct Property / REIT",
              "NOI, cap rate, DCF valuation, financing analysis" },
          }
        },
        { "hedge_funds", "Hedge Funds", "#EAB308", "HF",
          {
            { "hedge-funds",     "Long/Short Equity",
              "Strategy metrics, fee impact, liquidity risk" },
            { "managed-futures", "Managed Futures / CTA",
              "Fee drag, trend following, crisis alpha analysis" },
            { "market-neutral",  "Market Neutral",
              "Beta neutrality, factor exposure, leverage risk" },
          }
        },
        { "commodities", "Commodities", "#F97316", "C",
          {
            { "natural-resources", "Natural Resources",
              "Futures basis, contango/backwardation, roll yield" },
            { "pme",               "Precious Metals Equities",
              "Correlation to metals, inflation hedge, drawdowns" },
          }
        },
        { "private_capital", "Private Capital", "#EC4899", "PE",
          {
            { "private-capital", "Private Equity / Venture Capital",
              "IRR, MOIC, DPI, RVPI, J-curve, fee drag" },
          }
        },
        { "annuities", "Annuities", "#22C55E", "AN",
          {
            { "annuities",          "Fixed Annuity",
              "Payout analysis, breakeven years, inflation erosion" },
            { "variable-annuities", "Variable Annuity",
              "Total fee drag, tax deferral myth, vs. alternatives" },
            { "eia",                "Equity-Indexed Annuity",
              "Crediting rate, upside cap limitations, surrender charges" },
            { "inflation-annuity",  "Inflation-Indexed Annuity",
              "vs. fixed annuity, vs. TIPS ladder, longevity break-even" },
          }
        },
        { "structured", "Structured Products", "#06B6D4", "SP",
          {
            { "structured-products", "Structured Notes",
              "Complexity score, hidden costs, issuer credit risk" },
            { "leveraged-funds",     "Leveraged ETFs (2x/3x)",
              "Volatility decay, path-dependency, holding cost" },
          }
        },
        { "inflation", "Inflation Protected", "#FB923C", "IP",
          {
            { "tips",         "TIPS — Inflation-Linked Treasuries",
              "Real yield, inflation scenarios, tax efficiency" },
            { "ibonds",       "I-Bonds — Series I Savings Bonds",
              "Composite rate, early redemption penalty, vs. TIPS" },
            { "stable-value", "Stable Value Funds",
              "Market-to-book ratio, crediting rate, wrap contract risk" },
          }
        },
        { "strategies", "Strategies & Analysis", "#67E8F9", "ST",
          {
            { "covered-calls",  "Covered Call Strategy",
              "Tax consequences, opportunity cost, premium income" },
            { "sri",            "SRI / ESG Funds",
              "Performance vs. benchmark, screening costs, approaches" },
            { "asset-location", "Tax-Efficient Asset Location",
              "Optimal account placement, muni bonds, annual tax drag" },
          }
        },
        { "crypto", "Digital Assets", "#F87171", "DA",
          {
            { "digital-assets", "Cryptocurrency",
              "Fundamental metrics, NVT ratio, market cap analysis" },
          }
        },
    };
}
...
static QList<AltField> fields_for(const QString& id) {
    ...
    if (id == "convertible-bonds") return {
    ...
    if (id == "natural-resources") return {
    ...
    if (id == "variable-annuities") return {
    ...
    if (id == "inflation-annuity") return {
    ...
    if (id == "structured-products") return {
...
void AltInvestmentsScreen::showEvent(QShowEvent* e) { QWidget::showEvent(e); }
void AltInvestmentsScreen::hideEvent(QHideEvent* e) { QWidget::hideEvent(e); }
...
QWidget* AltInvestmentsScreen::create_center_panel() {
    ...
    connect(analyze_btn_, &QPushButton::clicked,
            this, &AltInvestmentsScreen::on_analyze);
    fhl->addWidget(fhtitle);
    fhl->addStretch(1);
    fhl->addWidget(analyze_btn_);
    fpvl->addWidget(fhdr);

    form_container_ = new QWidget;
...
QWidget* AltInvestmentsScreen::create_right_panel() {
    ...
    verdict_badge_->setStyleSheet(
        QString("color:%1; background:%2; font-size:11px; font-weight:700;"
    ...
    metrics_container_ = new QWidget;
...
void AltInvestmentsScreen::rebuild_form(int cat, int ana) {
    ...
    while (i < current_fields_.size()) {
        ...
        const bool pair = (f.type == AltField::Spin
                           && i + 1 < current_fields_.size()
        ...
        if (pair) {
            ...
            auto build_spin_col = [&](const AltField& field) -> QWidget* {
                ...
                if (!field.prefix.isEmpty()) sp->setPrefix(field.prefix);
                if (!field.suffix.isEmpty()) sp->setSuffix(field.suffix);
                cl->addWidget(sp);
            ...
            if (f.type == AltField::Text) {
                ...
                sp->setButtonSymbols(QAbstractSpinBox::NoButtons);
                if (!f.prefix.isEmpty()) sp->setPrefix(f.prefix);
                if (!f.suffix.isEmpty()) sp->setSuffix(f.suffix);
                cl->addWidget(sp);
                field_widgets_.append(sp);
            }
    ...
    for (int i = 0; i < current_fields_.size() && i < field_widgets_.size(); ++i) {
        ...
        if (f.type == AltField::Text) {
            ...
            if (auto* sp = qobject_cast<QDoubleSpinBox*>(w)) {
                double val = sp->value();
                if      (f.suffix == "B") val *= 1e9;
                else if (f.suffix == "M") val *= 1e6;
                else if (f.divide_100)    val /= 100.0;
                data[f.key] = val;
            }
        }
    }
...
void AltInvestmentsScreen::run_analysis(const QString& command,
                                         const QJsonObject& data)
{
    const QString data_json =
    ...
    if (!cached.isNull()) {
        ...
        if (doc.isObject()) { display_verdict(doc.object(), command); return; }
    }

    set_loading(true);
    verdict_badge_->setText("ANALYZING...");
    verdict_badge_->setStyleSheet(
        QString("color:%1; background:rgba(217,119,6,0.15);"
    ...
    python::PythonRunner::instance().run(
        ...
        [self, command, cache_key](const python::PythonResult& result) {
            ...
            if (!result.success) {
                self->display_error(
                    result.error.isEmpty() ? "Python process failed" : result.error);
                return;
            }
...
static QString stars_for(const QString& rating_str) {
    ...
    if (!match.hasMatch()) return rating_str;
    const int n     = match.captured(1).toInt();
    const int stars = qMax(1, qMin(5, (n + 1) / 2));
    return QString("\u2605").repeated(stars)
         + QString("\u2606").repeated(5 - stars)
         + "  " + rating_str;
}
...
{
    const auto metrics = result.contains("metrics")
                         ? result["metrics"].toObject()
                         : result;

    // ── Badge ─────────────────────────────────────────────────────────────────
    QString category = metrics.value("analysis_category")
                           .toString(metrics.value("category").toString());
    if (category.isEmpty())
        category = result.value("asset_type").toString(
                       result.value("product_type").toString(
                           result.value("fund_type").toString("RESULT")));

    QString badge_color, badge_bg;
    const QString lcat = category.toLower();
    if      (lcat.contains("good") || lcat.contains("ok"))
        { badge_color = colors::POSITIVE();  badge_bg = "rgba(22,163,74,0.15)";  }
    else if (lcat.contains("ugly") || lcat.contains("bad") || lcat.contains("avoid"))
        { badge_color = colors::NEGATIVE();  badge_bg = "rgba(220,38,38,0.15)";  }
    else if (lcat.contains("flaw") || lcat.contains("warn") || lcat.contains("caution"))
        { badge_color = "#FFD700";           badge_bg = "rgba(255,215,0,0.15)";  }
    else
        { badge_color = colors::CYAN();      badge_bg = "rgba(8,145,178,0.15)";  }
    ...
    // ── Rating ────────────────────────────────────────────────────────────────
    const QString rating = metrics.value("rating")
                               .toString(result.value("rating").toString());
    if (!rating.isEmpty())
        verdict_rating_->setText(stars_for(rating));

    // ── Recommendation ────────────────────────────────────────────────────────
    const QString rec =
        metrics.value("analysis_recommendation").toString(
            metrics.value("bottom_line").toString(
                metrics.value("analysis_summary").toString()));
    if (!rec.isEmpty())
    ...
    for (auto it = metrics.begin(); it != metrics.end(); ++it)
        append_metric_row(metrics_container_, metrics_layout_,
                          it.key(), it.value(), 0);
...
void AltInvestmentsScreen::append_metric_row(QWidget* /*parent*/,
                                              QVBoxLayout* vl,
                                              const QString& key,
                                              ...
                                              int depth)
{
    static const QSet<QString> kSkip = {
        ...
        "strategy", "analysis_category", "category", "rating",
        "analysis_recommendation", "bottom_line", "analysis_summary"
    };
    if (kSkip.contains(key)) return;
    ...
    if (val.isArray()) {
        ...
        for (const auto& elem : arr) {
            const QString text =
                "\xB7  " + (elem.isString() ? elem.toString() : format_value(elem));
            auto* row = new QWidget;
            ...
            if (lc.contains("warn") || lc.contains("risk") || lc.contains("bad")
                    || lc.contains("avoid") || lc.contains("toxic")
                    || lc.contains("never"))
                lbl->setObjectName("altMetricBad");
            else if (lc.contains("good") || lc.contains("accept")
                     || lc.contains("safe") || lc.contains("benefit"))
    ...
    if (val.isString()) {
        ...
        const bool is_annotation =
            key.startsWith("analysis_") || key == "note"
            || key == "interpretation"  || key == "situation"
            || key == "investor_risk"   || key == "reason";
        if (is_annotation) {
            auto* row = new QWidget;
            row->setObjectName("altMetricRow");
            auto* rl = new QHBoxLayout(row);
            ...
            lbl->setWordWrap(true);
            const QString lc = text.toLower();
            if (lc.contains("warn") || lc.contains("toxic") || lc.contains("avoid")
                    || lc.contains("bad") || lc.contains("never"))
                lbl->setObjectName("altMetricWarn");
            else if (lc.contains("good") || lc.contains("safe")
                     || lc.contains("acceptable"))
                lbl->setObjectName("altMetricGood");
            else
...
QString AltInvestmentsScreen::format_value(const QJsonValue& v) const {
    if (v.isBool())   return v.toBool() ? "Yes" : "No";
    if (v.isNull())   return "\u2014";
    ...
    if (v.isDouble()) {
        ...
        if (qAbs(d) >= 1e12) return QString::number(d / 1e12, 'f', 2) + "T";
        if (qAbs(d) >= 1e9)  return QString::number(d / 1e9,  'f', 2) + "B";
        if (qAbs(d) >= 1e6)  return QString::number(d / 1e6,  'f', 2) + "M";
    ...
    if (v.isArray())  return QString("[%1 items]").arg(v.toArray().size());
    return "[object]";
}
...
void AltInvestmentsScreen::display_error(const QString& error) {
    ...
    verdict_badge_->setStyleSheet(
        QString("color:%1; background:rgba(220,38,38,0.15);"
                " font-size:11px; font-weight:700; padding:4px 14px;")
            .arg(colors::NEGATIVE));
    verdict_rating_->clear();
