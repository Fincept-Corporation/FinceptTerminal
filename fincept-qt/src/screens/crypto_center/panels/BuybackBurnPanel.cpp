...
QString format_token_compact(const QString& raw, int decimals) {
    ...
    if (!ok)
        return QStringLiteral("—");
    const double ui = static_cast<double>(units) / std::pow(10.0, std::max(0, decimals));
    if (ui <= 0.0)
        return QStringLiteral("0");
    if (ui >= 1e9)
        return QStringLiteral("%1 B").arg(QLocale::system().toString(ui / 1e9, 'f', 2));
    if (ui >= 1e6)
        return QStringLiteral("%1 M").arg(QLocale::system().toString(ui / 1e6, 'f', 1));
    if (ui >= 1e3)
        return QStringLiteral("%1 K").arg(QLocale::system().toString(ui / 1e3, 'f', 1));
...
BuybackBurnPanel::BuybackBurnPanel(QWidget* parent) : QWidget(parent) {
    ...
    connect(&hub, &fincept::datahub::DataHub::topic_error, this, &BuybackBurnPanel::on_topic_error);
}
...
void BuybackBurnPanel::build_ui() {
    ...
    auto add_kv = [&](QVBoxLayout* parent, const QString& cap, QLabel*& cap_out, QLabel*& v_out,
        ...
        v_out->setObjectName(obj_name.isEmpty() ? QStringLiteral("buybackBurnValue") : obj_name);
    ...
    // THIS EPOCH section
    {
        ...
        add_kv(bl, tr("REVENUE"), revenue_caption_, revenue_total_, QStringLiteral("buybackBurnValueAmber"));
        ...
        connect(burn_signature_link_, &QPushButton::clicked, this, &BuybackBurnPanel::on_burn_signature_clicked);
    ...
    // ALL-TIME section
    {
        ...
        add_kv(bl, tr("BURNED"), total_burned_caption_, total_burned_, QStringLiteral("buybackBurnValueAmber"));
...
void BuybackBurnPanel::apply_theme() {
    ...
    const QString ss = QStringLiteral("QWidget#buybackBurnPanel { background:%1; }"
...
void BuybackBurnPanel::showEvent(QShowEvent* e) {
    QWidget::showEvent(e);
    auto& hub = fincept::datahub::DataHub::instance();
    hub.subscribe(this, QString::fromLatin1(kTopicEpoch), [this](const QVariant& v) { on_epoch_update(v); });
    hub.subscribe(this, QString::fromLatin1(kTopicBurnTotal), [this](const QVariant& v) { on_burn_total_update(v); });
...
void BuybackBurnPanel::retranslateUi() {
    if (title_)
        title_->setText(tr("BUYBACK & BURN"));
    ...
    if (burned_caption_)
        burned_caption_->setText(tr("$FNCPT BURNED"));
    if (burn_tx_caption_)
        burn_tx_caption_->setText(tr("BURN TX"));
    if (total_burned_caption_)
        total_burned_caption_->setText(tr("BURNED"));
    if (supply_caption_)
...
void BuybackBurnPanel::on_epoch_update(const QVariant& v) {
    if (!v.canConvert<fincept::wallet::BuybackEpoch>())
        return;
    ...
    auto pct_of_revenue = [&](double v) {
        return (e.revenue_total_usd > 0.0) ? (100.0 * v / e.revenue_total_usd) : 0.0;
    };
    auto fmt_pct = [&](double v) {
        return QStringLiteral("%1 (%2%)").arg(format_usd(v)).arg(QLocale::system().toString(pct_of_revenue(v), 'f', 0));
    };
...
void BuybackBurnPanel::on_burn_total_update(const QVariant& v) {
    if (!v.canConvert<fincept::wallet::BurnTotal>())
        return;
    ...
    supply_remaining_->setText(
        QStringLiteral("%1 $FNCPT").arg(format_token_compact(t.supply_remaining_raw, t.decimals)));
...
void BuybackBurnPanel::on_topic_error(const QString& topic, const QString& error) {
    if (topic != QLatin1String(kTopicEpoch) && topic != QLatin1String(kTopicBurnTotal)) {
        return;
    }
