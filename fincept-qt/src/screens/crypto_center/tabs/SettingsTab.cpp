...
SettingsTab::SettingsTab(QWidget* parent) : QWidget(parent) {
    ...
    connect(&svc, &fincept::wallet::WalletService::balance_mode_changed, this, &SettingsTab::on_mode_changed);
}
...
void SettingsTab::build_ui() {
    ...
    auto make_panel = [this](const QString& title_text, const QString& subtitle_text, QLabel*& title_out,
    ...
    {
        auto [panel, body] =
            make_panel(tr("BALANCE REFRESH"), tr("Mirrored on HOME"), balance_panel_title_, balance_panel_sub_);
        balance_hint_ = new QLabel(tr("POLL refreshes balances on a TTL via the configured RPC. "
    ...
    // ── Helius API key ────────────────────────────────────────────────────
    {
        auto [panel, body] = make_panel(tr("HELIUS API KEY"), tr("optional"), helius_panel_title_, helius_panel_sub_);
        helius_hint_ = new QLabel(tr("Paste a Helius API key for reliable account-subscribe streaming "
        ...
        connect(helius_save_button_, &QPushButton::clicked, this, &SettingsTab::on_save_helius_key);
        connect(helius_clear_button_, &QPushButton::clicked, this, &SettingsTab::on_clear_helius_key);
    ...
    {
        auto [panel, body] =
            make_panel(tr("ASSET FILTERS"), tr("affects holdings"), filters_panel_title_, filters_panel_sub_);
        filters_hint_ = new QLabel(tr("Pump.fun-launched wallets accumulate airdropped junk over time. "
        ...
        show_unverified_checkbox_ = new QCheckBox(tr("Show unverified tokens in the holdings panel"), panel);
        show_unverified_checkbox_->setObjectName(QStringLiteral("settingsTabCheckbox"));
        body->addWidget(show_unverified_checkbox_);

        connect(show_unverified_checkbox_, &QCheckBox::toggled, this, &SettingsTab::on_show_unverified_toggled);
...
void SettingsTab::apply_theme() {
    ...
    const QString ss =
        QStringLiteral("QWidget#settingsTab { background:%1; }"
...
void SettingsTab::load_initial_values() {
    apply_mode_to_buttons(fincept::wallet::WalletService::instance().balance_mode_is_stream());
    ...
    auto slip_res = SecureStorage::instance().retrieve(QString::fromLatin1(kSlippageKey));
...
void SettingsTab::on_show_unverified_toggled(bool checked) {
    SecureStorage::instance().store(QStringLiteral("wallet.show_unverified_tokens"),
                                    checked ? QStringLiteral("true") : QStringLiteral("false"));
    LOG_INFO("SettingsTab", QStringLiteral("show_unverified_tokens = %1").arg(checked ? "true" : "false"));
}
...
void SettingsTab::on_save_helius_key() {
    ...
    if (r.is_err()) {
        helius_status_->setText(tr("Failed: %1").arg(QString::fromStdString(r.error())));
        return;
    }
    helius_input_->clear();
...
void SettingsTab::retranslateUi() {
    ...
    if (balance_panel_title_)
        balance_panel_title_->setText(tr("BALANCE REFRESH"));
    if (balance_panel_sub_)
        balance_panel_sub_->setText(tr("Mirrored on HOME"));
    if (balance_hint_)
        balance_hint_->setText(tr("POLL refreshes balances on a TTL via the configured RPC. "
                                  "STREAM opens a WebSocket account subscription — requires Helius "
                                  "or a private RPC."));
    ...
    // Helius panel
    if (helius_panel_title_)
        helius_panel_title_->setText(tr("HELIUS API KEY"));
    if (helius_panel_sub_)
        helius_panel_sub_->setText(tr("optional"));
    if (helius_hint_)
        helius_hint_->setText(tr("Paste a Helius API key for reliable account-subscribe streaming "
                                 "and parsed transaction history. Stored in SecureStorage; never "
                                 "transmitted off-machine except in RPC requests to api.helius.xyz."));
    if (helius_input_)
        helius_input_->setPlaceholderText(tr("paste API key…"));
    if (helius_save_button_)
        helius_save_button_->setText(tr("SAVE"));
    if (helius_clear_button_)
        helius_clear_button_->setText(tr("CLEAR"));
    ...
    // Slippage panel
    if (slippage_panel_title_)
        slippage_panel_title_->setText(tr("DEFAULT SLIPPAGE"));
    ...
    if (filters_panel_title_)
        filters_panel_title_->setText(tr("ASSET FILTERS"));
    if (filters_panel_sub_)
        filters_panel_sub_->setText(tr("affects holdings"));
    if (filters_hint_)
        filters_hint_->setText(tr("Pump.fun-launched wallets accumulate airdropped junk over time. "
                                  "By default the holdings panel hides tokens that aren't in "
                                  "Jupiter's verified-tagged list. Toggle this on to see every "
                                  "SPL token account in the wallet."));
    if (show_unverified_checkbox_)
        show_unverified_checkbox_->setText(tr("Show unverified tokens in the holdings panel"));
}
