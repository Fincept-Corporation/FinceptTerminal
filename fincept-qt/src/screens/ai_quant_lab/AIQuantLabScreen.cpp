...
QWidget* AIQuantLabScreen::build_top_bar() {
    ...
    brand->setStyleSheet(QString(
        "color:#9D4EDD; font-size:%1px; font-weight:700; font-family:%2;"
    ...
    for (int i = 0; i < modules_.size(); ++i) {
        ...
        btn->setStyleSheet(QString(
            "QPushButton { color:%1; font-size:8px; font-family:%2;"
...
QWidget* AIQuantLabScreen::build_left_sidebar() {
    ...
    title->setStyleSheet(QString(
        "color:#9D4EDD; font-size:9px; font-weight:700; font-family:%1; letter-spacing:1px;")
...
QWidget* AIQuantLabScreen::build_right_sidebar() {
    ...
    right_title_->setStyleSheet(QString("font-size:11px; font-weight:700; font-family:%1;")
        .arg(ui::fonts::DATA_FAMILY));
    ...
    right_script_->setStyleSheet(QString(
        "color:%1; font-size:8px; font-family:%2; padding:6px;"
...
QWidget* AIQuantLabScreen::build_status_bar() {
    ...
    auto s = QString("color:%1; font-size:8px; font-family:%2;")
        .arg(ui::colors::TEXT_TERTIARY).arg(ui::fonts::DATA_FAMILY);
    auto* l1 = new QLabel("ENGINE:", bar); l1->setStyleSheet(s);
    ...
    for (int i = 0; i < module_buttons_.size(); ++i) {
        ...
        module_buttons_[i]->setStyleSheet(QString(
            ...
            .arg(active ? QString("rgba(%1,%2,%3,0.08)")
                .arg(mod.color.red()).arg(mod.color.green()).arg(mod.color.blue()) : "transparent")
            .arg(QString("%1,%2,%3").arg(mod.color.red()).arg(mod.color.green()).arg(mod.color.blue()))
            .arg(mod.color.name()));
    }
