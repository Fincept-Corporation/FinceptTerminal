...
void CellWidget::build_ui() {
    ...
    editor_->setStyleSheet(
        QString("QTextEdit { background:#0F0F0F; color:%1; border:none; border-left:3px solid %2;"
...
void CellWidget::set_outputs(const QVector<CellOutput>& outputs, int exec_count) {
    ...
    // Out label
    auto* out_label = new QLabel(
        QString("Out [%1]:").arg(exec_count), output_area_);
    out_label->setStyleSheet(
        QString("color:%1; font-family:%2; font-size:10px; font-weight:700;")
            .arg(kGreen, fonts::DATA_FAMILY));
    ...
    for (const auto& out : outputs) {
        if (out.type == "stream" || out.type == "execute_result") {
            ...
            text_lbl->setStyleSheet(
                QString("color:%1; font-family:%2; font-size:11px; padding:4px 0;")
            ...
            err_widget->setStyleSheet(
                QString("background:#1a0000; border-left:2px solid %1; padding:8px;").arg(kRed));
            auto* err_layout = new QVBoxLayout(err_widget);
            err_layout->setContentsMargins(8, 4, 8, 4);
            err_layout->setSpacing(2);

            auto* err_name = new QLabel(
                QString("<b>%1</b>: %2").arg(out.error_name, out.error_value), err_widget);
            err_name->setStyleSheet(
                QString("color:%1; font-family:%2; font-size:11px;")
                    .arg(kRed, fonts::DATA_FAMILY));
...
void CodeEditorScreen::build_ui() {
    setStyleSheet(
        QString("QWidget { background:%1; color:%2; }").arg("#000000", colors::TEXT_PRIMARY));
    ...
    status_bar->setStyleSheet(
        QString("background:#0F0F0F; border-top:1px solid %1;").arg(colors::BORDER_DIM));
...
QWidget* CodeEditorScreen::build_toolbar() {
    ...
    auto make_btn = [&](const QString& text) -> QPushButton* {
        ...
        btn->setStyleSheet(
            QString("QPushButton { background:#2A2A2A; color:%1; border:none;"
    ...
    py_label->setStyleSheet(
        QString("color:%1; font-family:%2; font-size:9px;").arg(kBlue, fonts::DATA_FAMILY));
...
void CodeEditorScreen::on_run_cell(const QString& cell_id) {
    ...
    python::PythonRunner::instance().run_code(code,
        [self, cid, exec_num](python::PythonResult result) {
            if (!self) return;
...
void CodeEditorScreen::on_save_notebook() {
    ...
    if (path.isEmpty()) {
        path = QFileDialog::getSaveFileName(
            this, "Save Notebook", "notebook.ipynb",
    ...
    for (const auto& cell : cells_) {
        ...
        if (cell.cell_type == "code") {
            co["execution_count"] = cell.execution_count > 0
                ? QJsonValue(cell.execution_count)
                : QJsonValue();
...
void CodeEditorScreen::update_status() {
    ...
    QString info = QString("%1 CODE  |  %2 MD  |  EXECUTED: %3")
