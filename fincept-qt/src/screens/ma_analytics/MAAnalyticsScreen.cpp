...
QWidget* MAAnalyticsScreen::build_top_bar() {
    ...
    subtitle->setStyleSheet(QString("color:%1; font-size:9px; font-family:%2; letter-spacing:1px;")
                                .arg(ui::colors::TEXT_TERTIARY)
                                .arg(ui::fonts::DATA_FAMILY));
...
QWidget* MAAnalyticsScreen::build_left_sidebar() {
    ...
    title->setStyleSheet(QString("color:#FF6B35; font-size:9px; font-weight:700; font-family:%1; letter-spacing:1px;")
                             .arg(ui::fonts::DATA_FAMILY));
    hhl->addWidget(title);
    hhl->addStretch();
    vl->addWidget(header);
    ...
    for (int i = 0; i < modules_.size(); ++i) {
        ...
        if (mod.category != last_cat) {
            ...
            cat_label->setStyleSheet(QString("color:%1; font-size:8px; font-weight:700; font-family:%2;"
...
QWidget* MAAnalyticsScreen::build_right_sidebar() {
    ...
    right_title_->setStyleSheet(
        QString("font-size:10px; font-weight:700; font-family:%1;").arg(ui::fonts::DATA_FAMILY));
    ...
    cap_header->setStyleSheet(
        QString("color:#FF6B35; font-size:9px; font-weight:700; font-family:%1; letter-spacing:1px;")
            .arg(ui::fonts::DATA_FAMILY));
    ...
    stats_card->setStyleSheet(QString("background:%1; border:1px solid %2; border-radius:2px; padding:10px;")
    ...
    stats_title->setStyleSheet(QString("color:%1; font-size:9px; font-weight:700; font-family:%2; letter-spacing:1px;")
                                   .arg(ui::colors::CYAN)
                                   .arg(ui::fonts::DATA_FAMILY));
    ...
    // Tips
    auto* tips = new QLabel("Click sidebar modules to switch views. "
                            "Each module provides professional-grade analytics with export capabilities.",
                            content);
    tips->setWordWrap(true);
    tips->setStyleSheet(QString("color:%1; font-size:8px; font-family:%2; line-height:1.5;"
...
void MAAnalyticsScreen::update_right_panel() {
    ...
    right_title_->setStyleSheet(QString("color:%1; font-size:10px; font-weight:700; font-family:%2;")
                                    .arg(mod.color.name())
                                    .arg(ui::fonts::DATA_FAMILY));
    ...
    if (card) {
        card->setStyleSheet(QString("background:rgba(%1,%2,%3,0.04); border:1px solid rgba(%1,%2,%3,0.18);"
    ...
    for (const auto& cap : module_capabilities(mod.id)) {
        ...
        row->setStyleSheet(QString("color:%1; font-size:9px; font-family:%2;")
                               .arg(ui::colors::TEXT_PRIMARY)
                               .arg(ui::fonts::DATA_FAMILY));
