...
static const char* kLabelStyle =
    "color: #808080; font-family: Consolas; font-size: 11px; font-weight: bold;";
} // namespace

namespace fincept::workflow {

QWidget* ParameterWidgetFactory::create(
    ...
    QWidget* parent)
{
    ...
    if (param.type == "string" || param.type == "expression") {
        ...
        edit->setStyleSheet(QString("QLineEdit { %1 } QLineEdit:focus { border: 1px solid #d97706; }").arg(kInputStyle));
        layout->addWidget(edit);

        QObject::connect(edit, &QLineEdit::textChanged, container,
            [key, on_change](const QString& text) {
        ...
        QObject::connect(spin, &QDoubleSpinBox::valueChanged, container,
            [key, on_change](double v) {
                on_change(key, QJsonValue(v));
            });
    }
        ...
        QObject::connect(check, &QCheckBox::toggled, container,
            [key, on_change](bool checked) {
                on_change(key, QJsonValue(checked));
            });
    }
        ...
        if (param.type == "json") {
            ...
            text_edit->setPlainText(doc.isEmpty()
        ...
        text_edit->setStyleSheet(
            QString("QPlainTextEdit { %1 }"
                    "QPlainTextEdit:focus { border: 1px solid #d97706; }").arg(kInputStyle));
        layout->addWidget(text_edit);

        QObject::connect(text_edit, &QPlainTextEdit::textChanged, container,
            [key, on_change, text_edit, param_type = param.type]() {
