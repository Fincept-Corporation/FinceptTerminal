...
static const QList<CbBank> kBanks = {
    {
        "BOE — Bank of England", "boe_data.py", "boe",
        {
            { "Bank Rate",                  "bank_rate"          },
    ...
    {
        "RBA — Reserve Bank of Australia", "rba_data.py", "rba",
        {
    ...
    {
        "BOC — Bank of Canada", "boc_data.py", "boc",
        {
    ...
    {
        "Riksbank — Sweden", "riksbank_data.py", "riksbank",
        {
    ...
    {
        "SNB — Swiss National Bank", "snb_data.py", "snb",
        {
    ...
    {
        "Norges Bank — Norway", "norges_bank_data.py", "norges",
        {
...
GlobalCentralBanksPanel::GlobalCentralBanksPanel(QWidget* parent)
    ...
    connect(&services::EconomicsService::instance(),
            ...
            this, &GlobalCentralBanksPanel::on_result);
}
...
void GlobalCentralBanksPanel::build_controls(QHBoxLayout* thl) {
    ...
    connect(bank_combo_, QOverload<int>::of(&QComboBox::currentIndexChanged),
            this, &GlobalCentralBanksPanel::update_series_for_bank);
...
void GlobalCentralBanksPanel::update_series_for_bank(int bank_idx) {
    if (bank_idx < 0 || bank_idx >= kBanks.size()) return;
    series_combo_->clear();
...
void GlobalCentralBanksPanel::on_fetch() {
    ...
    if (bi < 0 || bi >= kBanks.size()) return;
    if (si < 0 || si >= kBanks[bi].series.size()) return;
...
void GlobalCentralBanksPanel::on_result(const QString& request_id,
    ...
    for (int bi = 0; bi < kBanks.size(); ++bi) {
        const auto& bank = kBanks[bi];
        if (!request_id.startsWith(bank.req_prefix + "_")) continue;
        matched = true;
        matched_bank = bi;
    ...
    for (const auto& rv : raw) {
        ...
        for (const auto& key : r.keys()) {
            ...
            if (r[key].isDouble()) { has_value = true; break; }
        }
        if (has_value) rows.append(r);
    }
    ...
    const QString series_name = (matched_bank >= 0 && matched_series >= 0)
        ? kBanks[matched_bank].series[matched_series].label
        : request_id;
    const QString title = bank_name + ": " + series_name;

    display(rows, title);
    LOG_INFO("GlobalCentralBanksPanel",
