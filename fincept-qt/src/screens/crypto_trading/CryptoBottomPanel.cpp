...
void CryptoBottomPanel::set_positions(const QVector<trading::PtPosition>& positions) {
    ...
    if (positions_table_->rowCount() != n) positions_table_->setRowCount(n);
    ...
    for (int i = 0; i < n; ++i) {
        ...
        set(5, QString::number(pos.unrealized_pnl, 'f', 2),
            pos.unrealized_pnl >= 0 ? kColorPos : kColorNeg, Qt::AlignRight | Qt::AlignVCenter);
...
void CryptoBottomPanel::set_orders(const QVector<trading::PtOrder>& orders) {
    ...
    if (orders_table_->rowCount() != n) orders_table_->setRowCount(n);
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (existing) {
            ...
            connect(cancel_btn, &QPushButton::clicked, this, [this, cancel_btn]() {
                emit cancel_order_requested(cancel_btn->property("order_id").toString());
            });
...
void CryptoBottomPanel::set_trades(const QVector<trading::PtTrade>& trades) {
    ...
    if (trades_table_->rowCount() != n) trades_table_->setRowCount(n);
...
void CryptoBottomPanel::set_stats(const trading::PtStats& stats) {
    ...
    stat_values_[2]->setText(QString("%1 (W:%2 L:%3)")
...
void CryptoBottomPanel::set_mode(bool is_paper) { is_paper_ = is_paper; }
