...
QWidget* MarketPulsePanel::build_header() {
    ...
    bar->setStyleSheet(QString("background: %1; border-bottom: 1px solid rgba(217,119,6,0.25);")
    ...
    title->setStyleSheet(QString("color: %1; font-size: 10px; font-weight: bold; letter-spacing: 1px; background: transparent;")
        .arg(ui::colors::AMBER));
    hl->addWidget(title);
    hl->addStretch();
...
QWidget* MarketPulsePanel::build_section_header(const QString& title, const QString& icon_char, const QString& color) {
    ...
    lbl->setStyleSheet(QString("color: %1; font-size: 9px; font-weight: bold; letter-spacing: 0.5px; background: transparent;")
...
QWidget* MarketPulsePanel::build_fear_greed_section() {
    ...
    label->setStyleSheet(QString("color: %1; font-size: 9px; font-weight: bold; letter-spacing: 0.5px; background: transparent;")
        .arg(ui::colors::TEXT_SECONDARY));
    hrl->addWidget(label);
    hrl->addStretch();
    ...
    if (score <= 20) { sentiment_text = "EXTREME FEAR"; sentiment_color = ui::colors::NEGATIVE; }
    else if (score <= 40) { sentiment_text = "FEAR"; sentiment_color = "#FF6644"; }
    else if (score <= 60) { sentiment_text = "NEUTRAL"; sentiment_color = ui::colors::WARNING; }
    else if (score <= 80) { sentiment_text = "GREED"; sentiment_color = ui::colors::POSITIVE; }
    else { sentiment_text = "EXTREME GREED"; sentiment_color = ui::colors::POSITIVE; }
    ...
    score_val->setStyleSheet(QString("color: %1; font-size: 18px; font-weight: bold; background: transparent;").arg(sentiment_color));
    srl->addWidget(score_val);

    auto* score_max = new QLabel("/100");
    score_max->setStyleSheet(QString("color: %1; font-size: 9px; background: transparent;").arg(ui::colors::TEXT_TERTIARY));
    srl->addWidget(score_max);

    srl->addStretch();

    auto* sentiment = new QLabel(sentiment_text);
    sentiment->setStyleSheet(QString("color: %1; font-size: 9px; font-weight: bold; letter-spacing: 0.5px; background: transparent;").arg(sentiment_color));
...
QWidget* MarketPulsePanel::build_breadth_bar(const QString& label, int advancing, int declining) {
    ...
    name->setStyleSheet(QString("color: %1; font-size: 9px; font-weight: bold; background: transparent;").arg(ui::colors::TEXT_SECONDARY));
    tl->addWidget(name);
    tl->addStretch();
...
QWidget* MarketPulsePanel::build_mover_row(const QString& symbol, double change, const QString& volume) {
    ...
    sym->setStyleSheet(QString("color: %1; font-size: 10px; font-weight: bold; background: transparent;").arg(ui::colors::TEXT_PRIMARY));
    hl->addWidget(sym);
    hl->addStretch();
    ...
    if (!volume.isEmpty()) {
        ...
        vol->setStyleSheet(QString("color: %1; font-size: 8px; background: transparent;").arg(ui::colors::TEXT_TERTIARY));
        hl->addWidget(vol);
    }
...
QWidget* MarketPulsePanel::build_gainers_section() {
    ...
    struct Mover { const char* sym; double chg; const char* vol; };
    Mover data[] = {
        {"SMCI", 4.56, "2.1M"}, {"PLTR", 3.89, "45.3M"}, {"NVDA", 2.34, "38.7M"},
    };
    for (auto& m : data) {
...
QWidget* MarketPulsePanel::build_losers_section() {
    ...
    struct Mover { const char* sym; double chg; const char* vol; };
    Mover data[] = {
        {"RIVN", -5.67, "12.4M"}, {"COIN", -3.45, "8.9M"}, {"MARA", -4.12, "6.2M"},
    };
    for (auto& m : data) {
        vl->addWidget(build_mover_row(m.sym, m.chg, m.vol));
    }
...
// ── Global Snapshot ──
QWidget* MarketPulsePanel::build_stat_row(const QString& label, const QString& value, const QString& change, const QString& color) {
    auto* w = new QWidget;
    w->setStyleSheet(QString("border-bottom: 1px solid %1;").arg(ui::colors::BORDER_DIM));
    ...
    lbl->setStyleSheet(QString("color: %1; font-size: 9px; font-weight: bold; letter-spacing: 0.3px; background: transparent;")
        .arg(ui::colors::TEXT_SECONDARY));
    hl->addWidget(lbl);
    hl->addStretch();
    ...
    if (!change.isEmpty()) {
        ...
        chg->setStyleSheet(QString("color: %1; font-size: 8px; font-weight: bold; background: transparent;")
            .arg(positive ? ui::colors::POSITIVE : change.startsWith('-') ? ui::colors::NEGATIVE : ui::colors::TEXT_TERTIARY));
        hl->addWidget(chg);
    }
...
QWidget* MarketPulsePanel::build_market_hours_section() {
    ...
    struct Exchange { const char* name; const char* region; const char* color; };
    Exchange exchanges[] = {
    ...
    for (auto& ex : exchanges) {
        ...
        st->setStyleSheet(QString("color: %1; font-size: 8px; font-weight: bold; background: transparent;").arg(status_color));
        rl->addWidget(st);
