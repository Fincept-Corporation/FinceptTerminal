...
QuickTradeWidget::QuickTradeWidget(QWidget* parent)
    : BaseWidget("QUICK TRADE", parent, ui::colors::AMBER)
{
    ...
    lookup_btn_->setStyleSheet(QString(
    ...
    quote_card->setStyleSheet(QString("background: %1; border-radius: 2px;")
        .arg(ui::colors::BG_RAISED));
    ...
    change_label_->setStyleSheet(QString("color: %1; font-size: 10px; background: transparent;")
    ...
    bid_label_->setStyleSheet(QString("color: %1; font-size: 9px; background: transparent;")
        .arg(ui::colors::POSITIVE));
    bid_ask_row->addWidget(bid_label_);
    bid_ask_row->addSpacing(8);
    ask_label_ = new QLabel("ASK --");
    ask_label_->setStyleSheet(QString("color: %1; font-size: 9px; background: transparent;")
    ...
    qty_lbl->setStyleSheet(QString("color: %1; font-size: 9px; background: transparent;")
        .arg(ui::colors::TEXT_TERTIARY));
    ...
    price_lbl->setStyleSheet(QString("color: %1; font-size: 9px; background: transparent;")
        .arg(ui::colors::TEXT_TERTIARY));
    ...
    est_total_->setStyleSheet(QString("color: %1; font-size: 10px; background: transparent;")
    ...
    submit_btn_->setStyleSheet(QString(
    ...
    connect(side_combo_, QOverload<int>::of(&QComboBox::currentIndexChanged),
            this, &QuickTradeWidget::on_side_changed);
    connect(submit_btn_, &QPushButton::clicked, this, &QuickTradeWidget::submit_order);
    connect(order_type_, QOverload<int>::of(&QComboBox::currentIndexChanged),
            this, [this](int idx) {
...
void QuickTradeWidget::lookup_symbol() {
    ...
    if (sym.isEmpty()) { set_loading(false); return; }

    set_loading(true);
    services::MarketDataService::instance().fetch_quotes({sym},
        [this, sym](bool ok, QVector<services::QuoteData> quotes) {
            set_loading(false);
...
void QuickTradeWidget::on_side_changed(int idx) {
    ...
    submit_btn_->setText(idx == 0 ? "PLACE BUY ORDER"
                       : idx == 1 ? "PLACE SELL ORDER"
                       :            "PLACE SHORT ORDER");
    submit_btn_->setStyleSheet(QString(
        "QPushButton { background: %1; color: #ffffff; border: none; "
        "font-size: 11px; font-weight: bold; }"
...
void QuickTradeWidget::submit_order() {
    QString sym  = current_symbol_.isEmpty() ? symbol_input_->text().trimmed().toUpper()
                                              : current_symbol_;
    double  qty  = qty_input_->text().toDouble();
    ...
    QString price_str = type == "MARKET"
        ? QString("market price ($%1)").arg(current_price_, 0, 'f', 2)
        : QString("$%1").arg(price_input_->text());

    QMessageBox::information(this, "Order Submitted",
        QString("%1 %2 %3 @ %4\nOrder sent to trading engine.")
