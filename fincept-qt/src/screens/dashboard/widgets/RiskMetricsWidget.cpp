...
static const QStringList kRiskSymbols = {
    "^VIX", "SPY", "QQQ", "IWM", "TLT",
    "NVDA", "TSLA", "AMD", "META", "PLTR", "COIN"
};

RiskMetricsWidget::RiskMetricsWidget(QWidget* parent)
    : BaseWidget("RISK METRICS", parent, ui::colors::NEGATIVE)
{
    ...
    vix_bar_fill_->setStyleSheet(QString("color: %1; font-size: 8px; background: transparent;")
        .arg(ui::colors::TEXT_TERTIARY));
    ...
    for (const auto& sym : kVolatile) {
        ...
        sr.hi_lo = new QLabel("--");
        sr.hi_lo->setStyleSheet(QString("color: %1; font-size: 9px; background: transparent;")
            .arg(ui::colors::TEXT_TERTIARY));
    ...
    auto make_spread_row = [&](const QString& label, QLabel*& out) {
        ...
        lbl->setStyleSheet(QString("color: %1; font-size: 9px; background: transparent;")
...
void RiskMetricsWidget::populate(const QVector<services::QuoteData>& quotes) {
    ...
    for (const auto& q : quotes) map[q.symbol] = &q;
    ...
    if (map.contains("^VIX")) {
        ...
        vix_value_->setStyleSheet(QString("color: %1; font-size: 22px; font-weight: bold; background: transparent;").arg(color));
        vix_regime_->setText(regime);
        vix_regime_->setStyleSheet(QString("color: %1; font-size: 9px; font-weight: bold; background: transparent;").arg(color));
        ...
        vix_bar_fill_->setStyleSheet(QString(
            "color: %1; font-size: 8px; background: transparent; margin-left: %2px;")
            .arg(color).arg(margin_left));
    }
    ...
    for (int i = 0; i < kVolatile.size() && i < stock_rows_.size(); ++i) {
        ...
        // Hi/Lo as range: H/L
        if (q.high > 0 && q.low > 0) {
            stock_rows_[i].hi_lo->setText(
                QString("H%1 L%2").arg(q.high, 0, 'f', 0).arg(q.low, 0, 'f', 0));
        }
    }
    ...
    auto spread_color = [](double a, double b) -> QString {
        double diff = a - b;
        return diff > 0 ? ui::colors::POSITIVE
    ...
    if (map.contains("SPY") && map.contains("QQQ")) {
        ...
        spy_qqq_spread_->setStyleSheet(QString(
            "color: %1; font-size: 10px; font-weight: bold; background: transparent;")
    ...
    if (map.contains("SPY") && map.contains("IWM")) {
        ...
        spy_iwm_spread_->setStyleSheet(QString(
            "color: %1; font-size: 10px; font-weight: bold; background: transparent;")
    ...
    if (map.contains("SPY") && map.contains("TLT")) {
        ...
        equity_bond_lbl_->setStyleSheet(QString(
            "color: %1; font-size: 10px; font-weight: bold; background: transparent;")
