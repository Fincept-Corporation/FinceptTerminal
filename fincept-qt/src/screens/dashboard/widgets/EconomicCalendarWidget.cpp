...
void EconomicCalendarWidget::refresh_data() {
    ...
    fincept::HttpClient::instance().get(url,
        [this](fincept::Result<QJsonDocument> result) {
            set_loading(false);
            if (!result.is_ok()) {
...
void EconomicCalendarWidget::populate(const QJsonArray& events) {
    ...
    for (const auto& v : events) {
        ...
        if (event_name.isEmpty()) continue;
        ...
        if (!time_str.isEmpty()) date_display += " " + time_str.left(5);
        ...
        QString imp_text  = imp_int >= 3 ? "HIGH"
                          : imp_int == 2 ? "MED"
                          : imp_int == 1 ? "LOW" : "--";
        ...
        if (display_name.length() > 28) display_name = display_name.left(26) + "…";
        auto* ev_lbl = new QLabel(display_name);
        ev_lbl->setToolTip(event_name);  // full name on hover
        ev_lbl->setStyleSheet(QString("color: %1; font-size: 10px; background: transparent;")
            .arg(ui::colors::TEXT_PRIMARY));
        rl->addWidget(ev_lbl, 4);

        auto* cty_lbl = new QLabel(country);
        cty_lbl->setStyleSheet(QString("color: %1; font-size: 9px; background: transparent;")
            .arg(ui::colors::CYAN));
        rl->addWidget(cty_lbl, 1);

        auto* date_lbl = new QLabel(date_display);
        date_lbl->setStyleSheet(QString("color: %1; font-size: 9px; background: transparent;")
        ...
        fcst_lbl->setStyleSheet(QString("color: %1; font-size: 9px; background: transparent;")
            .arg(ui::colors::TEXT_SECONDARY));
        rl->addWidget(fcst_lbl, 1);

        auto* imp_lbl = new QLabel(imp_text);
        imp_lbl->setAlignment(Qt::AlignRight | Qt::AlignVCenter);
        imp_lbl->setStyleSheet(QString("color: %1; font-size: 9px; font-weight: bold; background: transparent;")
            .arg(imp_color));
