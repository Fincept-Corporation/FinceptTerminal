    ...
    : QFrame(parent), accent_color_(accent_color.isEmpty() ? ui::colors::AMBER : accent_color)
{
    setStyleSheet(QString(
        "BaseWidget { background: %1; border: 1px solid %2; border-radius: 2px; }")
    ...
    title_bar->setStyleSheet(QString("background: %1; border-bottom: 1px solid %2;")
    ...
    title_label_->setStyleSheet(QString(
        "color: %1; font-size: 9px; font-weight: bold; letter-spacing: 0.5px; "
        "background: transparent; text-transform: uppercase;").arg(accent_color_));
    hl->addWidget(title_label_);

    // Loading indicator
    loading_label_ = new QLabel("...");
    loading_label_->setStyleSheet(QString("color: %1; font-size: 9px; background: transparent;").arg(ui::colors::AMBER));
    ...
    refresh_btn_->setStyleSheet(QString(
        "QPushButton { color: %1; background: transparent; border: none; font-size: 11px; }"
        "QPushButton:hover { color: %2; }").arg(ui::colors::TEXT_TERTIARY, ui::colors::TEXT_PRIMARY));
    ...
    close_btn->setStyleSheet(QString(
        "QPushButton { color: %1; background: transparent; border: none; font-size: 10px; }"
        "QPushButton:hover { color: %2; }").arg(ui::colors::TEXT_TERTIARY, ui::colors::NEGATIVE));
...
void BaseWidget::set_error(const QString& error) {
    ...
    err_label->setStyleSheet(QString(
        "color: %1; font-size: 9px; padding: 16px; background: transparent;")
