...
void PortfolioSummaryWidget::fetch_prices(const QVector<Holding>& holdings) {
    QStringList symbols;
    for (const auto& h : holdings) symbols << h.symbol;

    services::MarketDataService::instance().fetch_quotes(symbols,
        [this, holdings](bool ok, QVector<services::QuoteData> quotes) {
            set_loading(false);
                                    ...
                                    const QVector<services::QuoteData>& quotes)
{
    ...
    for (const auto& q : quotes) qmap[q.symbol] = &q;
    ...
    for (const auto& h : holdings) {
        ...
        auto cell = [&](const QString& text, int stretch, Qt::Alignment align,
                        const QString& color) {
        ...
        cell(h.symbol,
             1, Qt::AlignLeft,  ui::colors::TEXT_PRIMARY);
        cell(QString::number(h.shares, 'f', h.shares == (int)h.shares ? 0 : 2),
             1, Qt::AlignRight, ui::colors::TEXT_SECONDARY);
        cell(price > 0 ? QString("$%1").arg(price, 0, 'f', 2) : "--",
             1, Qt::AlignRight, ui::colors::TEXT_PRIMARY);
        cell(value > 0 ? QString("$%1").arg(value, 0, 'f', 0) : "--",
             1, Qt::AlignRight, ui::colors::TEXT_PRIMARY);

        QString pnl_str = pnl >= 0 ? QString("+$%1").arg(pnl, 0, 'f', 0)
                                   : QString("-$%1").arg(-pnl, 0, 'f', 0);
        cell(pnl_str, 1, Qt::AlignRight,
    ...
    QString day_str = day_pnl >= 0 ? QString("+$%1").arg(day_pnl, 0, 'f', 0)
                                   : QString("-$%1").arg(-day_pnl, 0, 'f', 0);
    QString tot_str = total_pnl >= 0 ? QString("+$%1").arg(total_pnl, 0, 'f', 0)
                                     : QString("-$%1").arg(-total_pnl, 0, 'f', 0);
