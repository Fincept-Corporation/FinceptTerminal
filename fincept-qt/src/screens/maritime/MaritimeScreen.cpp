...
void MaritimeScreen::showEvent(QShowEvent* e) {
    ...
    if (first_show_) {
        ...
        // Load real vessel data from API on first show
        on_load_vessels();
    }
...
// ── Left Panel: Intelligence Stats + Trade Routes ────────────────────────────
QWidget* MaritimeScreen::build_left_panel() {
    auto* panel = new QWidget(this);
    panel->setFixedWidth(260);
    panel->setStyleSheet(QString("background:#000000; border-right:1px solid #1a1a1a;"));
    ...
    auto make_stat = [&](const QString& label, const QString& value, const QString& color) {
        auto* box = new QWidget(panel);
        box->setStyleSheet(QString("background:#0a0a0a; border:1px solid #1a1a1a; border-radius:2px;"));
...
QWidget* MaritimeScreen::build_center_panel() {
    ...
    // Splitter: map on top, table on bottom
    ...
    // Plot preset ports and trade routes on the map
    QVector<fincept::ui::MapPin> port_pins;
    for (const auto& port : preset_ports())
        port_pins.append({port.lat, port.lng, port.name, QColor("#00E5FF"), 6.0});
...
// ── Right Panel: Search + Route Detail ───────────────────────────────────────
QWidget* MaritimeScreen::build_right_panel() {
    auto* panel = new QWidget(this);
    panel->setFixedWidth(280);
    panel->setStyleSheet(QString("background:#000000; border-left:1px solid #1a1a1a;"));
    ...
    area_title->setStyleSheet(search_title->styleSheet());
    ...
    area_btn->setStyleSheet(track_btn->styleSheet());
    ...
    sys_title->setStyleSheet(label_style);
...
QWidget* MaritimeScreen::build_status_bar() {
    ...
    bar->setStyleSheet("background:#0a0a0a; border-top:1px solid #00ff00;");
...
void MaritimeScreen::on_vessel_history(QVector<VesselData> history) {
    if (history.isEmpty()) {
        status_label_->setText("NO HISTORY");
        return;
    }

    // Plot history trail on map — oldest (dim) to newest (bright)
    QVector<fincept::ui::MapPin> pins;
    ...
    for (int i = 0; i < total; ++i) {
        ...
        // Smaller dots for older positions, bigger for recent
    ...
    // Also update the vessels table with history data
    vessels_table_->setSortingEnabled(false);
    vessels_table_->setRowCount(history.size());
    for (int i = 0; i < history.size(); ++i) {
        const auto& v = history[i];
        auto* name_item = new QTableWidgetItem(v.name);
        name_item->setForeground(QColor("#FFC400"));
        vessels_table_->setItem(i, 0, name_item);
        vessels_table_->setItem(i, 1, new QTableWidgetItem(v.imo));
        auto* lat = new QTableWidgetItem(QString::number(v.latitude, 'f', 4));
        lat->setTextAlignment(Qt::AlignRight | Qt::AlignVCenter);
