...
std::vector<std::vector<std::string>> parse_csv_file(const QString& path, std::string& out_error) {
    ...
    if (rows.empty()) out_error = "No data rows found";
    return rows;
}
...
bool load_vol_surface(const std::vector<std::vector<std::string>>& rows, VolatilitySurfaceData& out, std::string& err) {
    if (rows.empty() || rows[0].size() < 3) { err = "Need columns: strike,dte,iv"; return false; }
    out = {};
    ...
    for (int dte : out.expirations) {
        ...
        for (float s : out.strikes) row.push_back(data_map[{dte, s}]);
        out.z.push_back(row);
    }
    return true;
}
...
bool load_greeks_surface(const std::vector<std::vector<std::string>>& rows, GreeksSurfaceData& out, std::string& err, const std::string& greek_name) {
    ...
    if (!load_vol_surface(rows, tmp, err)) return false;
...
bool load_skew_surface(const std::vector<std::vector<std::string>>& rows, SkewSurfaceData& out, std::string& err) {
    if (rows.empty() || rows[0].size() < 3) { err = "Need columns: delta,dte,skew"; return false; }
    out = {};
    ...
    for (int dte : out.expirations) {
        std::vector<float> row;
        for (float d : out.deltas) row.push_back(data_map[{dte, (int)d}]);
        out.z.push_back(row);
    }
    return true;
}
...
bool load_local_vol(const std::vector<std::vector<std::string>>& rows, LocalVolData& out, std::string& err) {
    VolatilitySurfaceData tmp;
    if (!load_vol_surface(rows, tmp, err)) return false;
    out.strikes = tmp.strikes; out.expirations = tmp.expirations; out.z = tmp.z;
    return true;
}

// ---- Generic 2D matrix loader: row_key, col_key, value ----
template<typename RowT, typename ColT>
static bool load_generic_matrix(
    const std::vector<std::vector<std::string>>& rows,
    std::vector<RowT>& out_rows, std::vector<ColT>& out_cols,
    std::vector<std::vector<float>>& out_z,
    std::string& err)
{
    if (rows.empty() || rows[0].size() < 3) { err = "Need 3 columns: row_key,col_key,value"; return false; }
    std::vector<RowT> row_keys;
    std::vector<ColT> col_keys;
    std::map<std::pair<RowT,ColT>, float> data_map;
    for (const auto& r : rows) {
        RowT rk; ColT ck;
        if constexpr (std::is_same_v<RowT, int>) rk = to_i(r[0]);
        else rk = r[0];
        if constexpr (std::is_same_v<ColT, int>) ck = to_i(r[1]);
        else ck = r[1];
        float v = to_f(r[2]);
        if (std::find(row_keys.begin(), row_keys.end(), rk) == row_keys.end()) row_keys.push_back(rk);
        if (std::find(col_keys.begin(), col_keys.end(), ck) == col_keys.end()) col_keys.push_back(ck);
        data_map[{rk,ck}] = v;
    }
    out_rows = row_keys; out_cols = col_keys;
    for (const auto& rk : out_rows) {
        std::vector<float> row;
        for (const auto& ck : out_cols) row.push_back(data_map[{rk,ck}]);
        out_z.push_back(row);
    }
    return true;
}
...
bool load_capfloor_vol(const std::vector<std::vector<std::string>>& rows, CapFloorVolData& out, std::string& err) {
    if (rows.empty() || rows[0].size() < 3) { err = "Need: maturity,strike,vol"; return false; }
    out = {};
    std::vector<int> mats; std::vector<float> strikes;
    std::map<std::pair<int,float>,float> dm;
    for (const auto& r : rows) {
        int m = to_i(r[0]); float s = to_f(r[1]); float v = to_f(r[2]);
    ...
    out.maturities=mats; out.strikes=strikes;
    for (int m : out.maturities) { std::vector<float> row; for (float s:out.strikes) row.push_back(dm[{m,s}]); out.z.push_back(row); }
    return true;
}
...
bool load_fx_vol(const std::vector<std::vector<std::string>>& rows, FXVolData& out, std::string& err) {
    if (rows.empty() || rows[0].size() < 3) { err = "Need: tenor,delta,vol"; return false; }
    out = {};
    std::vector<int> tenors; std::vector<float> deltas;
    std::map<std::pair<int,float>,float> dm;
    for (const auto& r : rows) {
        int t = to_i(r[0]); float d = to_f(r[1]); float v = to_f(r[2]);
    ...
    out.tenors=tenors; out.deltas=deltas;
    for (int t : out.tenors) { std::vector<float> row; for (float d:out.deltas) row.push_back(dm[{t,(int)d}]); out.z.push_back(row); }
    return true;
}
...
bool load_correlation(const std::vector<std::vector<std::string>>& rows, CorrelationMatrixData& out, std::string& err) {
    if (rows.empty() || rows[0].size() < 3) { err = "Need: asset_i,asset_j,corr"; return false; }
    out = {};
    std::vector<std::string> assets;
    std::map<std::pair<std::string,std::string>,float> dm;
    for (const auto& r : rows) {
        std::string a = r[0], b = r[1]; float v = to_f(r[2]);
    ...
    out.assets = assets;
    std::vector<float> row;
    for (const auto& a : assets) for (const auto& b : assets) row.push_back(dm[{a,b}]);
    out.z.push_back(row);
    return true;
}
...
bool load_var(const std::vector<std::vector<std::string>>& rows, VaRData& out, std::string& err) {
    if (rows.empty() || rows[0].size() < 3) { err = "Need: confidence,horizon,var"; return false; }
    out = {};
    std::vector<float> confs; std::vector<int> horizons;
    std::map<std::pair<float,int>,float> dm;
    for (const auto& r : rows) {
        float c = to_f(r[0]); int h = to_i(r[1]); float v = to_f(r[2]);
        if (std::find(confs.begin(),confs.end(),c)==confs.end()) confs.push_back(c);
        if (std::find(horizons.begin(),horizons.end(),h)==horizons.end()) horizons.push_back(h);
        dm[{c,h}]=v;
    }
    std::sort(confs.begin(),confs.end()); std::sort(horizons.begin(),horizons.end());
    out.confidence_levels=confs; out.horizons=horizons;
    for (float c:out.confidence_levels){std::vector<float> row;for(int h:out.horizons) row.push_back(dm[{c,h}]);out.z.push_back(row);}
    return true;
}
