...
void RiskManagementView::build_ui() {
    ...
    stress_title->setStyleSheet(QString("color:%1; font-size:12px; font-weight:700; letter-spacing:1px;")
                                .arg(ui::colors::AMBER));
    ...
    stress_table_->setStyleSheet(QString(
    ...
    contrib_title->setStyleSheet(QString("color:%1; font-size:12px; font-weight:700; letter-spacing:1px;")
                                 .arg(ui::colors::AMBER));
    contrib_layout->addWidget(contrib_title);

    contrib_table_ = new QTableWidget;
    contrib_table_->setColumnCount(6);
    contrib_table_->setHorizontalHeaderLabels({"SYMBOL", "WEIGHT", "VOL PROXY", "RISK CONTRIB", "VAR CONTRIB", "CONCENTRATION"});
    ...
    contrib_table_->setStyleSheet(QString(
        "QTableWidget { background:%1; color:%2; border:none; font-size:11px; }"
        "QTableWidget::item { padding:4px 8px; border-bottom:1px solid %3; }"
        "QHeaderView::section { background:%4; color:%5; border:none;"
        "  border-bottom:2px solid %6; padding:4px 8px; font-size:9px;"
...
void RiskManagementView::set_data(const portfolio::PortfolioSummary& summary,
...
void RiskManagementView::update_overview() {
    ...
    if (vol_count > 0) avg_vol /= vol_count;
    ...
    auto add_card = [&](int r, int c, const QString& label, const QString& value,
                        const char* color, const QString& sub = {}) {
    ...
    add_card(0, 0, "PORTFOLIO VALUE", QString("%1 %2").arg(currency_, fmt(total_mv)),
             ui::colors::WARNING, "Total market value");
    add_card(0, 1, "ANNUALIZED VOLATILITY", QString("%1%").arg(fmt(ann_vol, 1)),
             ui::colors::AMBER, "Based on day-change proxy");
    add_card(0, 2, "VALUE AT RISK (95%)", QString("%1 %2").arg(currency_, fmt(var95)),
             ui::colors::NEGATIVE, "1-day parametric");
    add_card(0, 3, "CONDITIONAL VaR", QString("%1 %2").arg(currency_, fmt(cvar95)),
             ui::colors::NEGATIVE, "Expected shortfall");
    ...
    add_card(1, 2, "TOP 5 CONCENTRATION", QString("%1%").arg(fmt(conc_top5, 1)),
             ui::colors::CYAN, "Sum of top 5");
...
void RiskManagementView::update_stress_test() {
    ...
    for (int r = 0; r < n; ++r) {
        ...
        set_cell(3, QString("%1%2%").arg(impact_pct < 0 ? "" : "+")
