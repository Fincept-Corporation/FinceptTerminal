...
void QuantStatsView::build_ui() {
    ...
    // ── RETURNS tab ───────────────────────────────────────────────────────────
    {
        auto* returns_w = new QWidget;
        returns_w->setStyleSheet(
        ...
        returns_stack_->addWidget(
            make_placeholder("Run QuantStats Analysis for return distribution"));
    ...
    // ── DRAWDOWN tab ──────────────────────────────────────────────────────────
    {
        ...
        drawdown_stack_->addWidget(
            make_placeholder("Run QuantStats Analysis for drawdown metrics"));
    ...
    // ── ROLLING tab ───────────────────────────────────────────────────────────
    {
        auto* roll_w = new QWidget;
        roll_w->setStyleSheet(
            QString("background:%1;").arg(ui::colors::BG_BASE));
        ...
        rolling_stack_->addWidget(
            make_placeholder("Run QuantStats Analysis for rolling metrics"));
    ...
    // ── MONTE CARLO tab ───────────────────────────────────────────────────────
    {
        ...
        auto* mc_desc = new QLabel(
            "Simulate 1,000 portfolio return paths using GBM to estimate probability\n"
            "distributions of future returns, drawdowns, and terminal wealth.");
        mc_desc->setWordWrap(true);
        mc_desc->setStyleSheet(
            QString("color:%1; font-size:11px;").arg(ui::colors::TEXT_TERTIARY));
        ...
        mc_stack_->addWidget(
            make_placeholder("Press RUN MONTE CARLO to simulate 1,000 return paths"));
...
void QuantStatsView::update_metrics() {
    ...
    auto add_section = [&](const QString& label) {
        rows.push_back({label, {}, {}, {}, false, true});
    };
    ...
    if (have_qs) {
        ...
        rows.push_back({"", "Total Return",       pct_str(total_ret), "--",
                        total_ret >= 0});
        rows.push_back({"", "Annualized Return",  pct_str(ann_ret),   "--",
                        ann_ret >= 0});
        rows.push_back({"", "Trading Days",
                        QString::number(t_days), "--", true});
        rows.push_back({"", "Best Day",    pct_str(best_day),   "--", true});
        rows.push_back({"", "Worst Day",   pct_str(worst_day),  "--",
                        worst_day >= 0});
        rows.push_back({"", "Avg Daily Return", pct_str(avg_daily, 4), "--",
                        avg_daily >= 0});
    } else {
        ...
        rows.push_back({"", "Unrealized P&L %",   pct_str(pnl_pct), "--",
                        pnl_pct >= 0});
        rows.push_back({"", "Day Change %",        pct_str(day_pct), "--",
                        day_pct >= 0});
        rows.push_back({"", "Total Positions",
                        QString::number(summary_.total_positions), "--", true});
        rows.push_back({"", "Gainers",
                        QString::number(summary_.gainers), "--", true});
        rows.push_back({"", "Losers",
                        QString::number(summary_.losers), "--",
                        summary_.losers == 0});
    }
    ...
    if (have_qs) {
        ...
        rows.push_back({"", "Annualized Volatility", pct_str(ann_vol), "--",
                        ann_vol < 20.0});
        rows.push_back({"", "Max Drawdown",    pct_str(max_dd),    "--",
                        max_dd >= 0});
        rows.push_back({"", "VaR 95% (Daily)", pct_str(var95),     "--",
                        var95 >= 0});
        rows.push_back({"", "CVaR 95% (Daily)",pct_str(cvar95),    "--",
                        cvar95 >= 0});
        rows.push_back({"", "Downside Deviation", pct_str(down_dev), "--",
                        down_dev < 15.0});
    } else {
    ...
    if (have_qs) {
        ...
        rows.push_back({"", "Loss Days", QString::number(los_days),  "--",
                        los_days == 0});
    ...
    for (int r = 0; r < rows.size(); ++r) {
        ...
        metrics_table_->setItem(r, 1,
            make_item(row.value.isEmpty() ? "--" : row.value,
...
void QuantStatsView::update_returns() {
    ...
    for (int i = 0; i < cards.size(); ++i) {
        ...
        val_lbl->setStyleSheet(
            QString("color:%1; font-size:18px; font-weight:700;").arg(c.color));
    ...
    struct TableRow { QString name; QString val; bool positive; };
...
void QuantStatsView::update_drawdown() {
    ...
    struct DdRow { QString name; QString val; bool positive; };
...
void QuantStatsView::update_rolling() {
    ...
    struct RatioRow { QString name; QString val; bool positive; };
    ...
    QVector<RatioRow> wrows = {
        {"Win Rate",      pct_str(win_rate),            win_rate >= 50.0},
        {"Win Days",      QString::number(win_days),    true},
        {"Loss Days",     QString::number(loss_days),   loss_days == 0},
        {"Avg Win/Day",   pct_str(avg_win),             true},
...
void QuantStatsView::update_monte_carlo_chart() {
    ...
    for (const auto& s : stats) {
        ...
        val->setStyleSheet(
            QString("color:%1; font-size:14px; font-weight:700;").arg(s.color));
        col_l->addWidget(val);
    ...
    for (int p = 0; p < n_paths; ++p) {
        ...
        for (int t = 0; t < pts; ++t) {
            series->append(static_cast<qreal>(t + 1),
                           path_data[t].toDouble());
        }
    ...
    chart_view->setStyleSheet(
        QString("background:%1; border:none;").arg(ui::colors::BG_BASE));
    ...
    auto* caption = new QLabel(
        QString("Showing %1 of 1000 simulated paths over 252 trading days (GBM)."
                " Bright line = median path.")
            .arg(n_shown));
    caption->setStyleSheet(
        QString("color:%1; font-size:9px; padding:4px 16px;")
            .arg(ui::colors::TEXT_TERTIARY));
...
void QuantStatsView::run_quantstats() {
    if (qs_running_ || summary_.holdings.isEmpty()) return;
    qs_running_ = true;
            ...
            QMetaObject::invokeMethod(
                ...
                [self, result]() {
                    ...
                    if (!result.success || result.output.trimmed().isEmpty()) {
                        ...
                        LOG_ERROR("QuantStats",
                                  "Script failed: " + result.error.left(200));
                        return;
                    }

                    const auto doc =
                        QJsonDocument::fromJson(
                            result.output.trimmed().toUtf8());
                    if (!doc.isObject()) {
                        ...
                        self->qs_status_->setStyleSheet(
                            QString("color:%1; font-size:9px;")
                                .arg(ui::colors::NEGATIVE));
                        LOG_ERROR("QuantStats",
                                  "Bad JSON: " + result.output.left(200));
                        return;
                    }

                    const QJsonObject root = doc.object();
                    if (root.contains("error")) {
                        self->qs_status_->setText(
...
void QuantStatsView::run_monte_carlo() {
    ...
    mc_status_->setStyleSheet(
        QString("color:%1; font-size:10px;").arg(ui::colors::AMBER));
    ...
    QPointer<QuantStatsView> self = this;
    PythonRunner::instance().run(
        "quantstats_monte_carlo", {args_str},
        [self](PythonResult result) {
            if (!self) return;
            QMetaObject::invokeMethod(
                self,
                [self, result]() {
                    ...
                    self->mc_status_->setText(
                        QString("Complete — %1 paths simulated")
                            .arg(root["num_paths_shown"].toInt() > 0
                                 ? "1000" : "0"));
                    self->mc_status_->setStyleSheet(
                        QString("color:%1; font-size:10px;")
                            .arg(ui::colors::POSITIVE));
