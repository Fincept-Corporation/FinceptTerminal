...
void EconomicsView::build_ui() {
    ...
    indicators_table_->setStyleSheet(
    ...
    auto* sens_note = new QLabel(
        "Estimated portfolio impact from macro factor shocks, weighted by holdings");
    sens_note->setStyleSheet(
        QString("color:%1; font-size:9px;").arg(ui::colors::TEXT_TERTIARY));
    ...
    sensitivity_table_->setHorizontalHeaderLabels(
        {"FACTOR SHOCK", "SENSITIVITY", "DIRECTION", "ESTIMATED IMPACT"});
...
void EconomicsView::set_data(const portfolio::PortfolioSummary& summary,
...
static QString sector_for(const QString& sym) {
    ...
    static const QHash<QString, QString> known = {
        ...
        {"DDOG","Technology"},{"CRWD","Technology"},{"PANW","Technology"},
        {"PLTR","Technology"},{"COIN","Technology"},{"SHOP","Technology"},
        {"UBER","Technology"},{"ABNB","Technology"},
        {"NFLX","Communication"},{"DIS","Communication"},{"CMCSA","Communication"},
        {"T","Communication"},{"VZ","Communication"},{"TMUS","Communication"},
        {"SNAP","Communication"},{"PINS","Communication"},{"SPOT","Communication"},
        {"TSLA","Consumer Cyclical"},{"HD","Consumer Cyclical"},{"LOW","Consumer Cyclical"},
        {"TGT","Consumer Cyclical"},{"BKNG","Consumer Cyclical"},{"MAR","Consumer Cyclical"},
        {"GM","Consumer Cyclical"},{"F","Consumer Cyclical"},{"NKE","Consumer Cyclical"},
        ...
        {"BLK","Financial Services"},{"SCHW","Financial Services"},{"COF","Financial Services"},
        {"BRK-B","Financial Services"},{"AXP","Financial Services"},
        ...
        {"CAT","Industrials"},{"LMT","Industrials"},{"RTX","Industrials"},{"FDX","Industrials"},
        {"PLD","Real Estate"},{"AMT","Real Estate"},{"EQIX","Real Estate"},{"PSA","Real Estate"},
        {"LIN","Basic Materials"},{"SHW","Basic Materials"},{"NEM","Basic Materials"},
        {"FCX","Basic Materials"},{"DOW","Basic Materials"},
        {"SPY","ETF/Index"},{"QQQ","ETF/Index"},{"IWM","ETF/Index"},{"VTI","ETF/Index"},
        {"VOO","ETF/Index"},{"GLD","ETF/Index"},{"TLT","ETF/Index"},{"AGG","ETF/Index"},
        {"BTC-USD","Cryptocurrency"},{"ETH-USD","Cryptocurrency"},{"SOL-USD","Cryptocurrency"},
        {"BNB-USD","Cryptocurrency"},{"XRP-USD","Cryptocurrency"},
    };
    auto it = known.find(s);
    if (it != known.end()) return *it;
    if (s.endsWith("-USD") || s.endsWith("-USDT")) return "Cryptocurrency";
...
void EconomicsView::update_indicators() {
    ...
    for (int r = 0; r < sorted.size(); ++r) {
        ...
        auto set = [&](int col, const QString& text,
            ...
            item->setTextAlignment(col == 0 || col == 1
        ...
        const char* pnl_pct_color =
            h.unrealized_pnl_percent >= 0 ? ui::colors::POSITIVE : ui::colors::NEGATIVE;

        set(0, h.symbol, ui::colors::CYAN);
        set(1, sector_for(h.symbol), ui::colors::TEXT_SECONDARY);
        set(2, QString("%1%").arg(QString::number(h.weight, 'f', 1)),
            ui::colors::TEXT_PRIMARY);
        set(3, QString("%1 %2")
                   .arg(currency_)
                   .arg(QString::number(h.cost_basis, 'f', 2)),
            ui::colors::TEXT_SECONDARY);
        set(4, QString("%1 %2")
                   .arg(currency_)
                   .arg(QString::number(h.market_value, 'f', 2)),
            ui::colors::WARNING);
        set(5, QString("%1%2 %3")
...
void EconomicsView::update_sensitivity() {
    ...
    QVector<Row> rows = {
        ...
        {"Consumer Spending (+1%)",   w_growth * 0.5,
         w_growth >= 0 ? "Positive" : "Negative"},
        {"Credit Spreads (+50bps)",   w_rate * 0.6,
         w_rate   >= 0 ? "Positive" : "Negative"},
        {"Unemployment (+1%)",        w_growth * -0.4,
         w_growth <= 0 ? "Positive" : "Negative"},
    };
    ...
    for (int r = 0; r < rows.size(); ++r) {
        ...
        const char* color =
            f.sensitivity >= 0 ? ui::colors::POSITIVE : ui::colors::NEGATIVE;
        ...
        set(1, QString("%1%2%")
                   .arg(f.sensitivity >= 0 ? "+" : "")
                   .arg(QString::number(f.sensitivity * 100.0, 'f', 1)),
            color);
