...
void EconomicsView::build_ui() {
    ...
    ind_title->setStyleSheet(QString("color:%1; font-size:11px; font-weight:700; letter-spacing:1px;")
                             .arg(ui::colors::AMBER));
    ...
    indicators_table_->setStyleSheet(QString(
    ...
    sens_title->setStyleSheet(QString("color:%1; font-size:11px; font-weight:700; letter-spacing:1px;")
                              .arg(ui::colors::AMBER));
...
void EconomicsView::update_sensitivity() {
    ...
    Factor factors[] = {
        {"Interest Rates (+1%)",  -0.12, "Negative"},
        {"GDP Growth (+1%)",       0.08, "Positive"},
        {"Inflation (+1%)",       -0.05, "Negative"},
        {"USD Strength (+1%)",    -0.03, "Negative"},
        {"Oil Price (+10%)",      -0.02, "Slightly Negative"},
        {"Unemployment (+1%)",    -0.06, "Negative"},
        {"Consumer Spending (+1%)", 0.04, "Positive"},
        {"Credit Spreads (+50bps)", -0.08, "Negative"},
    };
    ...
    for (int r = 0; r < n; ++r) {
        ...
        set(1, QString("%1%2%").arg(f.sensitivity >= 0 ? "+" : "")
                .arg(QString::number(f.sensitivity * 100.0, 'f', 1)), sens_color);
        set(2, f.direction, sens_color);
        set(3, QString("%1%2 %3").arg(impact >= 0 ? "+" : "")
                .arg(currency_).arg(QString::number(std::abs(impact), 'f', 0)), sens_color);
    }
}
