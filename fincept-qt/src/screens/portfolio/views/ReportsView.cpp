...
void ReportsView::build_ui() {
    ...
    txn_title->setStyleSheet(QString("color:%1; font-size:11px; font-weight:700; letter-spacing:1px;")
                             .arg(ui::colors::AMBER));
    ...
    txn_table_->setStyleSheet(QString(
    ...
    attr_title->setStyleSheet(QString("color:%1; font-size:11px; font-weight:700; letter-spacing:1px;")
                              .arg(ui::colors::AMBER));
    ...
    attr_table_->setStyleSheet(QString(
        "QTableWidget { background:%1; color:%2; border:none; font-size:11px; }"
        "QTableWidget::item { padding:4px 8px; border-bottom:1px solid %3; }"
        "QHeaderView::section { background:%4; color:%5; border:none;"
        "  border-bottom:2px solid %6; padding:4px 8px; font-size:9px;"
...
void ReportsView::set_data(const portfolio::PortfolioSummary& summary,
...
void ReportsView::update_summary() {
    ...
    add_card(0, 1, "TOTAL VALUE", QString("%1 %2").arg(currency_, fmt(summary_.total_market_value)),
             ui::colors::WARNING);
    add_card(0, 2, "COST BASIS", QString("%1 %2").arg(currency_, fmt(summary_.total_cost_basis)),
             ui::colors::CYAN);
    add_card(0, 3, "UNREALIZED P&L",
             QString("%1%2").arg(summary_.total_unrealized_pnl >= 0 ? "+" : "")
    ...
    add_card(1, 3, "RETURN",
             QString("%1%2%").arg(summary_.total_unrealized_pnl_percent >= 0 ? "+" : "")
    ...
    breakdown_title->setStyleSheet(QString("color:%1; font-size:10px; font-weight:700; letter-spacing:1px;")
    ...
    breakdown->setStyleSheet(QString(
        "QTableWidget { background:%1; color:%2; border:none; font-size:11px; }"
    ...
    for (int r = 0; r < sorted.size(); ++r) {
        ...
        set(4, QString("%1%2 (%3%4%)")
                .arg(h.unrealized_pnl >= 0 ? "+" : "").arg(fmt(h.unrealized_pnl))
                .arg(h.unrealized_pnl_percent >= 0 ? "+" : "").arg(fmt(h.unrealized_pnl_percent)), pc);
...
void ReportsView::update_attribution() {
    ...
    for (int r = 0; r < sorted.size(); ++r) {
        ...
        double contribution = (total_pnl != 0)
            ? (h.unrealized_pnl / std::abs(total_pnl)) * 100.0 : 0;
        ...
        set(2, QString("%1%2%").arg(h.unrealized_pnl_percent >= 0 ? "+" : "")
                .arg(QString::number(h.unrealized_pnl_percent, 'f', 2)), ret_color);
        set(3, QString("%1%2%").arg(contribution >= 0 ? "+" : "")
                .arg(QString::number(contribution, 'f', 1)), contrib_color);
