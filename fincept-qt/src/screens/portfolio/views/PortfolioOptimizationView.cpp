...
QWidget* PortfolioOptimizationView::build_optimize_tab() {
    ...
    method_cb_ = add_combo("METHOD:",
        {"Max Sharpe", "Min Volatility", "Risk Parity", "Max Return",
         "Equal Weight", "HRP", "Target Return"});
...
QWidget* PortfolioOptimizationView::build_strategies_tab() {
    ...
    strategies_stack_->addWidget(
        make_placeholder("Run optimization on the OPTIMIZE tab.\n"
    ...
    strategies_table_->setHorizontalHeaderLabels(
        {"STRATEGY", "EXP. RETURN", "VOLATILITY", "SHARPE", "DESCRIPTION"});
...
QWidget* PortfolioOptimizationView::build_compare_tab() {
    ...
    compare_stack_->addWidget(
        make_placeholder("Run optimization on the OPTIMIZE tab to populate this comparison."));
...
QWidget* PortfolioOptimizationView::build_backtest_tab() {
    ...
    layout->addWidget(make_placeholder(
        "Run an optimization first, then backtest the optimal weights\n"
...
QWidget* PortfolioOptimizationView::build_risk_tab() {
    ...
    layout->addWidget(make_placeholder(
        "Risk decomposition shows how each holding contributes to overall portfolio risk.\n"
...
QWidget* PortfolioOptimizationView::build_stress_tab() {
    ...
    layout->addWidget(make_placeholder(
        "Test how different optimization methods perform under stress conditions.\n"
...
void PortfolioOptimizationView::set_data(const portfolio::PortfolioSummary& summary,
            ...
            QMetaObject::invokeMethod(self, [self, result]() {
                ...
                if (!result.success || result.output.trimmed().isEmpty()) {
                    self->status_label_->setText(
                        "Optimization failed — check Python environment.");
                    self->status_label_->setStyleSheet(
                        QString("color:%1; font-size:10px;").arg(ui::colors::NEGATIVE));
                    LOG_ERROR("PortfolioOpt",
                              "Optimization failed: " + result.error.left(300));
                    return;
                }
...
void PortfolioOptimizationView::update_frontier(const QJsonArray& pts) {
    ...
    for (const auto& v : pts) {
        ...
        if (s > max_sharpe) { max_sharpe = s; opt_x = x; opt_y = y; }
...
void PortfolioOptimizationView::update_strategies(const QJsonObject& comparison) {
    ...
    static const struct { const char* key; const char* label; const char* desc; }
    kRows[] = {
    ...
    for (int r = 0; r < n; ++r) {
        ...
        auto set = [&](int col, const QString& text, const char* color = nullptr) {
            auto* item = new QTableWidgetItem(text);
            item->setTextAlignment(col == 0 || col == 4
                ? (Qt::AlignLeft | Qt::AlignVCenter)
                : (Qt::AlignRight | Qt::AlignVCenter));
            if (color) item->setForeground(QColor(color));
        ...
        set(1, obj.isEmpty() ? "--" : QString("%1%").arg(ret, 0, 'f', 1),
            ui::colors::POSITIVE);
        set(2, obj.isEmpty() ? "--" : QString("%1%").arg(vol, 0, 'f', 1),
            ui::colors::WARNING);
        set(3, obj.isEmpty() ? "--" : QString::number(sharpe, 'f', 2),
            ui::colors::CYAN);
...
void PortfolioOptimizationView::update_compare(const QJsonObject& comparison) {
    if (comparison.isEmpty() || summary_.holdings.isEmpty()) return;

    static const struct { const char* key; const char* label; }
    kCols[] = {
    ...
    QStringList headers;
    headers << "SYMBOL";
    for (int c = 0; c < n_methods; ++c) headers << kCols[c].label;
    ...
    for (int r = 0; r < static_cast<int>(summary_.holdings.size()); ++r) {
        ...
        for (int c = 0; c < n_methods; ++c) {
            ...
            const double w = obj.isEmpty()
                ? 0.0
                : obj["weights"].toObject().value(sym).toDouble() * 100.0;
