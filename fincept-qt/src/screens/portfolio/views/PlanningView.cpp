...
QWidget* PlanningView::build_retirement_tab() {
    ...
    input_panel->setStyleSheet(QString("background:%1; border:1px solid %2;")
    ...
    input_title->setStyleSheet(QString("color:%1; font-size:11px; font-weight:700; letter-spacing:1px;")
                               .arg(ui::colors::AMBER));
    ...
    current_age_ = new QDoubleSpinBox; current_age_->setRange(18, 100); current_age_->setValue(30);
    current_age_->setDecimals(0); style_spin(current_age_);
    auto* l1 = new QLabel("Current Age:"); l1->setStyleSheet(label_style);
    form->addRow(l1, current_age_);

    retire_age_ = new QDoubleSpinBox; retire_age_->setRange(30, 100); retire_age_->setValue(65);
    retire_age_->setDecimals(0); style_spin(retire_age_);
    auto* l2 = new QLabel("Retire Age:"); l2->setStyleSheet(label_style);
    form->addRow(l2, retire_age_);

    annual_expense_ = new QDoubleSpinBox; annual_expense_->setRange(0, 10000000); annual_expense_->setValue(60000);
    annual_expense_->setPrefix("$ "); annual_expense_->setDecimals(0); style_spin(annual_expense_);
    auto* l3 = new QLabel("Annual Expense:"); l3->setStyleSheet(label_style);
    form->addRow(l3, annual_expense_);

    monthly_contrib_ = new QDoubleSpinBox; monthly_contrib_->setRange(0, 1000000); monthly_contrib_->setValue(2000);
    monthly_contrib_->setPrefix("$ "); monthly_contrib_->setDecimals(0); style_spin(monthly_contrib_);
    auto* l4 = new QLabel("Monthly Savings:"); l4->setStyleSheet(label_style);
    form->addRow(l4, monthly_contrib_);

    expected_return_ = new QDoubleSpinBox; expected_return_->setRange(0, 30); expected_return_->setValue(8);
    expected_return_->setSuffix(" %"); expected_return_->setDecimals(1); style_spin(expected_return_);
    auto* l5 = new QLabel("Exp. Return:"); l5->setStyleSheet(label_style);
    form->addRow(l5, expected_return_);

    inflation_ = new QDoubleSpinBox; inflation_->setRange(0, 20); inflation_->setValue(3);
    inflation_->setSuffix(" %"); inflation_->setDecimals(1); style_spin(inflation_);
    auto* l6 = new QLabel("Inflation:"); l6->setStyleSheet(label_style);
    ...
    calc_btn->setStyleSheet(QString(
        "QPushButton { background:%1; color:#000; border:none; font-size:10px; font-weight:700; }"
    ...
    res_title->setStyleSheet(QString("color:%1; font-size:12px; font-weight:700; letter-spacing:1px;")
                             .arg(ui::colors::AMBER));
    ...
    status_label_->setStyleSheet(QString("color:%1; font-size:12px; padding:12px;")
...
void PlanningView::set_data(const portfolio::PortfolioSummary& summary,
...
void PlanningView::recalculate() {
    ...
    gap_label_->setText(QString("%1%2 %3")
        .arg(gap >= 0 ? "+" : "").arg(currency_).arg(QString::number(std::abs(gap), 'f', 0)));
    ...
    if (gap >= 0) {
        status_label_->setText(QString("\u2713 On track! Your projected retirement fund of %1 %2 "
            ...
            .arg(currency_).arg(QString::number(projected, 'f', 0))
        ...
        status_label_->setText(QString("\u26A0 Shortfall of %1 %2. Consider increasing monthly savings "
            ...
            .arg(currency_).arg(QString::number(std::abs(gap), 'f', 0))
