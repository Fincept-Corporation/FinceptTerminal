...
CreatePortfolioDialog::CreatePortfolioDialog(QWidget* parent) : QDialog(parent) {
    ...
    setStyleSheet(QString(
        ...
        .arg(ui::colors::BG_SURFACE, ui::colors::TEXT_PRIMARY,
             ...
             ui::colors::BORDER_MED, ui::colors::AMBER, ui::colors::AMBER_DIM));
    ...
    QStringList currencies = {
        ...
        "MXN", "KRW", "TWD", "THB", "ZAR", "TRY", "RUB", "PLN",
        "CZK", "HUF", "ILS", "AED", "SAR", "IDR", "MYR", "PHP",
        "VND", "NGN", "EGP", "BDT",
    };
...
QString CreatePortfolioDialog::name() const { return name_edit_->text().trimmed(); }
QString CreatePortfolioDialog::owner() const { return owner_edit_->text().trimmed(); }
QString CreatePortfolioDialog::currency() const { return currency_cb_->currentText(); }
...
ConfirmDeleteDialog::ConfirmDeleteDialog(const QString& portfolio_name, QWidget* parent)
    ...
    icon_label->setStyleSheet(QString("color:%1; font-size:13px; font-weight:700;")
...
QString AddAssetDialog::symbol() const { return symbol_edit_->text().trimmed().toUpper(); }
double  AddAssetDialog::quantity() const { return quantity_edit_->text().toDouble(); }
double  AddAssetDialog::price() const { return price_edit_->text().toDouble(); }
...
SellAssetDialog::SellAssetDialog(const QString& symbol, double held_qty, QWidget* parent)
    ...
    sell_btn->setStyleSheet(QString(
        "QPushButton { background:%1; color:#fff; border:none;"
        "  font-size:10px; font-weight:700; }"
        "QPushButton:hover { background:#ef4444; }")
        .arg(ui::colors::NEGATIVE));
...
double SellAssetDialog::quantity() const { return quantity_edit_->text().toDouble(); }
double SellAssetDialog::price() const { return price_edit_->text().toDouble(); }

// ── ImportPortfolioDialog ────────────────────────────────────────────────────

ImportPortfolioDialog::ImportPortfolioDialog(const QVector<portfolio::Portfolio>& portfolios,
                                               QWidget* parent)
    : QDialog(parent) {
    ...
    browse_btn->setStyleSheet(QString(
        "QPushButton { background:%1; color:#000; border:none;"
        "  font-size:10px; font-weight:700; }"
        "QPushButton:hover { background:%2; }")
    ...
    mode_label->setStyleSheet(QString("color:%1; font-size:9px; font-weight:700; letter-spacing:0.5px;")
...
void ImportPortfolioDialog::browse_file() {
    QString path = QFileDialog::getOpenFileName(this, "Select Portfolio JSON",
...
QString ImportPortfolioDialog::file_path() const { return file_edit_->text(); }
...
double  EditTransactionDialog::quantity() const { return quantity_edit_->text().toDouble(); }
double  EditTransactionDialog::price() const    { return price_edit_->text().toDouble(); }
QString EditTransactionDialog::date() const     { return date_edit_->text().trimmed(); }
QString EditTransactionDialog::notes() const    { return notes_edit_->text().trimmed(); }
...
static const QStringList kSectors = {
    ...
    "Communication", "Real Estate", "Utilities", "Basic Materials",
    "Cryptocurrency", "US Equity", "Bonds", "Commodities", "Other",
};

SectorMappingDialog::SectorMappingDialog(const QVector<portfolio::HoldingWithQuote>& holdings,
                                           QWidget* parent)
    : QDialog(parent) {
