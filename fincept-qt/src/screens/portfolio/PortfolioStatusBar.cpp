...
PortfolioStatusBar::PortfolioStatusBar(QWidget* parent) : QWidget(parent) {
    ...
    setStyleSheet(QString(
        "#portfolioStatusBar { background:%1; border-top:1px solid %2; }")
...
void PortfolioStatusBar::set_summary(const portfolio::PortfolioSummary& s) {
    ...
    nav_label_->setText(QString("NAV %1 %2")
        .arg(s.portfolio.currency, fmt(s.total_market_value)));

    double pnl = s.total_unrealized_pnl;
    pnl_label_->setText(QString("P&L %1%2")
        .arg(pnl >= 0 ? "+" : "").arg(fmt(pnl)));
