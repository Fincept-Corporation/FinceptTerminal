...
void PortfolioFFNView::build_ui() {
    ...
    title->setStyleSheet(QString("color:%1; font-size:11px; font-weight:700; letter-spacing:1px;")
                         .arg(ui::colors::AMBER));
    ...
    ov_title->setStyleSheet(QString("color:%1; font-size:11px; font-weight:700; letter-spacing:1px;")
                            .arg(ui::colors::AMBER));
    ...
    overview_table_->setStyleSheet(QString(
...
void PortfolioFFNView::update_overview() {
    ...
    QVector<Metric> metrics = {
        ...
        {"Win Rate",              summary_.gainers > 0
             ? summary_.gainers * 100.0 / summary_.total_positions : 0, 55.0},
        {"Best Holding",          0, 0},
        {"Worst Holding",         0, 0},
    ...
    if (!summary_.holdings.isEmpty()) {
        auto best = std::max_element(summary_.holdings.begin(), summary_.holdings.end(),
            [](const auto& a, const auto& b) { return a.unrealized_pnl_percent < b.unrealized_pnl_percent; });
        auto worst = std::min_element(summary_.holdings.begin(), summary_.holdings.end(),
            [](const auto& a, const auto& b) { return a.unrealized_pnl_percent < b.unrealized_pnl_percent; });
    ...
    for (int r = 0; r < metrics.size(); ++r) {
        ...
        const char* val_color = is_pct
            ? (m.portfolio_val >= 0 ? ui::colors::POSITIVE : ui::colors::NEGATIVE)
            : ui::colors::CYAN;

        if (is_currency)
            set(1, QString("%1 %2").arg(currency_, QString::number(m.portfolio_val, 'f', 2)), ui::colors::WARNING);
        else if (is_pct)
            set(1, QString("%1%2%").arg(m.portfolio_val >= 0 ? "+" : "")
                    .arg(QString::number(m.portfolio_val, 'f', 2)), val_color);
        else
        ...
        if (m.benchmark_val != 0) {
            if (is_pct)
                set(2, QString("%1%").arg(QString::number(m.benchmark_val, 'f', 2)),
