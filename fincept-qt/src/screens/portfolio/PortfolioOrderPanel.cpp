...
void PortfolioOrderPanel::build_ui() {
    setStyleSheet(QString("background:%1; border-left:2px solid %2;")
    ...
    close_btn_->setStyleSheet(QString(
    ...
    auto update_tabs = [this]() {
        ...
        buy_tab_->setStyleSheet(QString(
            "QPushButton { background:%1; color:%2; border:none;"
            "  font-size:10px; font-weight:700; }")
            .arg(is_buy ? ui::colors::POSITIVE : ui::colors::BG_RAISED,
                 is_buy ? "#000" : inactive_color));
        sell_tab_->setStyleSheet(QString(
            "QPushButton { background:%1; color:%2; border:none;"
            "  font-size:10px; font-weight:700; }")
            .arg(!is_buy ? ui::colors::NEGATIVE : ui::colors::BG_RAISED,
                 !is_buy ? "#fff" : inactive_color));

        // Update header and border color
        setStyleSheet(QString("background:%1; border-left:2px solid %2;")
                      .arg(ui::colors::BG_SURFACE, active_color));
    ...
    connect(buy_tab_, &QPushButton::clicked, this, [this, update_tabs]() {
        side_ = "BUY"; update_tabs(); });
    connect(sell_tab_, &QPushButton::clicked, this, [this, update_tabs]() {
        side_ = "SELL"; update_tabs(); });

    layout->addLayout(side_row);

    // Selected holding info
    symbol_label_ = new QLabel;
    symbol_label_->setStyleSheet(QString("color:%1; font-size:16px; font-weight:700;")
                                 .arg(ui::colors::CYAN));
    ...
    connect(submit_btn_, &QPushButton::clicked, this, [this]() {
        if (side_ == "BUY") emit buy_submitted();
        else emit sell_submitted();
    });
...
void PortfolioOrderPanel::update_display() {
    ...
    qty_label_->setText(QString::number(holding_->quantity, 'f',
        holding_->quantity == std::floor(holding_->quantity) ? 0 : 2));
    mv_label_->setText(QString("%1 %2")
        .arg(currency_)
        .arg(QString::number(holding_->market_value, 'f', 2)));
