...
PortfolioStatsRibbon::PortfolioStatsRibbon(QWidget* parent) : QWidget(parent) {
    ...
    setStyleSheet(QString(
        "#portfolioStatsRibbon { background:%1; border-bottom:1px solid %2; }")
...
PortfolioStatsRibbon::MetricCell PortfolioStatsRibbon::add_cell(
    const QString& label_text, const char* value_color) {
    ...
    cell.label->setStyleSheet(QString("color:%1; font-size:8px; font-weight:600; letter-spacing:0.5px;")
...
void PortfolioStatsRibbon::set_metrics(const portfolio::ComputedMetrics& m) {
    ...
    concentration_.value->setText(
        m.concentration_top3.has_value()
    ...
    volatility_.value->setText(
        m.volatility.has_value()
        ? QString("%1%").arg(QString::number(*m.volatility, 'f', 1))
        : "--");
    volatility_.sub->setText("30D Ann.");

    max_drawdown_.value->setText(
        m.max_drawdown.has_value()
    ...
    if (m.risk_score.has_value()) {
        ...
        risk_score_.value->setStyleSheet(
            QString("color:%1; font-size:13px; font-weight:700;").arg(rs_color));
    } else {
        risk_score_.value->setText("--");
    }

    var95_.value->setText(
        m.var_95.has_value()
        ? QString::number(*m.var_95, 'f', 2)
        : "--");
