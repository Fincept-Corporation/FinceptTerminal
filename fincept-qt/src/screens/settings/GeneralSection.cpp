...
void GeneralSection::build_ui() {
    ...
    auto* on_close_row = make_row(tr("On last window close"), on_close_combo_,
    ...
    connect(on_close_combo_, qOverload<int>(&QComboBox::currentIndexChanged), this, [this](int idx) {
        const QString val = on_close_combo_->itemData(idx).toString();
        SettingsRepository::instance().set(QString::fromLatin1(kKeyOnClose), val, QStringLiteral("general"));
    ...
    auto* language_row = make_row(tr("Interface language"), language_combo_,
    ...
    connect(language_combo_, qOverload<int>(&QComboBox::currentIndexChanged), this, [this](int idx) {
    ...
    auto* currency_row = make_row(tr("Display currency"), currency_combo_,
    ...
    connect(currency_combo_, qOverload<int>(&QComboBox::currentIndexChanged), this, [this](int idx) {
...
void GeneralSection::reload() {
    if (on_close_combo_) {
        const auto r =
            SettingsRepository::instance().get(QString::fromLatin1(kKeyOnClose), QString::fromLatin1(kDefaultOnClose));
...
void GeneralSection::retranslateUi() {
    // Section titles.
    if (window_title_)
        window_title_->setText(tr("WINDOW BEHAVIOUR"));
    if (lang_title_)
        lang_title_->setText(tr("LANGUAGE"));
    if (currency_title_)
        currency_title_->setText(tr("CURRENCY"));
