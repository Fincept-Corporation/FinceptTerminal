...
QString LlmConfigSection::default_base_url(const QString& provider) {
    ...
    if (p == "openai")      return {};  // uses default
...
QStringList LlmConfigSection::fallback_models(const QString& provider) {
    const QString p = provider.toLower();
    if (p == "openai")      return {"gpt-4o", "gpt-4o-mini", "gpt-4-turbo", "o3-mini"};
    if (p == "anthropic")   return {"claude-opus-4-6", "claude-sonnet-4-6", "claude-haiku-4-5-20251001"};
    if (p == "gemini")      return {"gemini-2.5-flash", "gemini-2.5-pro", "gemini-2.0-flash"};
    if (p == "groq")        return {"llama-3.3-70b-versatile", "mixtral-8x7b-32768", "gemma2-9b-it"};
    if (p == "deepseek")    return {"deepseek-chat", "deepseek-reasoner"};
    if (p == "openrouter")  return {"openai/gpt-4o", "anthropic/claude-sonnet-4-6", "google/gemini-2.5-flash"};
    if (p == "ollama")      return {"llama3:8b", "mistral:7b", "codellama:7b"};
    if (p == "fincept")     return {"fincept-llm"};
    return {};
}
...
LlmConfigSection::LlmConfigSection(QWidget* parent) : QWidget(parent) {
    ...
    connect(&ai_chat::LlmService::instance(), &ai_chat::LlmService::models_fetched,
            this, &LlmConfigSection::on_models_fetched);
...
QWidget* LlmConfigSection::build_provider_list_panel() {
    ...
    connect(provider_list_, &QListWidget::currentRowChanged,
            this, &LlmConfigSection::on_provider_selected);
    ...
    connect(add_btn_, &QPushButton::clicked, this, [this]() {
        ...
        bool ok;
        QString provider = QInputDialog::getItem(this, "Add Provider",
            "Select provider:", choices, 0, false, &ok);
...
QWidget* LlmConfigSection::build_form_panel() {
    ...
    auto lbl_style = [](QLabel* l) {
        l->setStyleSheet("color:#888;font-size:12px;");
    };

    auto* p_lbl = new QLabel("Provider");  lbl_style(p_lbl);
    ...
    provider_edit_->setStyleSheet(provider_edit_->styleSheet() +
        "QLineEdit{background:#0d0d0d;color:#666;}");
    form->addRow(p_lbl, provider_edit_);

    auto* k_lbl = new QLabel("API Key");   lbl_style(k_lbl);
    ...
    auto* m_lbl = new QLabel("Model");     lbl_style(m_lbl);
    ...
    model_combo_->setStyleSheet(
        "QComboBox{background:#111;color:#e0e0e0;border:1px solid #2a2a2a;"
        "border-radius:3px;padding:6px;font-size:12px;}"
    ...
    fetch_btn_->setStyleSheet(
        "QPushButton{background:#1a1a1a;color:#ff6600;border:1px solid #333;"
    ...
    auto* b_lbl = new QLabel("Base URL");  lbl_style(b_lbl);
    ...
    save_btn_->setStyleSheet(
        "QPushButton{background:#ff6600;color:#000;border:none;border-radius:4px;"
...
QWidget* LlmConfigSection::build_global_panel() {
    ...
    save_global_btn_->setStyleSheet(
        "QPushButton{background:#1a1a1a;color:#ff6600;border:1px solid #ff6600;"
        "border-radius:3px;font-size:11px;font-weight:600;}"
        "QPushButton:hover{background:#2a1a0a;}");
...
void LlmConfigSection::load_providers() {
    ...
    if (result.is_ok()) {
        for (const auto& p : result.value()) {
            ...
            if (p.is_active) display += "  ✓";
            if (!p.model.isEmpty()) display += "  [" + p.model + "]";
    ...
    for (const auto& p : result.value()) {
        if (p.provider.toLower() == provider.toLower()) {
            ...
            if (!p.model.isEmpty()) {
                ...
                else model_combo_->setCurrentText(p.model);
            }
...
void LlmConfigSection::on_save_provider() {
    ...
    // Fincept defaults
    if (is_fincept) {
        if (cfg.model.isEmpty())    cfg.model    = "fincept-llm";
        if (cfg.base_url.isEmpty()) cfg.base_url = "https://api.fincept.in/research/llm";
    }
...
void LlmConfigSection::on_delete_provider() {
    ...
    auto reply = QMessageBox::question(this, "Delete Provider",
...
void LlmConfigSection::on_test_connection() {
    ...
    *conn = connect(&ai_chat::LlmService::instance(), &ai_chat::LlmService::models_fetched,
            this, [this, provider, conn](const QString& p, const QStringList& models, const QString& err) {
                if (p.toLower() != provider.toLower()) return;
    ...
    ai_chat::LlmService::instance().fetch_models(
        provider, api_key_edit_->text().trimmed(), base_url_edit_->text().trimmed());
}
...
void LlmConfigSection::on_models_fetched(const QString& provider,
