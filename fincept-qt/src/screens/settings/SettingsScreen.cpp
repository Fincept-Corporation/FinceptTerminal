...
static const char* SUB_TITLE =
    "color: #808080; font-size: 11px; font-weight: 700; letter-spacing: 0.5px; "
    "background: transparent;";

static const char* LABEL =
    "color: #888888; font-size: 13px; background: transparent;";

static const char* NAV_BTN =
...
static const char* INPUT_SS =
    "QLineEdit { background: #111; color: #e0e0e0; border: 1px solid #2a2a2a; "
...
static const char* BTN_SECONDARY =
...
static const char* CHECK_SS =
    "QCheckBox { color: #888; font-size: 13px; background: transparent; }"
...
QWidget* SettingsScreen::make_row(const QString& label, QWidget* control,
...
QWidget* SettingsScreen::build_credentials() {
    ...
    for (const auto& def : CRED_KEYS) {
        ...
        card->setStyleSheet(
            "QFrame { background: #0d0d0d; border: 1px solid #1a1a1a; border-radius: 4px; }");
...
QWidget* SettingsScreen::build_appearance() {
    ...
    app_font_family_->addItems({
        "Consolas", "Courier New", "JetBrains Mono", "Fira Code", "Cascadia Code"
    });
    ...
    vl->addWidget(make_row("Content Density", app_density_,
...
QWidget* SettingsScreen::build_notifications() {
    ...
    connect(notif_email_, &QCheckBox::toggled,
            email_row, &QWidget::setVisible);
    ...
    notif_inapp_ = new QCheckBox;  notif_inapp_->setStyleSheet(CHECK_SS);
    notif_price_ = new QCheckBox;  notif_price_->setStyleSheet(CHECK_SS);
    notif_news_  = new QCheckBox;  notif_news_->setStyleSheet(CHECK_SS);
    notif_orders_= new QCheckBox;  notif_orders_->setStyleSheet(CHECK_SS);
    ...
    notif_sound_ = new QCheckBox;  notif_sound_->setStyleSheet(CHECK_SS);
    vl->addWidget(make_row("Sound Alerts", notif_sound_,
        "Plays a chime on price alerts and order fills."));
...
QWidget* SettingsScreen::build_storage() {
    ...
    auto* note = new QLabel(
        "Clears all cached market data, quotes, and news. "
    ...
    static const QStringList CACHE_CATS = {
        "market_data", "news", "quotes", "charts", "general"
    };
...
void SettingsScreen::refresh_storage_stats() {
    if (storage_count_)
        storage_count_->setText(
            QString::number(CacheManager::instance().entry_count()) + " entries");
}
...
QWidget* SettingsScreen::build_data_sources() {
    ...
    auto* info = new QLabel(
        "Toggle which external data providers are active. "
    ...
    for (auto it = grouped.constBegin(); it != grouped.constEnd(); ++it) {
        ...
        for (const auto& ds : it.value()) {
            ...
            auto* name_lbl = new QLabel(
                ds.display_name.isEmpty() ? ds.alias : ds.display_name);
            ...
            connect(toggle, &QCheckBox::toggled, this,
                [source_id](bool checked) {
                    DataSourceRepository::instance().set_enabled(source_id, checked);
                    LOG_INFO("DataSources",
                        (checked ? "Enabled: " : "Disabled: ") + source_id);
                });
            ...
            if (!ds.description.isEmpty()) {
                ...
                desc->setWordWrap(true);
                desc->setStyleSheet(
                    "color: #555; font-size: 11px; padding: 8px 12px; background: transparent;");
                cvl2->addWidget(desc);
            }
