...
UpdateComponentCommand::UpdateComponentCommand(ReportBuilderScreen* screen, int index,
                                               const QString& old_content,
                                               const QMap<QString, QString>& old_config,
                                               const QString& new_content,
                                               const QMap<QString, QString>& new_config)
    : QUndoCommand("Edit component"), screen_(screen), index_(index),
      old_content_(old_content), new_content_(new_content),
      old_config_(old_config), new_config_(new_config) {}
...
bool UpdateComponentCommand::mergeWith(const QUndoCommand* other) {
    ...
    if (cmd->index_ != index_) return false;
...
void ReportBuilderScreen::remove_component_direct(int index) {
    if (index >= 0 && index < components_.size()) {
        ...
        if (selected_ >= components_.size()) selected_ = components_.size() - 1;
        refresh_canvas();
        refresh_structure();
...
ReportBuilderScreen::ReportBuilderScreen(QWidget* parent) : QWidget(parent) {
    ...
    connect(comp_toolbar_, &ComponentToolbar::move_up, this, [this](int idx) {
        if (idx > 0) undo_stack_->push(new MoveComponentCommand(this, idx, idx - 1));
    });
    ...
    connect(properties_, &PropertiesPanel::content_changed, this, [this](int idx, const QString& content) {
        if (idx < 0 || idx >= components_.size()) return;
            ...
            [this](int idx, const QString& key, const QString& val) {
                ...
                if (components_[idx].type == "stats_block" && key == "fetch_ticker") {
                    ...
                    fincept::services::MarketDataService::instance().fetch_info(
                        sym, [self, idx, sym](bool ok, fincept::services::InfoData info) {
                            if (!self) return;
                            QMetaObject::invokeMethod(self, [self, idx, sym, ok, info]() {
                                if (!self || idx >= self->components_.size()) return;
                                QStringList lines;
                                if (!ok) {
                ...
                if (components_[idx].type == "chart" && key == "fetch_history") {
                    ...
                    fincept::services::MarketDataService::instance().fetch_history(
                        sym, "6mo", "1d",
                        [self, idx, sym](bool ok, QVector<fincept::services::HistoryPoint> history) {
                            if (!self) return;
                            QMetaObject::invokeMethod(self, [self, idx, sym, ok, history]() {
                                if (!self || idx >= self->components_.size()) return;
                                ...
                                if ((history.size() - 1) % step != 0) {
                                    ...
                                    lbls << QDateTime::fromSecsSinceEpoch(
                                                history.last().timestamp).toString("MMM d");
                                }
                    ...
                    if (!sym.isEmpty()) {
                        ...
                        fincept::services::MarketDataService::instance().fetch_quotes(
                            {sym}, [self, idx](bool ok, QVector<fincept::services::QuoteData> quotes) {
                                if (!self || !ok || quotes.isEmpty()) return;
                                QMetaObject::invokeMethod(self, [self, idx, quotes]() {
                                    if (!self || idx >= self->components_.size()) return;
                                    ...
                                    self->components_[idx].config["high"]       = q.high  > 0 ? QString::number(q.high,   'f', 2) : "";
                                    self->components_[idx].config["low"]        = q.low   > 0 ? QString::number(q.low,    'f', 2) : "";
                                    self->components_[idx].config["volume"]     = q.volume > 0 ? QString::number(q.volume, 'f', 0) : "";
                                    self->components_[idx].config["status"]     = "ok";
                                    self->refresh_canvas();
                                    // Re-show properties with updated data
                                    if (self->selected_ == idx) {
                                        self->properties_->show_properties(
                                            &self->components_[idx], idx);
                                    }
                                }, Qt::QueuedConnection);
                            });
                    }
                ...
                auto new_config  = old_config;
                new_config[key]  = val;
                undo_stack_->push(new UpdateComponentCommand(this, idx, old_content, old_config,
                                                              old_content, new_config));
            });
    ...
    connect(canvas_, &DocumentCanvas::image_dropped, this, [this](const QString& path) {
        ...
        if (selected_ >= 0 && selected_ < components_.size() &&
            components_[selected_].type == "image") {
            ...
            undo_stack_->push(new UpdateComponentCommand(this, selected_, old_content, old_config,
                                                          old_content, new_config));
            select_component(selected_);
        } else {
            ...
            int at = (selected_ >= 0 && selected_ < components_.size()) ? selected_ + 1
                                                                         : components_.size();
...
QWidget* ReportBuilderScreen::build_toolbar() {
    ...
    pdf_btn->setStyleSheet(
        QString("QPushButton { color: %1; font-weight: bold; }").arg(ui::colors::AMBER));
...
void ReportBuilderScreen::refresh_structure() {
    ...
    for (const auto& c : components_) {
        ...
        if (!c.content.isEmpty())          label += ": " + c.content.left(28);
        else if (c.type == "market_data")  label += ": " + c.config.value("symbol", "?");
        else if (c.type == "chart")        label += ": " + c.config.value("title", "Chart");
        else if (c.type == "stats_block")  label += ": " + c.config.value("title", "Stats");
        else if (c.type == "sparkline")    label += ": " + c.config.value("title", "");
        else if (c.type == "callout")      label += " [" + c.config.value("style", "info") + "]";
        items << label;
    }
...
bool ReportBuilderScreen::deserialize_from_json(const QString& json) {
    ...
    if (root.contains("theme")) {
        ...
        if      (tname == "Dark Corporate")     theme_ = report_themes::dark_corporate();
        else if (tname == "Bloomberg Terminal") theme_ = report_themes::bloomberg();
        else if (tname == "Midnight Blue")      theme_ = report_themes::midnight_blue();
        else                                    theme_ = report_themes::light_professional();
    }
    ...
    if (root.contains("next_id")) next_id_ = root["next_id"].toInt(next_id_);
...
void ReportBuilderScreen::show_recent_dialog() {
    ...
    dlg->setStyleSheet(
        QString("QDialog { background: %1; color: %2; }"
            ...
            .arg(ui::colors::DARK, ui::colors::WHITE, ui::colors::PANEL,
                 ui::colors::BORDER, ui::colors::BG_RAISED));
...
void ReportBuilderScreen::show_template_dialog() {
    ...
    struct TemplateEntry { QString name; QString description; };
    struct Category { QString label; QVector<TemplateEntry> entries; };
    ...
    dlg->setStyleSheet(
        QString("QDialog    { background: %1; color: %2; }"
            ...
            .arg(ui::colors::DARK, ui::colors::WHITE, ui::colors::PANEL,
                 ui::colors::BORDER, ui::colors::AMBER_DIM, ui::colors::AMBER,
                 ui::colors::BG_RAISED));
    ...
    pick_lbl->setStyleSheet(
        QString("color: %1; font-size: 12px; font-weight: bold;").arg(ui::colors::MUTED));
    ...
    tmpl_name_lbl->setStyleSheet(
        QString("color: %1; font-size: 14px; font-weight: bold;").arg(ui::colors::AMBER));
    ...
    desc_lbl->setStyleSheet(
        QString("color: %1; font-size: 12px; line-height: 160%;").arg(ui::colors::GRAY));
    ...
    connect(list, &QListWidget::currentItemChanged, this,
            [tmpl_name_lbl, desc_lbl](QListWidgetItem* cur, QListWidgetItem*) {
                if (!cur || !(cur->flags() & Qt::ItemIsSelectable)) return;
    ...
    connect(bb, &QDialogButtonBox::accepted, dlg, [this, list, dlg]() {
        ...
        if (cur && (cur->flags() & Qt::ItemIsSelectable)) {
            ...
            if (!tname.isEmpty()) apply_template(tname);
        }
        dlg->accept();
    });
    connect(bb, &QDialogButtonBox::rejected, dlg, &QDialog::reject);
...
void ReportBuilderScreen::show_theme_dialog() {
    ...
    for (const auto& t : themes) combo->addItem(t.name);
    // Select current theme
    for (int i = 0; i < themes.size(); ++i) {
        if (themes[i].name == theme_.name) { combo->setCurrentIndex(i); break; }
    }
    vl->addWidget(combo);
    ...
    auto update_preview = [themes, preview_lbl](int idx) {
        ...
        preview_lbl->setStyleSheet(
            QString("background: %1; color: %2; border: 2px solid %3; font-weight: bold;")
...
void ReportBuilderScreen::show_metadata_dialog() {
    ...
    auto* tl = new QLabel("Title:");        tl->setStyleSheet(lbl_style);
    auto* al = new QLabel("Author:");       al->setStyleSheet(lbl_style);
    auto* cl = new QLabel("Company:");      cl->setStyleSheet(lbl_style);
    auto* dl = new QLabel("Date:");         dl->setStyleSheet(lbl_style);
    ...
    auto f_row = [&](const char* left_text, QLineEdit* l, QLineEdit* c, QLineEdit* r) {
        ...
        rl->addWidget(l); rl->addWidget(c); rl->addWidget(r);
...
void ReportBuilderScreen::apply_template(const QString& name) {
    ...
    auto add = [&](const QString& type, const QString& content = {},
                   const QMap<QString, QString>& cfg = {}) {
        ReportComponent c;
        c.id      = next_id_++;
        c.type    = type;
        c.content = content;
    ...
    if (name == "Blank Report") {
        ...
        add("chart",   {}, {{"chart_type", "bar"}, {"title", "Valuation Scenarios"}, {"data", "80,100,130"}, {"labels", "Bear,Base,Bull"}});
    ...
    // ── Retail Investor ───────────────────────────────────────────────────────
        ...
        add("chart",   {}, {{"chart_type", "bar"}, {"title", "Revenue vs Net Income"}, {"data", "120,130,145,160"}, {"labels", "FY21,FY22,FY23,FY24"}});
        ...
        add("chart",   {}, {{"chart_type", "line"}, {"title", "Portfolio Value"}, {"data", "10000,10500,9800,11200,12000,11500,13000"}});
        add("heading", "Asset Allocation");
        add("chart",   {}, {{"chart_type", "pie"}, {"title", "Allocation"}, {"data", "40,30,20,10"}, {"labels", "Equities,Fixed Income,Crypto,Cash"}});
        ...
        add("chart",   {}, {{"chart_type", "bar"}, {"title", "Monthly Dividend Income"}, {"data", "200,180,220,210,195,230,215,240,200,220,210,250"}, {"labels", "Jan,Feb,Mar,Apr,May,Jun,Jul,Aug,Sep,Oct,Nov,Dec"}});
        add("heading", "Yield by Holding");
        add("chart",   {}, {{"chart_type", "pie"}, {"title", "Income Contribution"}, {"data", "30,25,20,15,10"}, {"labels", "VZ,T,PFE,KO,MO"}});
    ...
    // ── Trader ────────────────────────────────────────────────────────────────
        ...
        add("chart",   {}, {{"chart_type", "bar"}, {"title", "Sector Returns Today"}, {"data", "0.5,-0.3,1.2,-0.8,0.4,0.9,-0.2,1.1,0.3,-0.5,0.7"}, {"labels", "XLK,XLF,XLV,XLE,XLI,XLY,XLP,XLC,XLB,XLU,XLRE"}});
        ...
        add("chart",   {}, {{"chart_type", "line"}, {"title", "Cumulative P&L"}, {"data", "0,150,-80,320,210,480,390,620"}});
        add("heading", "Statistics");
        add("table",   {}, {{"rows", "7"}, {"cols", "2"}});   // Win Rate, Avg Win, Avg Loss, R:R, Max DD, Profit Factor, Trades
        add("heading", "Win/Loss Breakdown");
        add("chart",   {}, {{"chart_type", "pie"}, {"title", "Win vs Loss"}, {"data", "62,38"}, {"labels", "Wins,Losses"}});
        ...
        add("list",    "Reviewed overnight news\nIdentified key levels\nSet max daily loss\nNo FOMO trades\nLog every trade");
    ...
    // ── Institutional / Analyst ───────────────────────────────────────────────
        ...
        add("chart",   {}, {{"chart_type", "bar"}, {"title", "Revenue & EPS Growth"}, {"data", "100,115,130,148,169"}, {"labels", "FY20,FY21,FY22,FY23,FY24"}});
        add("heading", "Valuation");
        add("table",   {}, {{"rows", "5"}, {"cols", "4"}});   // DCF, P/E, EV/EBITDA, comparables
        add("chart",   {}, {{"chart_type", "bar"}, {"title", "Valuation vs Peers"}, {"data", "22,18,25,19,28"}, {"labels", "AAPL,MSFT,GOOGL,META,AMZN"}});
        add("heading", "Investment Thesis");
        add("list",    "Catalyst 1\nCatalyst 2\nCatalyst 3");
        add("heading", "Risks");
        ...
        add("chart",   {}, {{"chart_type", "bar"}, {"title", "Revenue by Segment"}, {"data", "42,18,10,8,6"}, {"labels", "iPhone,Services,Mac,iPad,Wearables"}});
        ...
        add("chart",   {}, {{"chart_type", "bar"}, {"title", "Synergy Build-Up ($M)"}, {"data", "50,120,200,250"}, {"labels", "Yr1,Yr2,Yr3,Yr4"}});
        ...
        add("chart",   {}, {{"chart_type", "bar"}, {"title", "Market Size ($B)"}, {"data", "800,900,1020,1160,1320"}, {"labels", "2020,2021,2022,2023,2024E"}});
        ...
        add("chart",   {}, {{"chart_type", "bar"}, {"title", "P/E Ratio by Company"}, {"data", "22,19,25,17,30"}, {"labels", "Co1,Co2,Co3,Co4,Co5"}});
    ...
    // ── Economist / Macro ─────────────────────────────────────────────────────
        ...
        add("chart",   {}, {{"chart_type", "line"}, {"title", "GDP Growth Rate (%)"}, {"data", "2.1,2.3,1.8,2.5,2.2,1.9,2.4"}, {"labels", "Q1 23,Q2 23,Q3 23,Q4 23,Q1 24,Q2 24,Q3 24"}});
        add("heading", "Inflation");
        add("chart",   {}, {{"chart_type", "line"}, {"title", "CPI YoY (%)"}, {"data", "6.0,5.2,4.3,3.7,3.2,2.9,2.6"}, {"labels", "Q1 23,Q2 23,Q3 23,Q4 23,Q1 24,Q2 24,Q3 24"}});
        ...
        add("chart",   {}, {{"chart_type", "line"}, {"title", "GDP Growth vs Inflation"}, {"data", "3.0,3.4,2.8,1.9,2.5"}, {"labels", "2020,2021,2022,2023,2024E"}});
        ...
        add("chart",   {}, {{"chart_type", "line"}, {"title", "Policy Rate (%)"}, {"data", "0.25,0.5,1.0,2.0,3.5,4.5,5.25,5.25,5.0"}, {"labels", "Jan22,Mar22,Jun22,Sep22,Dec22,Mar23,Jun23,Sep23,Dec23"}});
        ...
        add("chart",   {}, {{"chart_type", "bar"}, {"title", "Balance Sheet Size ($T)"}, {"data", "8.9,8.5,8.1,7.8,7.4"}, {"labels", "Q4 22,Q1 23,Q2 23,Q3 23,Q4 23"}});
    ...
    // ── Crypto / Digital Assets ───────────────────────────────────────────────
        ...
        add("chart",   {}, {{"chart_type", "pie"}, {"title", "Token Distribution"}, {"data", "20,15,30,25,10"}, {"labels", "Team,Investors,Community,Treasury,Public"}});
        add("heading", "On-Chain Metrics");
        add("table",   {}, {{"rows", "6"}, {"cols", "3"}});   // Metric | Value | Trend
        add("heading", "Price History");
        add("chart",   {}, {{"chart_type", "line"}, {"title", "Price (USD)"}, {"data", "16000,20000,25000,30000,28000,42000,38000,52000"}});
        ...
        add("chart",   {}, {{"chart_type", "line"}, {"title", "Total Value Locked ($M)"}, {"data", "500,800,1200,900,1500,2000,1800,2400"}});
        add("heading", "Revenue & Fees");
        add("chart",   {}, {{"chart_type", "bar"}, {"title", "Monthly Protocol Revenue ($K)"}, {"data", "120,150,180,140,200,230,210,250"}});
        ...
        add("chart",   {}, {{"chart_type", "pie"}, {"title", "Portfolio Allocation"}, {"data", "50,25,15,10"}, {"labels", "BTC,ETH,SOL,Other"}});
        add("heading", "Performance");
        add("chart",   {}, {{"chart_type", "line"}, {"title", "Portfolio Value (USD)"}, {"data", "10000,9500,12000,11000,15000,13000,18000"}});
        add("heading", "Notes");
    ...
    // ── Fixed Income ──────────────────────────────────────────────────────────
        ...
        add("chart",   {}, {{"chart_type", "line"}, {"title", "Yield vs Benchmark"}, {"data", "3.0,3.2,3.5,3.8,4.1,4.3,4.0"}, {"labels", "Jan,Feb,Mar,Apr,May,Jun,Jul"}});
        ...
        add("chart",   {}, {{"chart_type", "line"}, {"title", "US Treasury Yield Curve"}, {"data", "5.3,5.1,4.9,4.7,4.6,4.5,4.4,4.3"}, {"labels", "3M,6M,1Y,2Y,3Y,5Y,10Y,30Y"}});
        add("heading", "Key Spreads");
        add("table",   {}, {{"rows", "5"}, {"cols", "3"}});   // Spread | Current | 1M Ago
        add("heading", "Historical Comparison");
        add("chart",   {}, {{"chart_type", "bar"}, {"title", "10Y-2Y Spread (bps)"}, {"data", "-80,-60,-40,-20,5,15,30"}, {"labels", "Oct23,Nov23,Dec23,Jan24,Feb24,Mar24,Apr24"}});
        add("heading", "Duration & Convexity");
        add("table",   {}, {{"rows", "4"}, {"cols", "3"}});
        add("heading", "Macro Drivers");
        add("text",    "Explain the key macro factors driving current curve shape.");
        add("heading", "Implications");
        add("list",    "Recession signal interpretation\nFed policy implications\nSector rotation impact\nCurrency effects");
    ...
    // ── Quant / Risk ──────────────────────────────────────────────────────────
        ...
        add("chart",   {}, {{"chart_type", "line"}, {"title", "Strategy vs Benchmark"}, {"data", "100,108,115,112,124,131,128,142,150"}, {"labels", "Jan,Feb,Mar,Apr,May,Jun,Jul,Aug,Sep"}});
        add("heading", "Drawdown Analysis");
        add("chart",   {}, {{"chart_type", "bar"}, {"title", "Monthly Drawdowns (%)"}, {"data", "0,-2.1,-0.5,-3.2,-1.1,0,-1.8,-0.4,0"}});
        ...
        add("chart",   {}, {{"chart_type", "bar"}, {"title", "VaR by Confidence Level"}, {"data", "1.2,1.8,2.5,3.1"}, {"labels", "90%,95%,99%,99.9%"}});
        ...
        add("chart",   {}, {{"chart_type", "pie"}, {"title", "Position Concentration"}, {"data", "25,20,18,15,12,10"}, {"labels", "Pos1,Pos2,Pos3,Pos4,Pos5,Other"}});
    ...
    // ── Corporate / Business ──────────────────────────────────────────────────
        ...
        add("heading", "Revenue");
        add("chart",   {}, {{"chart_type", "bar"}, {"title", "Revenue ($M)"}, {"data", "42,45,48,44,51,53,50,55"}, {"labels", "Q1 22,Q2 22,Q3 22,Q4 22,Q1 23,Q2 23,Q3 23,Q4 23"}});
        add("heading", "Cost Breakdown");
        add("chart",   {}, {{"chart_type", "pie"}, {"title", "Cost Structure"}, {"data", "40,25,20,15"}, {"labels", "COGS,R&D,S&M,G&A"}});
        add("heading", "Department Performance");
        add("table",   {}, {{"rows", "6"}, {"cols", "4"}});
        add("heading", "Targets vs Actuals");
        add("chart",   {}, {{"chart_type", "bar"}, {"title", "Target vs Actual"}, {"data", "100,95,100,102,100,88"}, {"labels", "Jan,Feb,Mar,Apr,May,Jun"}});
        ...
        add("chart",   {}, {{"chart_type", "bar"}, {"title", "Revenue & Net Income"}, {"data", "400,450,500,420,480,530"}, {"labels", "H1 22,H2 22,H1 23,H2 23,H1 24,H2 24"}});
        add("page_break");
        add("heading", "Balance Sheet");
    ...
    refresh_canvas();
    refresh_structure();
    if (!components_.isEmpty()) select_component(0);
}
...
void ReportBuilderScreen::on_new() {
    auto answer = QMessageBox::question(this, "New Report",
                                        "Create a new report? Unsaved changes will be lost.",
...
void ReportBuilderScreen::on_open() {
    QString path = QFileDialog::getOpenFileName(this, "Open Report", "",
...
void ReportBuilderScreen::on_auto_save() {
    if (autosave_path_.isEmpty()) {
        autosave_path_ = QStandardPaths::writableLocation(QStandardPaths::TempLocation)
                         + "/fincept_report_autosave.fincept";
    }
...
void ReportBuilderScreen::on_export_pdf() {
    ...
    bool has_header = !metadata_.header_left.isEmpty()  ||
                      !metadata_.header_center.isEmpty() ||
                      !metadata_.header_right.isEmpty();
    bool has_footer = !metadata_.footer_left.isEmpty()  ||
                      !metadata_.footer_center.isEmpty() ||
                      !metadata_.footer_right.isEmpty()  ||
                      metadata_.show_page_numbers;
    ...
    if (!has_header && !has_footer) {
        ...
        auto draw_hf_line = [&](int page, bool is_header) {
            ...
            painter.drawLine(QPointF(page_rect.left() + 8, rule_y),
                             QPointF(page_rect.right() - 8, rule_y));
        };
        ...
        for (int page = 1; page <= total_pages; ++page) {
            ...
            painter.setClipRect(QRectF(page_rect.left(), content_top,
                                       page_rect.width(), content_bottom - content_top));
            ...
            if (has_header) draw_hf_line(page, true);
            if (has_footer) draw_hf_line(page, false);
        }
