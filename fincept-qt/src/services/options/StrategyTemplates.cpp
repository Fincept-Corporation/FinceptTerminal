...
QVector<StrategyTemplate> build_catalog() {
    ...
    v.append({
        ...
        {{InstrumentType::PE, -1, -1},
         {InstrumentType::PE, -2, +1},
         {InstrumentType::CE, +1, -1},
         {InstrumentType::CE, +2, +1}},
        true,
        1,
    });
    v.append({
...
Result<Strategy> instantiate(const StrategyTemplate& tpl, const OptionChain& chain,
    ...
    for (const auto& recipe : tpl.legs) {
        ...
        if (target < 0 || target >= chain.rows.size()) {
            return Result<Strategy>::err(QString("leg out of chain bounds (template=%1 offset=%2 target=%3 size=%4) — "
        ...
        if (recipe.type == InstrumentType::CE) {
            if (row.ce_token == 0)
                return Result<Strategy>::err(
                    QString("no CE token at strike %1 (template=%2)").arg(row.strike).arg(tpl.id).toStdString());
            ...
            if (row.pe_token == 0)
                return Result<Strategy>::err(
                    QString("no PE token at strike %1 (template=%2)").arg(row.strike).arg(tpl.id).toStdString());
            ...
            return Result<Strategy>::err(
                QString("template %1 has unsupported leg type (CE/PE only in v1)").arg(tpl.id).toStdString());
        }
