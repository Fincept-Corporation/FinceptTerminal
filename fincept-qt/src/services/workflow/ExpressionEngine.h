...
class ExpressionEngine {
  public:
    ...
    static QJsonValue evaluate(const QJsonValue& param_value,
                               const QJsonObject& context);

    /// Resolve all string values in a parameters object.
    static QJsonObject resolve_params(const QJsonObject& params,
                                      const QJsonObject& context);

  private:
    /// Resolve a single expression string like "$input.key" or "$input.nested.key".
    static QJsonValue resolve_expression(const QString& expr,
                                         const QJsonObject& context);
};
