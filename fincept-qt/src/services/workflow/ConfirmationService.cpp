...
void ConfirmationService::request(const ConfirmationRequest& req,
                                   std::function<void(bool, const QString&)> callback)
{
    ...
    if (req.paper_trading && auto_approve_paper_ && req.risk != RiskLevel::Critical) {
        ...
        AuditLogger::instance().log(AuditAction::ConfirmationApproved,
            {}, {}, {}, "Auto-approved (paper trading)", req.details, true);
    ...
    LOG_INFO("Confirmation", QString("Requesting: %1 (risk=%2)")
             ...
             .arg(req.risk == RiskLevel::Critical ? "CRITICAL" :
                  ...
                  req.risk == RiskLevel::Medium ? "MEDIUM" : "LOW"));
}
...
void ConfirmationService::resolve(const QString& id, bool approved, const QString& notes)
{
    ...
    AuditLogger::instance().log(
        approved ? AuditAction::ConfirmationApproved : AuditAction::ConfirmationRejected,
        entry.request.details.value("workflow_id").toString(),
        {}, {}, notes.isEmpty() ? (approved ? "Approved" : "Rejected") : notes,
        entry.request.details, entry.request.paper_trading);

    entry.callback(approved, notes);
    emit confirmation_resolved(id, approved);

    LOG_INFO("Confirmation", QString("%1: %2")
             .arg(approved ? "Approved" : "Rejected", entry.request.title));
}
