...
{
    auto r = fincept::Database::instance().exec(
        ...
        ")");
    if (r.is_err())
        LOG_ERROR("AuditLogger", QString("Failed to create audit table: %1")
                  .arg(QString::fromStdString(r.error())));

    fincept::Database::instance().exec(
        "CREATE INDEX IF NOT EXISTS idx_audit_ts ON workflow_audit_log(timestamp)");
    fincept::Database::instance().exec(
        "CREATE INDEX IF NOT EXISTS idx_audit_action ON workflow_audit_log(action)");
    fincept::Database::instance().exec(
        "CREATE INDEX IF NOT EXISTS idx_audit_wf ON workflow_audit_log(workflow_id)");
}

void AuditLogger::log(AuditAction action, const QString& workflow_id,
                       ...
                       bool paper_trading)
{
    ...
    auto r = db.execute(
        "INSERT INTO workflow_audit_log (action, workflow_id, node_id, symbol, details, metadata, paper) "
        "VALUES (?, ?, ?, ?, ?, ?, ?)",
...
Result<QVector<AuditEntry>> AuditLogger::query(const QDateTime& from, const QDateTime& to,
                                                AuditAction action_filter, bool filter_by_action) const
{
...
{
    switch (a) {
    case AuditAction::OrderPlaced:           return "order_placed";
    case AuditAction::OrderCancelled:        return "order_cancelled";
    case AuditAction::OrderModified:         return "order_modified";
    case AuditAction::OrderFilled:           return "order_filled";
    case AuditAction::WorkflowStarted:       return "workflow_started";
    case AuditAction::WorkflowCompleted:     return "workflow_completed";
    case AuditAction::WorkflowFailed:        return "workflow_failed";
    case AuditAction::NodeExecuted:          return "node_executed";
    case AuditAction::NodeFailed:            return "node_failed";
    case AuditAction::RiskCheckPassed:       return "risk_check_passed";
    case AuditAction::RiskCheckFailed:       return "risk_check_failed";
    case AuditAction::ConfirmationApproved:  return "confirmation_approved";
    case AuditAction::ConfirmationRejected:  return "confirmation_rejected";
    case AuditAction::LimitChanged:          return "limit_changed";
    case AuditAction::SettingsChanged:       return "settings_changed";
    }
    return "unknown";
}
...
{
    if (s == "order_placed")           return AuditAction::OrderPlaced;
    if (s == "order_cancelled")        return AuditAction::OrderCancelled;
    if (s == "order_modified")         return AuditAction::OrderModified;
    if (s == "order_filled")           return AuditAction::OrderFilled;
    if (s == "workflow_started")       return AuditAction::WorkflowStarted;
    if (s == "workflow_completed")     return AuditAction::WorkflowCompleted;
    if (s == "workflow_failed")        return AuditAction::WorkflowFailed;
    if (s == "node_executed")          return AuditAction::NodeExecuted;
    if (s == "node_failed")            return AuditAction::NodeFailed;
    if (s == "risk_check_passed")      return AuditAction::RiskCheckPassed;
    if (s == "risk_check_failed")      return AuditAction::RiskCheckFailed;
    if (s == "confirmation_approved")  return AuditAction::ConfirmationApproved;
    if (s == "confirmation_rejected")  return AuditAction::ConfirmationRejected;
    if (s == "limit_changed")          return AuditAction::LimitChanged;
    if (s == "settings_changed")       return AuditAction::SettingsChanged;
