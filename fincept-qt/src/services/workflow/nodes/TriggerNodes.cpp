...
void register_trigger_nodes(NodeRegistry& registry)
{
    ...
    registry.register_type({
        .type_id = "trigger.cron_market", .display_name = "Market Hours Cron",
        .category = "Triggers", .description = "Scheduled trigger that only fires during market hours",
        .icon_text = ">>", .accent_color = "#d97706", .version = 1,
        .inputs  = {},
        .outputs = {{ "output_main", "Main", PortDirection::Output, ConnectionType::Main }},
        .parameters = {
    ...
    registry.register_type({
        .type_id = "trigger.portfolio_drift", .display_name = "Portfolio Drift",
        .category = "Triggers", .description = "Trigger when portfolio drifts from target allocation",
        .icon_text = ">>", .accent_color = "#d97706", .version = 1,
        .inputs  = {},
        .outputs = {{ "output_main", "Main", PortDirection::Output, ConnectionType::Main }},
        .parameters = {
            { "drift_threshold_pct", "Drift Threshold %", "number", 5.0, {}, "Fire when any position drifts by this %" },
