...
void DiscordProvider::send(const NotificationRequest& req,
                           std::function<void(bool, QString)> cb) {
    if (!is_configured()) { cb(false, "Not configured"); return; }

    const int color = [&]() -> int {
        switch (req.level) {
            case NotifLevel::Warning:  return 0xF59E0B; // amber
            case NotifLevel::Alert:    return 0xF97316; // orange
            case NotifLevel::Critical: return 0xEF4444; // red
