...
void TelegramProvider::send(const NotificationRequest& req,
    ...
    const QString level_tag = [&]() -> QString {
        switch (req.level) {
            case NotifLevel::Warning:  return "⚠️ ";
            case NotifLevel::Alert:    return "🔔 ";
            case NotifLevel::Critical: return "🚨 ";
            default:                   return "ℹ️ ";
        }
    }();
    ...
    body["text"]       = QString("<b>%1%2</b>\n%3\n<i>%4</i>")
                             .arg(level_tag, req.title, req.message,
