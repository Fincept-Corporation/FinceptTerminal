...
// ── Python helper ────────────────────────────────────────────────────────────
void AIQuantLabService::run_python(const QString& script, const QStringList& args,
    ...
    python::PythonRunner::instance().run(script, args,
        [self, module_id, command](python::PythonResult result) {
            if (!self) return;
            if (!result.success) {
                LOG_ERROR("AIQuantLab", QString("[%1/%2] Failed: %3")
                    .arg(module_id, command, result.error));
...
void AIQuantLabService::run_module(const QString& module_id, const QString& command,
...
void AIQuantLabService::get_factor_library() {
    run_python("ai_quant_lab/qlib_service.py", {"get_factor_library"},
               "factor_discovery", "get_factor_library");
}
...
void AIQuantLabService::list_advanced_models() {
    run_python("ai_quant_lab/qlib_advanced_models.py", {"list_models"},
               "advanced_models", "list_models");
}
