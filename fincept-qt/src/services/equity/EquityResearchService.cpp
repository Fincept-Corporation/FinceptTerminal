...
void EquityResearchService::search_symbols(const QString& query) {
    ...
    run_python("yfinance_data.py", {"search", query, "20"},
...
void EquityResearchService::load_symbol(const QString& symbol, const QString& period) {
    ...
    if (qit != quote_cache_.end() && (now - qit->ts) < kQuoteTtlSec) {
        ...
        run_python("yfinance_data.py", {"quote", symbol},
    ...
    if (iit != info_cache_.end() && (now - iit->ts) < kInfoTtlSec) {
        ...
        run_python("yfinance_data.py", {"info", symbol},
            [this, symbol](bool ok, const QString& out) {
    ...
    if (hit != candle_cache_.end() && (now - hit->ts) < kHistoricalTtlSec) {
        ...
        run_python("yfinance_data.py", {"historical_period", symbol, period, "1d"},
            [this, symbol](bool ok, const QString& out) {
                if (!ok) { emit error_occurred("Historical", "Failed to fetch historical for " + symbol); return; }
...
void EquityResearchService::fetch_financials(const QString& symbol) {
    run_python("yfinance_data.py", {"financials", symbol},
        [this, symbol](bool ok, const QString& out) {
            if (!ok) { emit error_occurred("Financials", "Failed to fetch financials for " + symbol); return; }
            auto obj = QJsonDocument::fromJson(python::extract_json(out).toUtf8()).object();
            if (obj.contains("error")) { emit error_occurred("Financials", obj["error"].toString()); return; }
...
void EquityResearchService::fetch_technicals(const QString& symbol, const QString& period) {
    ...
    auto run_compute = [this, symbol](const QString& hist_json) {
        run_python("compute_technicals.py", {hist_json},
            [this, symbol](bool ok2, const QString& out2) {
                if (!ok2) { emit error_occurred("Technicals", "Indicator computation failed"); return; }
    ...
    if (!cached_json.isEmpty()) {
        ...
        run_python("yfinance_data.py", {"historical_period", symbol, period, "1d"},
            [this, symbol, run_compute](bool ok, const QString& out) {
                if (!ok) { emit error_occurred("Technicals", "Failed historical fetch"); return; }
...
void EquityResearchService::fetch_peers(const QString& symbol, const QStringList& peer_symbols) {
    ...
    run_python("yfinance_data.py", {"multiple_ratios", joined},
        [this](bool ok, const QString& out) {
            if (!ok) { emit error_occurred("Peers", "Failed to fetch peer data"); return; }
            auto arr = QJsonDocument::fromJson(python::extract_json(out).toUtf8()).array();
...
void EquityResearchService::fetch_news(const QString& symbol, int count) {
    ...
    run_python("yfinance_data.py", {"news", symbol, QString::number(count)},
        [this, symbol](bool ok, const QString& out) {
            if (!ok) { emit error_occurred("News", "Failed to fetch news for " + symbol); return; }
            auto obj = QJsonDocument::fromJson(python::extract_json(out).toUtf8()).object();
...
// ── TALIpp ────────────────────────────────────────────────────────────────────
void EquityResearchService::compute_talipp(const QString& symbol, const QString& indicator,
                                            const QVariantMap& params, const QString& period) {
    auto run_talipp = [this, indicator, params](const QString& hist_json) {
        ...
        run_python("equity_talipp.py", {indicator, hist_json, params_json},
            [this, indicator](bool ok, const QString& out) {
                if (!ok) { emit error_occurred("TALIpp", out); return; }
    ...
    if (hit != candle_cache_.end()) {
        ...
        for (const auto& c : hit->data) {
            QJsonObject o;
            o["timestamp"] = c.timestamp;
            o["open"]  = c.open;  o["high"] = c.high;
            o["low"]   = c.low;   o["close"] = c.close;
            o["volume"] = c.volume;
            arr.append(o);
        }
        ...
        run_python("yfinance_data.py", {"historical_period", symbol, period, "1d"},
            [this, symbol, run_talipp](bool ok, const QString& out) {
                if (!ok) { emit error_occurred("TALIpp", "Historical fetch failed"); return; }
                auto raw = python::extract_json(out);
...
TechSignal EquityResearchService::score_indicator(const QString& name, double value,
                                                   double sma20, double sma50) const {
    // RSI (0-100): <=30 buy, >=70 sell
    if (name == "rsi") {
        if (value <= 25) return TechSignal::StrongBuy;
        if (value <= 40) return TechSignal::Buy;
        if (value <= 60) return TechSignal::Neutral;
        if (value <= 75) return TechSignal::Sell;
    ...
    if (name == "sma_20" && sma50 > 0) {
        if (sma20 > sma50 * 1.02) return TechSignal::StrongBuy;
        if (sma20 > sma50)        return TechSignal::Buy;
        if (sma20 < sma50 * 0.98) return TechSignal::StrongSell;
        if (sma20 < sma50)        return TechSignal::Sell;
        return TechSignal::Neutral;
    }
    // CCI: <=-100 buy, >=100 sell
    if (name == "cci") {
        if (value <= -100) return TechSignal::StrongBuy;
        if (value <= -50)  return TechSignal::Buy;
        if (value >= 100)  return TechSignal::StrongSell;
        if (value >= 50)   return TechSignal::Sell;
        return TechSignal::Neutral;
    }
    // MFI (0-100): <=20 buy, >=80 sell
    if (name == "mfi") {
        if (value <= 20) return TechSignal::StrongBuy;
        if (value <= 40) return TechSignal::Buy;
        if (value >= 80) return TechSignal::StrongSell;
        if (value >= 60) return TechSignal::Sell;
        return TechSignal::Neutral;
    }
    // Stochastic %K and %D
    ...
    if (name == "williams_r") {
        if (value <= -80) return TechSignal::StrongBuy;
        if (value <= -60) return TechSignal::Buy;
        if (value >= -20) return TechSignal::StrongSell;
        if (value >= -40) return TechSignal::Sell;
        return TechSignal::Neutral;
    }
    // ROC: positive = buy
    if (name == "roc") {
        if (value > 5)  return TechSignal::Buy;
        if (value < -5) return TechSignal::Sell;
        return TechSignal::Neutral;
    }
    ...
    if (name == "bb_pband") {
        if (value < 0.2) return TechSignal::StrongBuy;
        if (value > 0.8) return TechSignal::StrongSell;
        return TechSignal::Neutral;
    }
    // CMF: >0.1 buy, <-0.1 sell
    if (name == "cmf") {
        if (value > 0.1)  return TechSignal::Buy;
        if (value < -0.1) return TechSignal::Sell;
        return TechSignal::Neutral;
    }
...
TechnicalsData EquityResearchService::parse_technicals(const QString& symbol,
                                                         const QJsonArray& rows) const {
    TechnicalsData td;
    td.symbol = symbol;
    if (rows.isEmpty()) return td;
    ...
    auto build = [&](const QList<QPair<QString,QString>>& keys, const QString& cat) {
        ...
        for (const auto& kv : keys) {
            ...
            if (!last.contains(col)) continue;
            auto jval = last[col];
            if (jval.isNull() || jval.isUndefined()) continue;
    ...
    auto tally = [&](const QVector<TechIndicator>& v) {
        for (const auto& ti : v) {
            switch (ti.signal) {
                case TechSignal::StrongBuy:  td.strong_buy++;  break;
                case TechSignal::Buy:        td.buy++;         break;
                case TechSignal::Neutral:    td.neutral++;     break;
                case TechSignal::Sell:       td.sell++;        break;
                case TechSignal::StrongSell: td.strong_sell++; break;
            }
        }
    };
    tally(td.trend); tally(td.momentum); tally(td.volatility); tally(td.volume);

    int bulls = td.strong_buy * 2 + td.buy;
    int bears = td.strong_sell * 2 + td.sell;
    if      (bulls > bears * 2) td.overall_signal = TechSignal::StrongBuy;
    else if (bulls > bears)     td.overall_signal = TechSignal::Buy;
    else if (bears > bulls * 2) td.overall_signal = TechSignal::StrongSell;
    else if (bears > bulls)     td.overall_signal = TechSignal::Sell;
    else                        td.overall_signal = TechSignal::Neutral;
