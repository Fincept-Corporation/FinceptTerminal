...
void BacktestingService::execute(const QString& provider, const QString& command,
    ...
    python::PythonRunner::instance().run(script, {command, json_str},
        [self, provider, command, ctx](python::PythonResult result) {
            if (!self) return;
            if (!result.success) {
...
void BacktestingService::list_strategies() {
    ...
    python::PythonRunner::instance().run(
        "Analytics/backtesting/fincept/fincept_provider.py",
        {"list_strategies"},
