...
inline QVector<Provider> all_providers() {
    return {
        {"vectorbt",      "VectorBT",       QColor("#00E5FF"),
        ...
        {"fasttrade",     "FastTrade",       QColor("#FFC400"),
         {"backtest"}},
        {"zipline",       "Zipline",         QColor("#FF3B8E"),
         {"backtest", "optimize", "walk_forward", "indicator", "indicator_signals"}},
        {"bt",            "BT",              QColor("#FF6B35"),
         {"backtest", "optimize", "walk_forward", "indicator", "indicator_signals"}},
        {"fincept",       "Fincept",         QColor("#d97706"),
...
inline QVector<Strategy> default_strategies() {
    return {
        ...
        {"dual_momentum", "Dual Momentum", "momentum", {
            {"period",   "Period",   12, 2, 50, 1},
            {"lookback", "Lookback", 6, 1, 24, 1},
        }},
        {"macd_zero_cross", "MACD Zero Cross", "momentum", {
            {"fast",   "Fast Period",   12, 2, 50, 1},
            {"slow",   "Slow Period",   26, 5, 100, 1},
            {"signal", "Signal Period",  9, 2, 50, 1},
        }},
        ...
        // ── Breakout ────────────────────────────────────────────────────
        {"donchian", "Donchian Breakout", "breakout", {
            {"period", "Period", 20, 5, 100, 1},
        }},
        {"volatility_bo", "Volatility Breakout", "breakout", {
            {"period",     "Period",     20, 5, 50, 1},
            {"multiplier", "Multiplier", 2.0, 0.5, 5.0, 0.1},
        }},
        {"atr_trailing", "ATR Trailing Stop", "breakout", {
            {"period",     "Period",     14, 5, 50, 1},
            {"multiplier", "Multiplier", 3.0, 1.0, 6.0, 0.5},
        }},
        {"keltner_breakout", "Keltner Breakout", "breakout", {
            {"period",     "Period",     20, 5, 50, 1},
            {"multiplier", "Multiplier", 1.5, 0.5, 4.0, 0.1},
        }},

        // ── Multi-Indicator ─────────────────────────────────────────────
        {"rsi_macd", "RSI + MACD", "multi", {
            {"rsi_period", "RSI Period",  14, 5, 50, 1},
            {"macd_fast",  "MACD Fast",   12, 2, 50, 1},
            {"macd_slow",  "MACD Slow",   26, 5, 100, 1},
        }},
        {"macd_adx", "MACD + ADX", "multi", {
            {"macd_fast",  "MACD Fast",   12, 2, 50, 1},
            {"adx_period", "ADX Period",  14, 5, 50, 1},
        }},
        {"bollinger_rsi", "Bollinger + RSI", "multi", {
            {"bb_period",  "BB Period",   20, 5, 50, 1},
            {"rsi_period", "RSI Period",  14, 5, 50, 1},
        }},
        {"trend_momentum", "Trend + Momentum", "multi", {
        ...
        {"inverse_vol", "Inverse Volatility", "portfolio", {
        ...
        {"target_vol", "Target Volatility", "portfolio", {
            {"targetVol", "Target Vol %", 15, 5, 30, 1},
            {"lookback",  "Lookback Days", 60, 20, 252, 5},
        }},
        {"min_variance", "Min Variance", "portfolio", {
            {"lookback", "Lookback Days", 252, 60, 504, 20},
        }},
    };
}
...
inline QStringList pct_metric_keys() {
    return {"total_return", "annualized_return", "max_drawdown", "win_rate",
            "volatility", "totalReturn", "annualizedReturn", "maxDrawdown",
            "winRate"};
}
...
inline QStringList count_metric_keys() {
    return {"total_trades", "winning_trades", "losing_trades",
            ...
            "winning_days", "losing_days", "consecutive_wins", "consecutive_losses"};
}
