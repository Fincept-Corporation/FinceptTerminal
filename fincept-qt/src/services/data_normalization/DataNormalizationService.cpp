...
void DataNormalizationService::fetch_and_normalize(const DataMapping& mapping,
...
NormalizedRecord DataNormalizationService::normalize_raw(const DataMapping& mapping,
                                                          const QJsonDocument& raw) {
    ...
    const QJsonArray field_mappings =
        QJsonDocument::fromJson(mapping.field_mappings_json.toUtf8()).array();

    const QJsonValue root = raw.isArray()
        ? QJsonValue(raw.array())
        : QJsonValue(raw.object());
    ...
    if (!record.errors.isEmpty()) {
        LOG_WARN(TAG, QString("Mapping '%1' produced %2 validation error(s)")
                          .arg(mapping.name).arg(record.errors.size()));
    }

    return record;
}

std::optional<NormalizedRecord> DataNormalizationService::latest_for_mapping(
    const QString& mapping_id) {
    auto r = Database::instance().execute(
        "SELECT id, mapping_id, source_id, schema_name, normalized_json, raw_json, "
        "       validation_errors, extracted_at "
        "FROM normalized_data WHERE mapping_id = ? "
        "ORDER BY extracted_at DESC LIMIT 1",
        {mapping_id});

    if (r.is_err() || !r.value().next()) return std::nullopt;
    ...
    for (const auto& e : errs) rec.errors << e.toString();
    rec.extracted_at = q.value(7).toString();
    return rec;
}

QVector<NormalizedRecord> DataNormalizationService::records_for_schema(
    const QString& schema_name) {
    auto r = Database::instance().execute(
        "SELECT id, mapping_id, source_id, schema_name, normalized_json, raw_json, "
        "       validation_errors, extracted_at "
...
QJsonValue DataNormalizationService::extract_jsonpath(const QJsonValue& root,
                                                       const QString& path) {
    ...
    for (const QString& token : tokens) {
        if (token.startsWith('[') && token.endsWith(']')) {
            ...
            if (inner == "*") {
                ...
                if (!current.isArray()) return QJsonValue::Undefined;
                return current; // caller handles the array
            }
            bool ok = false;
            int idx = inner.toInt(&ok);
            if (!ok) return QJsonValue::Undefined;
            if (!current.isArray()) return QJsonValue::Undefined;
            const QJsonArray arr = current.toArray();
            if (idx < 0 || idx >= arr.size()) return QJsonValue::Undefined;
            current = arr[idx];
        } else {
...
QJsonValue DataNormalizationService::apply_transform(const QJsonValue& val,
                                                      const QString& transform) {
    if (transform == "to_number") {
        if (val.isDouble()) return val;
        bool ok = false;
    ...
    if (transform == "unix_ms_to_iso") {
        qint64 ms = val.isDouble()
            ? static_cast<qint64>(val.toDouble())
            : val.toString().toLongLong();
        return QJsonValue(
            QDateTime::fromMSecsSinceEpoch(ms, Qt::UTC).toString(Qt::ISODate));
    }

    if (transform == "unix_s_to_iso") {
        qint64 s = val.isDouble()
            ? static_cast<qint64>(val.toDouble())
            : val.toString().toLongLong();
        return QJsonValue(
            QDateTime::fromSecsSinceEpoch(s, Qt::UTC).toString(Qt::ISODate));
    }
...
QStringList DataNormalizationService::validate_schema(const QJsonObject& data,
                                                       const QString& schema_name) {
    QStringList errors;
    const auto it = kRequiredFields.find(schema_name);
    if (it == kRequiredFields.end()) return errors; // unknown schema, skip
...
void DataNormalizationService::persist(const DataMapping& mapping,
                                        const NormalizedRecord& record) {
    const QString norm_json =
        QJsonDocument(record.normalized).toJson(QJsonDocument::Compact);
    const QString raw_json =
        QJsonDocument(record.raw).toJson(QJsonDocument::Compact);

    QJsonArray err_array;
    for (const QString& e : record.errors) err_array.append(e);
    const QString err_json =
        QJsonDocument(err_array).toJson(QJsonDocument::Compact);

    const QString hash = QCryptographicHash::hash(
        norm_json.toUtf8(), QCryptographicHash::Sha256).toHex();
    ...
    auto r = Database::instance().execute(
        ...
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        {record.id, mapping.id, mapping.source_id, mapping.schema_name,
         norm_json, raw_json, err_json, hash});

    if (r.is_err()) {
        LOG_ERROR(TAG, QString("Failed to persist normalized record: %1")
