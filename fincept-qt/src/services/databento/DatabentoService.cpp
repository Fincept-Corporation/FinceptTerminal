...
static QString extract_json_from_output(const QString& output) {
    ...
    for (int i = output.size() - 1; i >= 0; --i) {
        if (output[i] == '}' || output[i] == ']') {
            ...
            int depth = 0;
            for (int j = i; j >= 0; --j) {
                if (output[j] == close) ++depth;
                else if (output[j] == open) { --depth; if (depth == 0) { start = j; break; } }
            }
            if (start >= 0) return output.mid(start, i - start + 1);
            break;
        }
    }
...
void DatabentoService::run_script(const QStringList& args,
                                   std::function<void(bool, const QJsonObject&)> cb)
{
    ...
    PythonRunner::instance().run(SCRIPT, full_args,
        [self, cb](PythonResult result) {
            if (!self) return;
            if (!result.success) {
...
void DatabentoService::test_connection() {
    ...
    QPointer<DatabentoService> self = this;
    run_script({"--action", "test_connection"},
...
void DatabentoService::fetch_ohlcv(const QStringList& symbols, int days) {
    ...
    QPointer<DatabentoService> self = this;
    QStringList args = {"--action", "get_historical_ohlcv",
...
void DatabentoService::fetch_options_surface(const QString& symbol, float spot) {
    ...
    QPointer<DatabentoService> self = this;
    QStringList args = {"--action",  "get_options_definitions",
                        "--symbols", symbol,
                        "--spot",    QString::number((double)spot, 'f', 2)};
...
void DatabentoService::fetch_futures_term_structure(const QStringList& commodities) {
    ...
    QPointer<DatabentoService> self = this;
    QStringList args = {"--action",  "get_futures_term_structure",
...
// ── Parsers ────────────────────────────────────────────────────────────────
surface::VolatilitySurfaceData DatabentoService::parse_vol_surface(
    const QJsonObject& j, const std::string& symbol, float spot)
{
    ...
    if (strikes_set.empty()) return data;
...
surface::GreeksSurfaceData DatabentoService::parse_greek_surface(
    const QJsonObject& j, const std::string& greek, const std::string& symbol, float spot)
{
    ...
    QJsonArray opts = j["options"].toArray();
    if (opts.isEmpty()) opts = j["data"].toArray();
...
surface::SkewSurfaceData DatabentoService::parse_skew(
    const QJsonObject& j, const std::string& symbol)
{
    ...
    if (skew_arr.isEmpty()) return data;

    std::set<int> dtes_set;
    for (const auto& v : skew_arr) {
        int dte = v.toObject()["dte"].toInt();
        if (dte > 0) dtes_set.insert(dte);
    }
    data.expirations.assign(dtes_set.begin(), dtes_set.end());
...
surface::CommodityForwardData DatabentoService::parse_commodity_forward(const QJsonObject& j) {
    ...
    for (const QString& sym : term.keys()) {
        ...
        for (int i = 0; i < curve.size(); i++) months_set.insert(i+1);
    }
    ...
    for (const QString& sym : term.keys()) {
        ...
        for (int i = 0; i < (int)data.contract_months.size(); i++) {
            if (i < curve.size())
                row.push_back((float)curve[i].toObject()["price"].toDouble(
                                curve[i].toDouble()));
            else
                row.push_back(0.0f);
        }
...
surface::ContangoData DatabentoService::parse_contango(const QJsonObject& j) {
    ...
    for (const QString& sym : term.keys()) {
        QJsonArray curve = term[sym].toArray();
        float spot_price = curve.isEmpty() ? 1.0f :
            (float)curve[0].toObject()["price"].toDouble(curve[0].toDouble());
        if (spot_price == 0) spot_price = 1.0f;

        std::vector<float> row;
        for (int i = 0; i < (int)data.contract_months.size(); i++) {
            float price = (i < curve.size()) ?
                (float)curve[i].toObject()["price"].toDouble(curve[i].toDouble()) : spot_price;
