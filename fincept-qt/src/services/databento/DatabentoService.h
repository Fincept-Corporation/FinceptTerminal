...
private:
    DatabentoService();
    void run_script(const QStringList& args,
                    std::function<void(bool, const QJsonObject&)> cb);

    static surface::VolatilitySurfaceData  parse_vol_surface(const QJsonObject& j, const std::string& symbol, float spot);
    static surface::GreeksSurfaceData      parse_greek_surface(const QJsonObject& j, const std::string& greek, const std::string& symbol, float spot);
