"""
European Bank for Reconstruction and Development (EBRD) Data Fetcher
Transition indicators, project data, and economic data for Eastern Europe
and Central Asia from the EBRD open data portal.
"""
import sys
import json
import os
...
API_KEY = os.environ.get('EBRD_API_KEY', '')
BASE_URL = "https://data.ebrd.com/api"
...
def get_transition_indicators(country: str = None, year: str = None) -> Any:
    params = {}
    if country:
        params["country"] = country
    if year:
        params["year"] = year
    return _make_request("transition-indicators", params)
...
def get_economic_data(country: str = None, indicator: str = None, year: str = None) -> Any:
    params = {}
    if country:
        params["country"] = country
    if indicator:
    ...
    if year:
        params["year"] = year
    return _make_request("economic-data", params)
...
def main(args=None):
    ...
    if command == "transition":
        ...
        result = get_transition_indicators(country, year)
        ...
        result = get_economic_data(country, indicator, year)
