"""
World Justice Rule of Law Data Fetcher
World Justice Project Rule of Law Index: rule of law scores for 140 countries across 8 factors.
"""
import sys
import json
import os
...
BASE_URL = "https://worldjusticeproject.org/rule-of-law-index/api"
...
def get_scores(year: int = 2023, country: str = None) -> Any:
    params = {"year": year}
    if country:
...
def get_rankings(year: int = 2023, factor: str = None) -> Any:
    params = {"year": year}
    if factor:
...
def get_factors(country: str, year: int = 2023) -> Any:
    params = {"country": country, "year": year}
    return _make_request("factors", params=params)


def get_regional_averages(year: int = 2023, region: str = None) -> Any:
    params = {"year": year}
    if region:
...
def main(args=None):
    ...
    if command == "countries":
        ...
        result = get_scores(year, country)
    elif command == "rankings":
        year = int(args[1]) if len(args) > 1 else 2023
        factor = args[2] if len(args) > 2 else None
        result = get_rankings(year, factor)
    elif command == "trend":
        ...
        result = get_factors(country, year)
