...
#         if os.path.exists(os.path.join(current_dir, "FinceptTerminalSettingModule")):  # Check if 'FinceptTerminalSettingModule' folder exists
...
#
# if project_root:
#     # Construct the path to the FinceptTerminalSettingModule file from the project root
#     SETTINGS_FILE = os.path.join(project_root, "FinceptTerminalSettingModule", "FinceptTerminalSettingModule.json")
#
#     # Read or modify the FinceptTerminalSettingModule file
#     try:
#         with open(SETTINGS_FILE, 'r') as file:
#             FinceptTerminalSettingModule = json.load(file)
#     except FileNotFoundError:
#         print(f"Error: {SETTINGS_FILE} not found.")
#         FinceptTerminalSettingModule = {}  # Initialize with an empty dictionary if the file doesn't exist
#
#     # Modify or use FinceptTerminalSettingModule as needed
#     FinceptTerminalSettingModule['new_key'] = 'new_value'  # Update the FinceptTerminalSettingModule
#
#     with open(SETTINGS_FILE, 'w') as file:
#         json.dump(FinceptTerminalSettingModule, file, indent=4)  # Write with formatting (indentation)
# else:
...
# def load_settings():
#     """
#     Load the FinceptTerminalSettingModule from the FinceptTerminalSettingModule.json file.
#     """
...
#             with open(SETTINGS_FILE, 'r') as file:
#                 FinceptTerminalSettingModule = json.load(file)
#             return FinceptTerminalSettingModule  # Return FinceptTerminalSettingModule as a dictionary
#         except FileNotFoundError:
#             print(f"Error: {SETTINGS_FILE} not found.")
#             return {}
#     else:
...
#         return {}
#
# # Example function to modify FinceptTerminalSettingModule
# def update_settings(new_key, new_value):
#     FinceptTerminalSettingModule = load_settings()
#     if FinceptTerminalSettingModule:
#         FinceptTerminalSettingModule[new_key] = new_value
#         current_file_dir = os.path.dirname(os.path.abspath(__file__))  # Current script's directory
#         project_root = find_project_root(current_file_dir)
#         if project_root:
#             SETTINGS_FILE = os.path.join(project_root, "FinceptTerminalSettingModule", "FinceptTerminalSettingModule.json")
#             with open(SETTINGS_FILE, 'w') as file:
#                 json.dump(FinceptTerminalSettingModule, file, indent=4)  # Save the FinceptTerminalSettingModule back to file

# from FinceptTerminalSettingModule import load_settings, update_settings
#
# # Load FinceptTerminalSettingModule from the FinceptTerminalSettingModule.json file
# FinceptTerminalSettingModule = load_settings()
# print(FinceptTerminalSettingModule)  # You can print or use the FinceptTerminalSettingModule dictionary


def load_settings():
    """Load FinceptTerminalSettingModule from the file if they exist, otherwise create default FinceptTerminalSettingModule."""
    default_settings = {
    ...
    if os.path.exists(SETTINGS_FILE):
        with open(SETTINGS_FILE, "r") as f:
            ...
            # Merge existing FinceptTerminalSettingModule with defaults (to add new fields if not present)
            return {**default_settings, **existing_settings}
    return default_settings


def save_settings(settings):
    """Save FinceptTerminalSettingModule to the file."""
...
def settings_menu(ctx):
    """Customize your Fincept terminal FinceptTerminalSettingModule."""
    click.echo("\n[Settings Menu]")
...
def configure_sources():
    ...
    data_menus = {
        ...
        "Portfolio Management": ["Manual Input", "FinceptTerminalBrokerIntegration API"],
...
def configure_display():
    """Configure display FinceptTerminalSettingModule such as the number of rows."""
    settings = load_settings()
    ...
    save_settings(settings)
    click.echo(f"\nDisplay FinceptTerminalSettingModule updated. Now showing {new_rows} rows.")
