...
import os
import json
from fincept_terminal.FinceptTerminalUtils import update_checker
from fincept_terminal.FinceptTerminalUtils.display import display_art
...
SETTINGS_FOLDER = os.path.join(BASE_DIR, "FinceptTerminalSettings")
SETTINGS_FILE = os.path.join(SETTINGS_FOLDER, "FinceptTerminalSettings.json")
...
def handle_registration():
    ...
    # Step 4: Save user FinceptTerminalSettings to file
    console.print("[cyan]Saving user FinceptTerminalSettings...[/cyan]")
...
def save_user_settings(api_key, email, username):
    ...
    console.print(f"[green]User FinceptTerminalSettings saved to {SETTINGS_FILE}[/green]")
...
def show_main_menu():
    ...
    if choice == 1:
        ...
        from fincept_terminal.FinceptTerminalPortfolioModule.portfolio import show_portfolio_menu
        show_portfolio_menu()
    elif choice == 11:
        console.print("[bold red] Feature still in development [/bold red]", style="danger")
        show_main_menu()
        ...
        from fincept_terminal.FinceptTerminalPortfolioModule.portfolio import show_backtesting_menu
        ...
        from fincept_terminal.FinceptTerminalGenAI.GenAI import show_genai_query
        show_genai_query()
    elif choice == 19:
        from fincept_terminal.FinceptTerminalLang.finscript import show_finscript_menu
        show_finscript_menu()
    elif choice == 20:
        # Call the FinceptTerminalSettings menu
...
def show_settings_menu():
    ...
    while True:  # Use a loop to keep returning to the FinceptTerminalSettings menu after each operation
        ...
        # Display FinceptTerminalSettings options in columns
        ...
        try:
            ...
            console.print("[bold red]Invalid input. Please enter a valid option number.[/bold red]")
            continue  # Show the FinceptTerminalSettings menu again on invalid input

        console.print("\n")

        # Handle FinceptTerminalSettings menu choices
        if choice == 1:
            from fincept_terminal.FinceptTerminalSettings.settings import configure_sources
            configure_sources()
        elif choice == 2:
            from fincept_terminal.FinceptTerminalSettings.settings import change_theme
            change_theme()
        elif choice == 3:
            from fincept_terminal.FinceptTerminalSettings.settings import configure_display
            configure_display()
        elif choice == 4:
            from fincept_terminal.FinceptTerminalSettings.settings import toggle_notifications
            toggle_notifications()
        elif choice == 5:
            from fincept_terminal.FinceptTerminalSettings.settings import toggle_auto_update
            toggle_auto_update()
        elif choice == 6:
...
def display_current_configurations():
    ...
    # Load the current FinceptTerminalSettings from the FinceptTerminalSettings file
    settings = load_settings()

    # Create a table to display FinceptTerminalSettings in a structured way
    ...
    # Add FinceptTerminalSettings and their values to the table
