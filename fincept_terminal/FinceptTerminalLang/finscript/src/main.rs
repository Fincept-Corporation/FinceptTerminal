...
impl fmt::Display for ASTNode {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ASTNode::CandlestickChart { symbol, label } => write!(f, "Candlestick Chart for {}: {}", symbol, label), // ✅ FIXED
        }
    }
}
            ...
            match node {
                ASTNode::CandlestickChart { symbol, label } => {
                    ...
                    let ohlc: Vec<_> = quotes.iter().map(|q| {
                        (q.timestamp as i64, q.open, q.high, q.low, q.close) // Convert u64 -> i64
                    }).collect();

                    // Plot candlestick chart
                    plot_candlestick_chart(&symbol, &label, &ohlc[..]); // Convert Vec to slice
                    None
                }
                ASTNode::Assignment { variable, value } => {
...
fn plot_candlestick_chart(symbol: &str, label: &str, ohlc_data: &[(i64, f64, f64, f64, f64)]) {
    ...
    chart.draw_series(ohlc_data.iter().enumerate().map(|(i, &(timestamp, open, high, low, close))| {
        CandleStick::new(
            ...
            3 // ✅ FIXED: Added missing bar width argument
        )
    })).unwrap();
    ...
    while let Some(&ch) = chars.peek() {
        match ch {
            ...
            _ if ch.is_digit(10) || ch == '.' => {
                ...
                let mut has_dot = false; // Track decimal point
                ...
                while let Some(&c) = chars.peek() {
                    if c.is_digit(10) {
                        ...
                        if has_dot { break; } // Prevent multiple dots
                        has_dot = true;
                        number.push(c);
                        chars.next();
                    } else {
            ...
            _ if ch.is_alphabetic() || ch == '_' || ch == '.' => {  // ✅ Allow `RITES.NS` and `AAPL`
                let mut identifier = String::new();
                while let Some(&c) = chars.peek() {
                    if c.is_alphabetic() || c.is_digit(10) || c == '_' || c == '.' {  // ✅ Fix: Allow dot in symbols
                        identifier.push(c);
                        chars.next();
                    } else {
                        break;
                ...
                match identifier.as_str() {
                    ...
                    _ => tokens.push(Token::Identifier(identifier)),  // ✅ Correctly classify `AAPL` and `RITES.NS`
                }
            }
...
fn parse(tokens: Vec<Token>) -> Vec<ASTNode> {
    ...
    while let Some(token) = iter.next() {
        match token {
            ...
            Token::Keyword(ref keyword) if keyword == "if" => {
                let left_expr = parse_expression(&mut iter);
                if let Some(Token::Operator(op)) = iter.next() {
                    let right_expr = parse_expression(&mut iter);
    ...
    } // ✅ Correctly closed `while let Some(token) = iter.next()`
...
fn parse_expression(iter: &mut std::iter::Peekable<std::vec::IntoIter<Token>>) -> ASTNode {
    if let Some(Token::Identifier(func_name)) = iter.next() {
        ...
        // ✅ Ensure that a function call is properly enclosed in ()
        if let Some(Token::OpenParen) = iter.next() {
            while let Some(arg_token) = iter.next() {
                match arg_token {
                    ...
                    Token::Number(num) => args.push(ASTNode::Number(num)),        // ✅ Fix: Handle numbers
                    Token::CloseParen => break,                                   // ✅ Properly close function call
                    _ => {
            ...
            panic!("Expected '(' after function name '{}', but got {:?}", func_name, iter.peek());
        }
    }
