...
pub async fn execute_datagovsg_command(
    ...
    if !script_path.exists() {
        return Err(format!(
            "Singapore Government script not found at: {}",
            script_path.display()
        ));
    }
    ...
    let output = Command::new(&python_path)
        .args(&cmd_args)
        .output()
        .map_err(|e| format!("Failed to execute Singapore Government command: {}", e))?;
    ...
    if output.status.success() {
        ...
        Err(format!("Singapore Government command failed: {}", stderr))
    }
}
