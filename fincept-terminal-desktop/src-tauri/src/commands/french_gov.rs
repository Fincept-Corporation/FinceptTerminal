...
pub async fn execute_french_gov_command(
    ...
    let script_path = get_script_path(&app, "french_gov_api.py")?;

    if !script_path.exists() {
        return Err(format!(
            "French Government script not found at: {}",
            script_path.display()
        ));
    }
    ...
    let output = Command::new(&python_path)
        .args(&cmd_args)
        .output()
        .map_err(|e| format!("Failed to execute French Government command: {}", e))?;
    ...
    if output.status.success() {
        ...
        Err(format!("French Government command failed: {}", stderr))
    }
}
