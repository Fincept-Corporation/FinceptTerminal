...
pub async fn execute_datagov_au_command(
    ...
    if !script_path.exists() {
        return Err(format!(
            "Australian Government script not found at: {}",
            script_path.display()
        ));
    }
    ...
    let output = Command::new(&python_path)
        .args(&cmd_args)
        .output()
        .map_err(|e| format!("Failed to execute Australian Government command: {}", e))?;
    ...
    if output.status.success() {
        ...
        Err(format!("Australian Government command failed: {}", stderr))
    }
}
