#!/usr/bin/env python3
"""
Data.gov API Wrapper - U.S. Government Open Data Portal API

A wrapper for the Data.gov (CKAN-based) API to access U.S. federal government data.
...
- Data.gov Developer Resources: https://data.gov/developers/apis/

Environment Variables:
- DATA_GOV_API_KEY: Optional API key for enhanced access (optional)
"""
...
API_KEY = os.environ.get('DATA_GOV_API_KEY')  # Optional API key
TIMEOUT = 30
...
def _make_request(action: str, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        headers = {
            ...
            'User-Agent': 'Fincept-Terminal/1.0 (Data.gov API Wrapper)'
        }
        ...
        return {
            ...
            "error": "Request timeout - Data.gov API took too long to respond"
        }
        ...
        return {
            ...
            "error": "Connection error - unable to reach Data.gov API"
        }
        ...
        return {
            ...
            "error": "Invalid JSON response from Data.gov API"
        }
...
def get_organizations() -> Dict[str, Any]:
    """
    Get all organizations (data providers/catalogs) from Data.gov
    This provides the catalog level - all U.S. government agencies publishing data
    ...
    try:
        ...
        result = _make_request('organization_list', {'all_fields': True})
        ...
        enhanced_orgs = []
        raw_orgs = result.get("data", [])

        for org in raw_orgs:
            enhanced_org = {
                "id": org.get("id", ""),
                "name": org.get("name", ""),
                "title": org.get("title", org.get("display_name", "")),
                ...
                "display_name": org.get("title", org.get("display_name", org.get("name", "")))
            }
        ...
        result["metadata"]["description"] = "List of all U.S. government agencies publishing data on Data.gov"
...
def search_datasets(query: str = "", rows: int = 50, start: int = 0,
                   organization: Optional[str] = None, tags: Optional[List[str]] = None,
                   format_filter: Optional[str] = None, sort: str = "relevance desc") -> Dict[str, Any]:
    """
    ...
    Args:
        ...
        format_filter: Filter by file format (e.g., CSV, JSON)
        sort: Sort order (default 'relevance desc')
    ...
    try:
        ...
        # Build filtered query components
        fq_parts = []
...
def get_recent_datasets(limit: int = 50) -> Dict[str, Any]:
    ...
    """
    try:
        result = search_datasets(rows=limit, sort="metadata_modified desc")
...
def list_all_dataset_names() -> Dict[str, Any]:
    ...
    try:
        ...
        result["metadata"]["description"] = "Simple list of all dataset names on Data.gov"
...
def get_agency_datasets_by_format(agency_id: str, format_type: str, limit: int = 50) -> Dict[str, Any]:
    """
    Get datasets from a specific agency filtered by file format

    Args:
        agency_id: The ID or name of the agency
        format_type: File format to filter by (e.g., CSV, JSON, XML)
        limit: Maximum number of datasets to return

    Returns:
        List of datasets from the specified agency in the specified format
    """
    try:
        result = search_datasets(organization=agency_id, format_filter=format_type, rows=limit)

        if result["error"]:
            return result

        result["metadata"]["agency_id"] = agency_id
        result["metadata"]["format_type"] = format_type
        result["metadata"]["description"] = f"Datasets from {agency_id} in {format_type} format (limit: {limit})"
        ...
        return {
            "data": {},
            "metadata": {"agency_id": agency_id, "format_type": format_type},
            "error": f"Error fetching agency datasets by format: {str(e)}"
        }
...
def main():
    """
    CLI interface for Data.gov API Wrapper

    Usage:
        python datagov_us_api.py <command> [arguments]
    ...
    Available Commands:
        ...
        agency-format <agency> <format> [limit] - Get datasets from agency by format
    """

    if len(sys.argv) < 2:
        print(json.dumps({
            "error": "Usage: python datagov_us_api.py <command> [arguments]",
            "available_commands": [
                "organizations - List all data publishing organizations",
                "org-details <id> - Get details for a specific organization",
                "search <query> [rows] - Search datasets with optional result limit",
            ...
            "examples": [
                "python datagov_us_api.py organizations",
                "python datagov_us_api.py search \"climate\" 25",
                "python datagov_us_api.py dataset-details consumer-complaint-database",
                "python datagov_us_api.py org-datasets noaa-gov",
                "python datagov_us_api.py recent 20",
                "python datagov_us_api.py agency-format noaa-gov CSV 30"
            ],
            "environment_variables": {
                "DATA_GOV_API_KEY": "Optional API key for enhanced rate limits"
            }
        }))
        sys.exit(1)
    ...
    try:
        if command == "organizations":
            ...
            if len(sys.argv) < 3:
                print(json.dumps({
                    ...
                    "usage": "python datagov_us_api.py org-details <organization_id>"
                }))
                sys.exit(1)
            ...
            if len(sys.argv) < 3:
                print(json.dumps({
                    ...
                    "usage": "python datagov_us_api.py search <query> [rows]"
                }))
                sys.exit(1)
            query = sys.argv[2]
            ...
            if len(sys.argv) < 3:
                print(json.dumps({
                    ...
                    "usage": "python datagov_us_api.py dataset-details <dataset_id>"
                }))
                sys.exit(1)
            ...
            if len(sys.argv) < 3:
                print(json.dumps({
                    "error": "Organization ID required",
                    "usage": "python datagov_us_api.py org-datasets <organization_id> [rows]"
                }))
                sys.exit(1)
            org_id = sys.argv[2]
            ...
            if len(sys.argv) < 3:
                print(json.dumps({
                    ...
                    "usage": "python datagov_us_api.py resource-details <resource_id>"
                }))
                sys.exit(1)
        ...
        elif command == "agency-format":
            if len(sys.argv) < 4:
                print(json.dumps({
                    "error": "Agency ID and format type required",
                    "usage": "python datagov_us_api.py agency-format <agency_id> <format> [limit]"
                }))
                sys.exit(1)
            agency_id = sys.argv[2]
            format_type = sys.argv[3]
            limit = int(sys.argv[4]) if len(sys.argv) > 4 else 50
            result = get_agency_datasets_by_format(agency_id, format_type, limit)
            ...
            print(json.dumps({
                ...
                "available_commands": [
                    ...
                    "all-datasets", "list-names", "agency-format"
                ]
            }))
            sys.exit(1)
