...
export interface SegmentData {
  name: string;
  value: number;
  type: 'geographic' | 'business_unit' | 'product';
  period?: string;
}

export interface DebtHolder {
  name: string;
  type: 'bank_lender' | 'bond_trustee' | 'credit_facility';
  filing_date?: string;
  description?: string;
  source?: string;
}
...
export interface CorporateEventV2 {
  ...
  items: string[];
  description: string;
  category: 'earnings' | 'management' | 'merger' | 'restructuring' | 'governance' | 'acquisition' | 'material_agreement' | 'financial_results' | 'debt' | 'asset' | 'other';
}
