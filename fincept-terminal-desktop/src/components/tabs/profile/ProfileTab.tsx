...
// Support Section - Feedback, Contact, Newsletter
const SupportSection: React.FC = () => {
  ...
  const handleNewsletterSubmit = async () => {
    ...
    if (result.success) {
      ...
      setTimeout(() => resetNewsletter(), 3000);
    } else {
  ...
  const InputField: React.FC<{
    ...
    <div style={{ marginBottom: '12px' }}>
      <label style={{ color: COLORS.WHITE, fontSize: '10px', display: 'block', marginBottom: '4px' }}>
  ...
  const SubmitButton: React.FC<{
    ...
    text: string;
    color?: string;
  }> = ({ onClick, loading, success, text, color = COLORS.ORANGE }) => (
    <button
      onClick={onClick}
      disabled={loading}
      style={{
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
      {/* Feedback Form */}
      <DataPanel title="SEND FEEDBACK" icon={<MessageSquarePlus size={16} />}>
        <InputField
          label="Name"
          value={feedbackName}
      ...
      {/* Contact Form */}
      <DataPanel title="CONTACT US" icon={<Mail size={16} />}>
        <InputField
          label="Name"
          value={contactName}
      ...
      <div style={{ gridColumn: '1 / -1' }}>
        <DataPanel title="NEWSLETTER SUBSCRIPTION" icon={<Newspaper size={16} />}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: '12px', alignItems: 'flex-end' }}>
            <div>
              <p style={{ color: COLORS.MUTED, fontSize: '10px', marginBottom: '12px', lineHeight: '1.5' }}>
              ...
              />
              {newsletterError && (
                <div style={{ color: COLORS.RED, fontSize: '10px', marginBottom: '8px' }}>{newsletterError}</div>
              )}
            </div>
            <div style={{ paddingBottom: newsletterError ? '28px' : '0' }}>
              <button
                ...
                style={{
                  backgroundColor: newsletterSuccess ? COLORS.GREEN : '#10b981',
                  color: COLORS.WHITE,
                  border: 'none',
                  padding: '10px 24px',
                  fontSize: '11px',
              ...
              >
                {newsletterLoading ? (
                  ...
                  <>
                    <Newspaper size={14} /> SUBSCRIBE
                  </>
                )}
              </button>
            </div>
          </div>
          <p style={{ color: COLORS.MUTED, fontSize: '9px', marginTop: '8px' }}>
