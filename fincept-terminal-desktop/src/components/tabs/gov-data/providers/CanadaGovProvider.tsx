// CanadaGovProvider - Government of Canada Open Data portal interface
...
import { useCanadaGovData } from '../hooks/useCanadaGovData';
import type { CanadaGovView, CanadaDataset, CanadaResource } from '../types';

const VIEWS: { id: CanadaGovView; label: string }[] = [
...
export function CanadaGovProvider() {
  const {
  ...
  const handleViewChange = (view: CanadaGovView) => {
    setActiveView(view);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{
        ...
        {/* Search input (shown when search view) */}
        {activeView === 'search' && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flex: 1, maxWidth: '400px' }}>
            ...
            <input
              ...
              placeholder="Search Canadian datasets..."
        ...
        {(activeView === 'datasets' && selectedPublisher) && (
          <div style={{ fontSize: '9px', color: FINCEPT.GRAY, display: 'flex', alignItems: 'center', gap: '4px' }}>
            ...
            <span style={{ color: PROVIDER_COLOR }}>{selectedPublisher}</span>
      ...
      <div style={{ flex: 1, overflow: 'auto' }}>
        {loading ? (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%' }}>
            ...
            <div style={{ fontSize: '12px', color: FINCEPT.WHITE }}>Loading Canadian government data...</div>
          </div>
        ) : error ? (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%' }}>
          ...
          searchResults.length > 0
            ...
            : <EmptyState icon={Search} message="Search Canadian government datasets" sub="Enter a query above and press SEARCH or Enter" />
        ) : activeView === 'recent' ? (
...
function PublishersTable({ publishers, onSelect }: { publishers: { id: string; name: string }[]; onSelect: (id: string) => void }) {
  return (
    <div style={{ padding: '0' }}>
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        ...
        <tbody>
          {publishers.map(p => (
            ...
            >
              <td style={{ ...tdStyle, color: PROVIDER_COLOR, fontWeight: 600 }}>{p.id}</td>
...
function DatasetsTable({ datasets, onSelect, totalCount }: { datasets: CanadaDataset[]; onSelect: (id: string) => void; totalCount: number }) {
  return (
    <div>
...
function ResourcesTable({ resources }: { resources: CanadaResource[] }) {
  const handleOpenUrl = useCallback(async (url: string) => {
    try {
      const { openUrl } = await import('@tauri-apps/plugin-opener');
      await openUrl(url);
    } catch {
      // Fallback for dev mode / non-Tauri context
...
export default CanadaGovProvider;
