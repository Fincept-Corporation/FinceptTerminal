...
export class DuckDBAdapter extends BaseAdapter {
  async testConnection(): Promise<TestConnectionResult> {
    try {
      ...
      const readOnly = this.getConfig<boolean>('readOnly', false);

      if (!filePath && !inMemory) {
        return this.createErrorResult('Either file path or in-memory flag is required');
      }

      console.log('Validating DuckDB configuration (client-side only)');

      return this.createSuccessResult(
        `Configuration validated for DuckDB ${inMemory ? '(in-memory)' : `"${filePath}"`}`,
        {
  ...
  async query(sql: string): Promise<any> {
    try {
      ...
      throw new Error(`DuckDB query error: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
