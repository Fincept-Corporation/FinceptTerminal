...
このスクリプトは、コンパイラ チェック、CMake、Ninja、Python、vcpkg、すべての C++ 依存関係、ビルド、起動を処理します。

* * *

## ダウンロードして実行 (ビルドは必要ありません)

事前に構築されたバイナリは、[リリースページ](https://github.com/Fincept-Corporation/FinceptTerminal/releases).
...
| **忍者**       | 最新    | `winget install Ninja-build.Ninja`                           | `apt install ninja-build` | `brew install ninja`               |
| **C++コンパイラ** | C++20 | MSVC 2022 ([ビジュアルスタジオ](https://visualstudio.microsoft.com/)) | `apt install g++`         | Xcode CLT:`xcode-select --install` |
| **vcpkg**    | 最新    | 以下を参照してください                                                  | 以下を参照してください               | 以下を参照してください                        |
...
すべての依存関係は vcpkg によって自動的にインストールされます。
...
**フィンセプトターミナル**は、従来のソフトウェアによる制限を拒否する人のために構築されたオープンソースの金融プラットフォームです。私たちは次の点で競争します**分析の深さ**そして**データへのアクセシビリティ**— インサイダー情報や限定フィードではありません。
