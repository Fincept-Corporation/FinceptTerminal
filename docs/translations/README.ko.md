...
**Fincept 터미널 vch**순수 네이티브 C++20 데스크톱 애플리케이션으로, 이전 Tauri/React/Rust 스택을 완전히 다시 작성했습니다. 그것은 사용한다**Dear ImGui**GPU 가속 UI의 경우,**GLFW + OpenGL**렌더링용, 임베디드**파이썬**분석을 위해 단일 네이티브 바이너리로 Bloomberg 터미널 수준의 성능을 제공합니다.
...
스크립트는 컴파일러 검사, CMake, Ninja, Python, vcpkg, 모든 C++ 종속성, 빌드 및 실행을 처리합니다.

* * *

## 다운로드 및 실행(빌드 필요 없음)

사전 구축된 바이너리는 다음에서 사용할 수 있습니다.[릴리스 페이지](https://github.com/Fincept-Corporation/FinceptTerminal/releases).

| Platform          | 다운로드                                     | 달리다                       |
| ----------------- | ---------------------------------------- | ------------------------- |
| **윈도우 x64**       | `FinceptTerminal-Windows-x64.zip`        | 추출 →`FinceptTerminal.exe` |
| **리눅스 x64**       | `FinceptTerminal-Linux-x64.tar.gz`       | 추출 →`./FinceptTerminal`   |
...
설치가 필요하지 않습니다. 추출하고 실행하기만 하면 됩니다.
...
| **닌자**       | 최신    | `winget install Ninja-build.Ninja`                          | `apt install ninja-build` | `brew install ninja`               |
| **C++ 컴파일러** | C++20 | MSVC 2022 ([비주얼 스튜디오](https://visualstudio.microsoft.com/)) | `apt install g++`         | Xcode CLT:`xcode-select --install` |
| **vcpkg**    | 최신    | 아래를 참조하세요                                                   | 아래를 참조하세요                 | 아래를 참조하세요                          |
