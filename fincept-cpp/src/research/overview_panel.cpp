...
void OverviewPanel::render(ResearchData& data, ChartPeriod& period) {
    ...
    float col1_w = avail.x * 0.20f;
    float col4_w = avail.x * 0.20f;
    float chart_w = avail.x - col1_w - col4_w - 16; // 16px for spacing
    float top_h = avail.y * 0.65f;

    // ── Top row: 3 columns ──
    ImGui::BeginChild("##ov_col1", ImVec2(col1_w, top_h), true);
    render_trading_valuation(q, si, col1_w);
    ImGui::EndChild();

    ImGui::SameLine(0, 4);

    ImGui::BeginChild("##ov_chart", ImVec2(chart_w, top_h), true);
    render_chart(hist, period, chart_w, top_h - 40);
    ImGui::EndChild();

    ImGui::SameLine(0, 4);

    ImGui::BeginChild("##ov_col4", ImVec2(col4_w, top_h), true);
    render_analyst_performance(si, col4_w);
    ImGui::EndChild();

    // ── Bottom: Company overview ──
    ImGui::Spacing();
    ImGui::BeginChild("##ov_bottom", ImVec2(avail.x, 0), true);
...
void OverviewPanel::render_company_overview(const StockInfo& si) {
    ...
    float desc_w = avail_w * 0.65f;
    float info_w = avail_w - desc_w - 8;
