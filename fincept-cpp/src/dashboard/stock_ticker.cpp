...
static std::string find_script_path() {
    ...
    for (auto& path : candidates) {
        if (std::filesystem::exists(path)) {
            ...
            std::printf("[StockTicker] Found script: %s\n", canonical.c_str());
            return canonical;
        }
    }

    std::printf("[StockTicker] yfinance_data.py not found!\n");
    return "";
}
...
static std::string run_python(const std::string& script_path, const std::string& args) {
    ...
    std::printf("[StockTicker] Running: %s \"%s\" %s\n", python_cmd, script_path.c_str(), args.c_str());
    ...
    if (!CreatePipe(&hReadPipe, &hWritePipe, &sa, 0)) {
        std::printf("[StockTicker] CreatePipe failed\n");
        return "";
    }
    ...
    if (!CreateProcessA(nullptr, cmd_buf, nullptr, nullptr, TRUE, CREATE_NO_WINDOW, nullptr, nullptr, &si, &pi)) {
        std::printf("[StockTicker] CreateProcess failed (error %lu)\n", GetLastError());
...
void StockTicker::initialize() {
    ...
    std::printf("[StockTicker] Initialized with %zu symbols\n", TICKER_SYMBOLS.size());
}
...
void StockTicker::fetch_prices() {
    ...
    if (script.empty()) {
        std::printf("[StockTicker] Cannot fetch — script not found\n");
        return;
    }
    ...
    if (output.empty()) {
        std::printf("[StockTicker] No output from python script\n");
        return;
    }
    ...
    if (json_str.empty()) {
        std::printf("[StockTicker] No JSON found in output. Raw output:\n%s\n", output.c_str());
        return;
    }
    ...
    try {
        ...
        if (data.is_array()) {
            ...
            fetched_ = true;
            std::printf("[StockTicker] Loaded %d/%zu prices from yfinance\n", matched, TICKER_SYMBOLS.size());
        } else if (data.contains("error")) {
            std::printf("[StockTicker] Script error: %s\n", data["error"].get<std::string>().c_str());
        }
    } catch (const std::exception& e) {
        std::printf("[StockTicker] JSON parse error: %s\nRaw: %.200s\n", e.what(), json_str.c_str());
    }
}
