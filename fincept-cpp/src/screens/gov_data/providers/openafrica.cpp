...
static constexpr auto& COLOR = provider_colors::OPENAFRICA;

void OpenAfricaProvider::render() {
    pickup_result();
    ...
    ImGui::BeginChild("##oaf_body", ImVec2(0, 0));
    switch (active_view_) {
...
void OpenAfricaProvider::render_toolbar() {
    using namespace theme::colors;

    ImGui::PushStyleColor(ImGuiCol_ChildBg, BG_PANEL);
    ImGui::BeginChild("##oaf_toolbar", ImVec2(0, 36), ImGuiChildFlags_Borders);
    ...
    const char* tabs[] = {"ORGANIZATIONS", "DATASETS", "RESOURCES", "SEARCH", "RECENT"};
    for (int i = 0; i < 5; i++) {
    ...
    if (active_view_ == 3) {
        ImGui::SameLine(0, 12);
        if (SearchBar(search_buf_, sizeof(search_buf_), "Search openAFRICA...", COLOR, data_.is_loading()))
            fetch_search();
    }
...
// Fetch
void OpenAfricaProvider::fetch_organizations() {
    data_.run_script("openafrica_api.py", {"--action", "organizations"});
    status_ = "Fetching organizations..."; status_time_ = ImGui::GetTime();
}
void OpenAfricaProvider::fetch_datasets(const std::string& org_name) {
    selected_org_ = org_name;
    data_.run_script("openafrica_api.py", {"--action", "datasets", "--organization", org_name});
    status_ = "Fetching datasets..."; status_time_ = ImGui::GetTime();
}
void OpenAfricaProvider::fetch_resources(const std::string& dataset_id) {
    selected_dataset_ = dataset_id;
    data_.run_script("openafrica_api.py", {"--action", "resources", "--dataset", dataset_id});
    status_ = "Fetching resources..."; status_time_ = ImGui::GetTime();
}
void OpenAfricaProvider::fetch_search() {
    if (search_buf_[0] == '\0') return;
    data_.run_script("openafrica_api.py", {"--action", "search", "--query", search_buf_});
    status_ = "Searching..."; status_time_ = ImGui::GetTime();
}
void OpenAfricaProvider::fetch_recent() {
    data_.run_script("openafrica_api.py", {"--action", "recent"});
    status_ = "Fetching recent..."; status_time_ = ImGui::GetTime();
}

void OpenAfricaProvider::pickup_result() {
    ...
    if (data_.has_result()) {
        ...
        switch (active_view_) {
            ...
            case 4: { int d = 0; parse_datasets(r, recent_, d); } break;
        }
        status_ = "Data loaded";
...
// Parse (same pattern as Canada)
void OpenAfricaProvider::parse_organizations(const json& j) {
...
void OpenAfricaProvider::parse_datasets(const json& j, std::vector<DatasetRecord>& out, int& total) {
    out.clear();
    ...
    for (auto& d : arr) {
        ...
        rec.organization = d.contains("organization") && d["organization"].is_object()
            ? d["organization"].value("title", "") : d.value("organization_name", "");
...
void OpenAfricaProvider::parse_resources(const json& j) {
    resources_.clear();
...
// Render
void OpenAfricaProvider::render_organizations() {
    using namespace theme::colors;
    ...
    ImGui::TextColored(TEXT_DIM, "%zu organizations", organizations_.size());
    ImGui::Spacing();

    if (ImGui::BeginTable("##oaf_org_tbl", 4,
        ...
        for (auto& org : organizations_) {
            ...
            ImGui::TableNextColumn();
            ImGui::TextColored(COLOR, "%d", org.dataset_count);
            ImGui::TableNextColumn();
            ImGui::TextColored(TEXT_DIM, ">");
        }
        ImGui::EndTable();
...
void OpenAfricaProvider::render_datasets(const std::vector<DatasetRecord>& ds, int total) {
    using namespace theme::colors;
    ...
    if (ImGui::BeginTable("##oaf_ds_tbl", 5,
            ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg |
            ImGuiTableFlags_ScrollY | ImGuiTableFlags_Resizable)) {
        ImGui::TableSetupScrollFreeze(0, 1);
...
void OpenAfricaProvider::render_resources() {
    using namespace theme::colors;
    ...
    if (ImGui::BeginTable("##oaf_res_tbl", 5,
            ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg |
            ImGuiTableFlags_ScrollY | ImGuiTableFlags_Resizable)) {
        ImGui::TableSetupScrollFreeze(0, 1);
        ImGui::TableSetupColumn("Name");
