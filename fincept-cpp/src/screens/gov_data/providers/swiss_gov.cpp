...
static constexpr auto& COLOR = provider_colors::SWISS;

void SwissGovProvider::render() {
    pickup_result();
    ...
    ImGui::BeginChild("##swi_body", ImVec2(0, 0));
    switch (active_view_) {
...
void SwissGovProvider::render_toolbar() {
    using namespace theme::colors;

    ImGui::PushStyleColor(ImGuiCol_ChildBg, BG_PANEL);
    ImGui::BeginChild("##swi_toolbar", ImVec2(0, 36), ImGuiChildFlags_Borders);
    ...
    if (active_view_ == 3) {
        ImGui::SameLine(0, 12);
        if (SearchBar(search_buf_, sizeof(search_buf_), "Search Swiss datasets...", COLOR, data_.is_loading()))
            fetch_search();
    }
...
void SwissGovProvider::fetch_publishers() {
    data_.run_script("swiss_gov_api.py", {"--action", "publishers"});
    status_ = "Fetching publishers..."; status_time_ = ImGui::GetTime();
}
void SwissGovProvider::fetch_datasets(const std::string& publisher_id) {
    selected_publisher_ = publisher_id;
    data_.run_script("swiss_gov_api.py", {"--action", "datasets", "--publisher", publisher_id});
    status_ = "Fetching datasets..."; status_time_ = ImGui::GetTime();
}
void SwissGovProvider::fetch_resources(const std::string& dataset_id) {
    selected_dataset_ = dataset_id;
    data_.run_script("swiss_gov_api.py", {"--action", "resources", "--dataset", dataset_id});
    status_ = "Fetching resources..."; status_time_ = ImGui::GetTime();
}
void SwissGovProvider::fetch_search() {
    if (search_buf_[0] == '\0') return;
    data_.run_script("swiss_gov_api.py", {"--action", "search", "--query", search_buf_});
    status_ = "Searching..."; status_time_ = ImGui::GetTime();
}
void SwissGovProvider::fetch_recent() {
    data_.run_script("swiss_gov_api.py", {"--action", "recent"});
    status_ = "Fetching recent..."; status_time_ = ImGui::GetTime();
}

void SwissGovProvider::pickup_result() {
...
void SwissGovProvider::parse_publishers(const json& j) {
    publishers_.clear();
...
void SwissGovProvider::parse_datasets(const json& j, std::vector<DatasetRecord>& out, int& total) {
    out.clear();
...
void SwissGovProvider::parse_resources(const json& j) {
    resources_.clear();
...
// Render — identical table layout to Canada/openAFRICA
void SwissGovProvider::render_publishers() {
    using namespace theme::colors;
    if (data_.is_loading() && publishers_.empty()) { LoadingState("Loading...", COLOR); return; }
    if (publishers_.empty()) { EmptyState("No publishers loaded"); return; }

    ImGui::TextColored(TEXT_DIM, "%zu publishers", publishers_.size());
    ImGui::Spacing();

    if (ImGui::BeginTable("##swi_pub_tbl", 4,
...
void SwissGovProvider::render_datasets(const std::vector<DatasetRecord>& ds, int total) {
    using namespace theme::colors;
    ...
    if (ImGui::BeginTable("##swi_ds_tbl", 5,
            ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg |
            ImGuiTableFlags_ScrollY | ImGuiTableFlags_Resizable)) {
        ImGui::TableSetupScrollFreeze(0, 1);
...
void SwissGovProvider::render_resources() {
    using namespace theme::colors;
    ...
    if (ImGui::BeginTable("##swi_res_tbl", 5,
            ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg |
            ImGuiTableFlags_ScrollY | ImGuiTableFlags_Resizable)) {
        ImGui::TableSetupScrollFreeze(0, 1);
        ImGui::TableSetupColumn("Name");
