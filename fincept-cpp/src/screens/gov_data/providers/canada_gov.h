...
class CanadaGovProvider {
public:
    void render();

private:
    GovData data_;
    int active_view_ = 0; // 0=publishers, 1=datasets, 2=resources, 3=search, 4=recent

    std::vector<OrganizationRecord> publishers_;
    std::string selected_publisher_;
    ...
    void fetch_publishers();
    void fetch_datasets(const std::string& publisher_id);
    void fetch_resources(const std::string& dataset_id);
    void fetch_search();
    void fetch_recent();

    void render_toolbar();
    void render_publishers();
    void render_datasets(const std::vector<DatasetRecord>& ds, int total);
    void render_resources();
    void pickup_result();

    void parse_publishers(const nlohmann::json& j);
