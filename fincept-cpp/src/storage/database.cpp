...
std::optional<CacheEntry> cache_get(const std::string& key) {
    ...
    auto stmt = db.prepare(
        ...
        "last_accessed_at, hit_count FROM unified_cache WHERE cache_key = ?1");
    stmt.bind_text(1, key);
    if (!stmt.step()) return std::nullopt;
    ...
    CacheEntry entry{stmt.col_text(0), stmt.col_text(1), stmt.col_text(2),
                     stmt.col_int(3), stmt.col_int(4), expires_at,
                     stmt.col_int(7), expired};
